...
def run(ctx: ScanContext) -> list[Finding]:
    ...
    for path in _iter_py_files(ctx.repo):
        ...
        # AST passes: importorskip inside functions (only on added lines in --diff)
        if is_test:
            findings.extend(_ast_test_patterns(text, rel, ctx))

    return findings


def _ast_test_patterns(text: str, rel: str, ctx: ScanContext) -> list[Finding]:
    out: list[Finding] = []
    try:
        tree = ast.parse(text)
    except SyntaxError:
        ...
        for child in ast.walk(node):
            ...
            if isinstance(child, ast.Call):
                ...
                if name == 'pytest.importorskip' or name == 'importorskip':
                    ...
                    if lineno is not None and not ctx.is_added_line(rel, lineno):
                        continue
                    out.append(
                        Finding(
                            check=CHECK,
