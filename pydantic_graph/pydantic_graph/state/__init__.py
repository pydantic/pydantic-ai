...
__all__ = 'StateT', 'NodeSnapshot', 'EndSnapshot', 'Snapshot', 'StatePersistence', 'node_type_adapter'
...
class EndSnapshot(Generic[StateT, RunEndT]):
    ...
    state: StateT
    """The state of the graph at the end of teh run."""
    result: RunEndT
...
class StatePersistence(ABC, Generic[StateT, RunEndT]):
    ...
    async def snapshot_node(
        ...
        Args:
            ...
            node_type_adapter: Pydantic [`TypeAdapter`][pydantic.TypeAdapter] for the node.
    ...
    async def snapshot_end(
        ...
        Args:
            ...
            end_data_type_adapter: a Pydantic [`TypeAdapter`][pydantic.TypeAdapter] for the end data.
        """
        raise NotImplementedError
...
def node_type_adapter(
    ...
    debug([repr(node) for node in nodes])
    token = _utils.nodes_schema_var.set(nodes)
    try:
        ta = pydantic.TypeAdapter(BaseNode[state_t, Any, end_t])
    finally:
        _utils.nodes_schema_var.reset(token)
    return ta
