...
- Use `asyncio.create_task` only for a task that must outlive the frame starting it. Then keep it in a set that outlives that frame too (`SyncStreamBridge._pump_tasks`, `RealtimeSession._background_tasks`): asyncio's own registry is a `WeakSet`, so a task nothing else refers to may be collected before it finishes. Pass `name=` when there are many of a kind (`_tool_execution.py` names each by tool), and cancel *and await* it via `_utils.cancel_and_drain` in the owner's `finally` — `cancel()` requests, it does not tear down.
