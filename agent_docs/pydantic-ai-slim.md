...
Listeners run when the event's stream position is consumed, except for a `CapabilityEvent` family declared `dispatch='inline'`, which is dispatched before `emit` returns so the emitter can read decision fields listeners set. Under Temporal, `emit` from a tool or event stream handler raises: those run in activities that can't reach the buffer. Under DBOS and Prefect the buffer is in-process, so it works, but an event emitted inside a durable unit is a side effect of running it — a replayed step or a cached task does not re-emit it. See `durable_exec/AGENTS.md` and #7971.
