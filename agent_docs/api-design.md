...
<!-- rule:1187 -->
- When an upstream API or protocol removes a capability we already expose, make the option that configured it warn and degrade; reserve `UserError` for a requested operation that can't return a result — Raising on an option that used to work converts an upstream removal into a breaking change on our side, which the [version policy](../docs/version-policy.md) rules out for minor releases — The `raise UserError('... is not supported ...')` precedents in `models/` are not a counter-example: each one has raised since the surface was introduced, so no working code ever started failing. The test is whether the application can carry on without the capability. An unapplied log level or a sampling handler the server can no longer invoke leaves the application working, so warn and continue. An operation with nothing to return — a call the server can't run, an output mode the model can't produce — has no degraded form, so it raises.
