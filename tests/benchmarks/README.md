...
The replay benchmark captures a 256 KiB ASCII `FunctionModel` response in 1,024 or 4,096 chunks. Capture happens outside measurement. Keeping the total payload fixed isolates fragmentation and bounds allocation sizes. The test measures `CompletedStreamedResponse` replay through completion and checks its final response, not live generation or network latency.
