...
async def test_dynamic_toolset_args_validator_runs_in_temporal_activity():
    ...
    assert (
        temporal_toolset._unwrap_call_tool_result(  # pyright: ignore[reportPrivateUsage]
            await temporal_toolset._call_tool_in_activity(  # pyright: ignore[reportPrivateUsage]
                'guarded_weather', {'location': 'paris'}, ctx, tool
            )
        )
    ...
    with pytest.raises(ModelRetry, match='location `a` is blocked by validator'):
        temporal_toolset._unwrap_call_tool_result(  # pyright: ignore[reportPrivateUsage]
            await temporal_toolset._call_tool_in_activity(  # pyright: ignore[reportPrivateUsage]
                'guarded_weather', {'location': 'a'}, ctx, tool
            )
        )


async def test_dynamic_toolset_async_args_validator_runs_in_temporal_activity():
    toolset = FunctionToolset[None](id='async_validator_dynamic_function_toolset')
    validator_calls = 0

    async def args_validator(run_ctx: RunContext[None], location: str) -> None:
