...
@workflow.defn
class SimpleAgentWorkflowWithOverrideCapabilities:
    @workflow.run
    async def run(self, prompt: str) -> None:
        with simple_temporal_agent.override(capabilities=[]):
            pass


async def test_temporal_agent_override_capabilities_in_workflow(allow_model_requests: None, client: Client):
    async with Worker(
        client,
        task_queue=TASK_QUEUE,
        workflows=[SimpleAgentWorkflowWithOverrideCapabilities],
        plugins=[AgentPlugin(simple_temporal_agent)],
    ):
        with workflow_raises(
            UserError,
            ...
            await client.execute_workflow(
                SimpleAgentWorkflowWithOverrideCapabilities.run,
                args=['What is the capital of Mexico?'],
                id=SimpleAgentWorkflowWithOverrideCapabilities.__name__,
                task_queue=TASK_QUEUE,
            )
