...
# --- Durability with a `@agent.toolset`-registered dynamic toolset ---
# The decorator appends to the agent's toolsets *after* `TemporalDurability` bound in
# `Agent.__init__` and registered the activities for the toolsets it saw there, so the late
# registration has to be picked up for the toolset to get any activities at all.
# Regression for https://github.com/pydantic/pydantic-ai/issues/6902.


late_dynamic_durability = TemporalDurability[DynamicToolsetDeps](activity_config=BASE_ACTIVITY_CONFIG)
late_dynamic_agent = Agent(
    TestModel(),
    name='durability_late_dynamic',
    deps_type=DynamicToolsetDeps,
    capabilities=[late_dynamic_durability],
)


@late_dynamic_agent.toolset(id='late_dynamic_tools')
def late_dynamic_toolset(ctx: RunContext[DynamicToolsetDeps]) -> FunctionToolset[DynamicToolsetDeps]:
    toolset = FunctionToolset[DynamicToolsetDeps](id='late_dynamic_weather')

    @toolset.tool_plain
    def get_late_weather(location: str) -> str:
        """Get the weather for a location."""
        return f'Weather in {location} for {ctx.deps.user_name}: sunny.'

    return toolset


@workflow.defn
class LateDynamicToolsetWorkflow:
    @workflow.run
    async def run(self, prompt: str, deps: DynamicToolsetDeps) -> str:
        result = await late_dynamic_agent.run(prompt, deps=deps)
        return result.output


async def test_durability_decorator_registered_dynamic_toolset_in_workflow(client: Client):
    """A `@agent.toolset`-registered `DynamicToolset` gets its own activities and runs in them.

    The activity names are asserted directly because they're what the worker registers: with the
    late registration dropped, the toolset had no activities and its factory and tools ran inside
    the workflow sandbox instead — durably lost, but with a passing workflow.
    """
    durability = TemporalDurability.from_agent(late_dynamic_agent)
    assert durability is not None
    activity_names = {
        ActivityDefinition.must_from_callable(activity).name  # pyright: ignore[reportUnknownMemberType]
        for activity in durability.temporal_activities
    }
    assert 'agent__durability_late_dynamic__dynamic_toolset__late_dynamic_tools__get_tools' in activity_names
    assert 'agent__durability_late_dynamic__dynamic_toolset__late_dynamic_tools__call_tool' in activity_names

    async with Worker(
        client,
        task_queue=TASK_QUEUE,
        workflows=[LateDynamicToolsetWorkflow],
        plugins=[AgentPlugin(late_dynamic_agent)],
    ):
        output = await client.execute_workflow(
            LateDynamicToolsetWorkflow.run,
            args=['Get the weather for London', DynamicToolsetDeps(user_name='Alice')],
            id=LateDynamicToolsetWorkflow.__name__,
            task_queue=TASK_QUEUE,
        )
        assert output == snapshot('{"get_late_weather":"Weather in a for Alice: sunny."}')


def test_durability_toolset_decorator_requires_id():
    """`@agent.toolset` needs an `id` under Temporal, exactly like a `DynamicToolset` in `toolsets=`.

    The toolset is rejected before it lands on the agent, so a caught error doesn't leave behind a
    toolset that would run without activities.
    """
    agent = Agent(_durability_fn_model, name='durability_decorator_no_id', capabilities=[TemporalDurability()])

    with pytest.raises(UserError, match=r'need to have a unique `id` in order to be used with Temporal'):

        @agent.toolset
        def dynamic_toolset(ctx: RunContext[Any]) -> FunctionToolset[Any]:
            return FunctionToolset[Any]()  # pragma: no cover

    assert [toolset.id for toolset in agent.toolsets] == ['<agent>']


def test_durability_toolset_decorator_through_wrapper_capability():
    """A durability capability nested inside a `WrapperCapability` still sees the late registration."""
    agent = Agent(
        _durability_fn_model,
        name='durability_decorator_wrapped',
        capabilities=[WrapperCapability(wrapped=TemporalDurability())],
    )

    @agent.toolset(id='wrapped_dynamic_tools')
    def dynamic_toolset(ctx: RunContext[Any]) -> FunctionToolset[Any]:
        return FunctionToolset[Any]()  # pragma: no cover

    # A wrapper over a leaf capability is that leaf's proxy, so `from_agent` doesn't surface it.
    (wrapper,) = [cap for cap in agent.root_capability.capabilities if isinstance(cap, WrapperCapability)]
    durability = wrapper.wrapped
    assert isinstance(durability, TemporalDurability)
    activity_names = {
        ActivityDefinition.must_from_callable(activity).name  # pyright: ignore[reportUnknownMemberType]
        for activity in durability.temporal_activities
    }
    assert 'agent__durability_decorator_wrapped__dynamic_toolset__wrapped_dynamic_tools__get_tools' in activity_names


def test_durability_toolset_decorator_after_activities_handed_off():
    """Registering a toolset after the activities were handed to the worker raises.

    Temporal activities can only be registered before the worker starts, so unlike DBOS steps and
    Prefect tasks this one late registration can't be honored — it gets a clear error instead of an
    activity the worker has never heard of.
    """
    agent = Agent(_durability_fn_model, name='durability_decorator_late', capabilities=[TemporalDurability()])
    durability = TemporalDurability.from_agent(agent)
    assert durability is not None
    assert durability.temporal_activities

    with pytest.raises(UserError, match='after the agent'):

        @agent.toolset(id='too_late')
        def dynamic_toolset(ctx: RunContext[Any]) -> FunctionToolset[Any]:
            return FunctionToolset[Any]()  # pragma: no cover
