...
async def _anthropic_outbound(history: list[ModelMessage]) -> Any:
...
async def _xai_outbound(history: list[ModelMessage]) -> Any:
...
async def _bedrock_outbound(history: list[ModelMessage]) -> Any:
...
class Case:
    id: str
    outbound: Callable[[list[ModelMessage]], Awaitable[Any]]
...
async def test_foreign_thinking_marker(case: Case):
    """A foreign/unsigned `ThinkingPart` is re-rendered with the explicit foreign marker, never bare
    `<thinking>` tags; a history without thinking is unaffected."""
