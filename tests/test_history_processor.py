...
async def test_history_processor_replace_resumed_request_excluded(
    ...
    because its *position* — captured before the processor ran — is the boundary,
    not the (now-replaced) object identity.
    """
    ...
    # The resumed request's position was captured before the processor replaced it,
    # so new_messages() excludes it — only the model response is new.
    assert result.new_messages() == result.all_messages()[-1:]
