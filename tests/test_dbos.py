...
async def test_dbos_agent_run_in_workflow_rejects_runtime_mcp_toolset(dbos: DBOS):
    with workflow_raises(
        UserError,
        snapshot(
            'MCPToolset cannot be passed to `run(toolsets=...)` or `override(toolsets=...)` at runtime '
            'with DBOS, because toolsets that execute their own tools or resolve dynamically must be '
...
async def test_dbos_agent_run_in_workflow_rejects_runtime_dynamic_toolset(dbos: DBOS):
    with workflow_raises(
        UserError,
        snapshot(
            'DynamicToolset cannot be passed to `run(toolsets=...)` or `override(toolsets=...)` at runtime '
            'with DBOS, because toolsets that execute their own tools or resolve dynamically must be '
            'registered for durable execution when the agent is constructed. Pass them to the agent '
            'constructor instead. Non-executing toolsets like `ExternalToolset` can be passed at runtime.'
        ),
