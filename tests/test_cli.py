...
pytestmark = pytest.mark.skipif(
    not imports_successful(), reason="install cli extras to run cli tests"
)
...
def test_invalid_model(capfd: CaptureFixture[str]):
    ...
    assert capfd.readouterr().out.splitlines() == snapshot(
        ["Error initializing potato:", "Unknown model: potato"]
    )
...
def test_agent_flag(
    ...
    assert (
        "using custom agent test_module:custom_agent"
        in capfd.readouterr().out.replace("\n", "")
    )
...
def test_agent_flag_set_model(
    ...
    assert (
        cli(["--agent", "test_module:custom_agent", "--model", "gpt-4o", "hello"]) == 0
    )

    assert (
        "using custom agent test_module:custom_agent with openai:gpt-4o"
        in capfd.readouterr().out.replace("\n", "")
    )
...
def test_agent_flag_bad_module_variable_path(
    capfd: CaptureFixture[str], mocker: MockerFixture, env: TestEnv
):
    assert cli(["--agent", "bad_path", "hello"]) == 1
    assert (
        'Agent must be specified in "module:variable" format' in capfd.readouterr().out
    )
...
def test_list_models(capfd: CaptureFixture[str]):
    ...
    assert output[:3] == snapshot(
        [IsStr(regex="pai - PydanticAI CLI .*"), "", "Available models:"]
    )
    ...
    model_lines = [
        ...
        if line.strip()
        and not line.strip().startswith("Last")
...
def test_cli_prompt(capfd: CaptureFixture[str], env: TestEnv):
    ...
    with cli_agent.override(
        model=TestModel(custom_output_text="# result\n\n```py\nx = 1\n```")
    ):
        assert cli(["hello"]) == 0
        assert capfd.readouterr().out.splitlines() == snapshot(
            [IsStr(), "# result", "", "py", "x = 1", "/py"]
        )
...
def test_handle_slash_command_markdown():
    ...
    assert handle_slash_command(
        "/markdown", [], False, Console(file=io), "default"
    ) == (None, False)
    assert io.getvalue() == snapshot("No markdown output available.\n")

    messages: list[ModelMessage] = [
        ModelResponse(parts=[TextPart("[hello](# hello)"), ToolCallPart("foo", "{}")])
    ]
    io = StringIO()
    assert handle_slash_command(
        "/markdown", messages, True, Console(file=io), "default"
    ) == (None, True)
...
def test_handle_slash_command_multiline():
    io = StringIO()
    assert handle_slash_command(
        "/multiline", [], False, Console(file=io), "default"
    ) == (None, True)
    assert io.getvalue()[:70] == IsStr(regex=r"Enabling multiline mode.*")

    io = StringIO()
    assert handle_slash_command(
        "/multiline", [], True, Console(file=io), "default"
    ) == (None, False)
...
def test_code_theme_unset(mocker: MockerFixture, env: TestEnv):
    ...
    mock_run_chat.assert_awaited_once_with(
        True, IsInstance(Agent), IsInstance(Console), "monokai", "pai"
    )
...
def test_code_theme_light(mocker: MockerFixture, env: TestEnv):
    ...
    mock_run_chat.assert_awaited_once_with(
        True, IsInstance(Agent), IsInstance(Console), "default", "pai"
    )
...
def test_discover_models_use_saved_endpoint(
    capfd: CaptureFixture[str], mocker: MockerFixture
):
    ...
    mock_discover = mocker.patch(
        "pydantic_ai._cli.discover_local_models", return_value=[]
    )
...
def test_discover_direct_success(
    capfd: CaptureFixture[str], mocker: MockerFixture, env: TestEnv
):
    ...
    with patch("pydantic_ai._cli.load_discovery_config", return_value=mock_config):
        ...
        output = capfd.readouterr().out
        assert (
            "Connecting directly to: test-model at http://localhost:1234/v1" in output
        )
...
def test_discover_direct_with_invalid_timestamp(
    capfd: CaptureFixture[str], mocker: MockerFixture, env: TestEnv
):
...
def test_discover_models_with_invalid_last_used_timestamp(
    mocker: MockerFixture, capfd: CaptureFixture[str]
):
    ...
    mocker.patch(
        "pydantic_ai._cli.discover_local_models", return_value=["model-a", "model-b"]
    )
...
def test_discover_models_interactive_selection(
    mocker: MockerFixture, capfd: CaptureFixture[str]
):
...
def test_discover_models_invalid_selection(
    mocker: MockerFixture, capfd: CaptureFixture[str]
):
...
def test_discover_models_keyboard_interrupt(
    mocker: MockerFixture, capfd: CaptureFixture[str]
):
...
def test_discover_models_default_selection(
    mocker: MockerFixture, capfd: CaptureFixture[str]
):
...
def test_discover_models_exit_no_default(
    mocker: MockerFixture, capfd: CaptureFixture[str]
):
...
def test_discover_models_usererror(mocker: MockerFixture, capfd: CaptureFixture[str]):
    ...
    mocker.patch(
        "pydantic_ai._cli.discover_local_models", side_effect=UserError("fail!")
    )
