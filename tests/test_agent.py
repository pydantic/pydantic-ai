...
def test_result_pydantic_model_validation_error():
    ...
    messages_part_kinds = [
        (m.kind, [p.part_kind for p in m.parts]) for m in result.all_messages()
    ]
...
def test_output_validator_retries():
    ...
    agent = Agent(
        FunctionModel(return_model), output_type=Foo, output_retries=target_retries
    )
...
def test_output_function_retries():
    ...
    agent = Agent(
        ...
        output_type=TextOutput(get_weather),
...
class TestPartialOutput:
    ...
    def test_output_function_text(self):
        ...
        agent = Agent(
            FunctionModel(return_model), output_type=TextOutput(process_output)
        )
        result = agent.run_sync("test")
    ...
    def test_output_function_structured_get_output(self):
        ...
        agent = Agent(
            FunctionModel(return_model),
            output_type=ToolOutput(process_foo, name="my_output"),
        )
        result = agent.run_sync("test")
    ...
    def return_tuple(_: list[ModelMessage], info: AgentInfo) -> ModelResponse:
        ...
        if call_index == 1:
            ...
            return ModelResponse(
                parts=[ToolCallPart(info.output_tools[0].name, args_json)]
            )
...
def test_output_tool_return_content_no_tool():
    ...
    with pytest.raises(
        LookupError, match=re.escape("No tool call found with tool name 'wrong'.")
    ):
        result.all_messages(output_tool_return_content="foobar")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "type OutputType = Foo | Bar",
            marks=pytest.mark.skipif(sys.version_info < (3, 12), reason="3.12+"),
        ),
    ],
)
def test_response_multiple_return_tools(
    create_module: Callable[[str], Any], union_code: str
):
    module_code = f'''
...
def test_output_type_generic_class_name_sanitization():
    ...
    assert tool_names == snapshot(
        ["final_result_ResultGenericStringData", "final_result_ResultGenericint"]
    )
...
def test_output_type_with_two_descriptions():
    ...
    m = TestModel()
    agent = Agent(
        m, output_type=ToolOutput(MyOutput, description="Description from ToolOutput")
    )
    result = agent.run_sync("Hello")
...
def test_output_type_text_output_invalid():
    ...
    with pytest.raises(
        UserError,
        match="TextOutput must take a function taking a single `str` argument",
    ):
...
def test_output_type_function_with_custom_tool_name():
    ...
    agent = Agent(
        ...
        output_type=ToolOutput(get_weather, name="get_weather"),
    )
    result = agent.run_sync("Mexico City")
    assert result.output == snapshot(Weather(temperature=28.7, description="sunny"))
    assert output_tools == snapshot(
...
def test_default_structured_output_mode():
    ...
    native_model = TestModel(
        profile=ModelProfile(
            supports_json_schema_output=True, default_structured_output_mode="native"
        ),
...
def test_prompted_output():
    ...
    agent = Agent(
        ...
        output_type=PromptedOutput(
            CityLocation,
            name="City & Country",
            description="Description from PromptedOutput",
        ),
    )
...
def test_prompted_output_with_defs():
    ...
    agent = Agent(
        ...
        output_type=PromptedOutput(
            [FooBar, FooBaz],
            name="FooBar or FooBaz",
            description="FooBar or FooBaz description",
        ),
    )
...
def test_run_with_history_new():
    ...
    assert result1.new_messages() == snapshot(
        [
            ModelRequest(
                parts=[
                    SystemPromptPart(
                        content="Foobar", timestamp=IsNow(tz=timezone.utc)
                    ),
                    UserPromptPart(content="Hello", timestamp=IsNow(tz=timezone.utc)),
                ],
                timestamp=IsNow(tz=timezone.utc),
            ...
            ModelResponse(
                parts=[
                    ToolCallPart(
                        tool_name="ret_a", args={"x": "a"}, tool_call_id=IsStr()
                    )
                ],
    ...
    assert result2.all_messages() == snapshot(
        [
            ...
            ModelRequest(
                parts=[
                    UserPromptPart(
                        content="Hello again", timestamp=IsNow(tz=timezone.utc)
                    )
                ],
                timestamp=IsNow(tz=timezone.utc),
                run_id=IsStr(),
    ...
    assert result2.usage() == snapshot(
        RunUsage(requests=1, input_tokens=55, output_tokens=13)
    )
    new_msg_part_kinds = [
        (m.kind, [p.part_kind for p in m.parts]) for m in result2.all_messages()
    ]
    ...
    assert result2.new_messages_json().startswith(
        b'[{"parts":[{"content":"Hello again",'
    )
...
def test_run_with_history_new_structured():
    ...
    assert result2.all_messages() == snapshot(
        [
            ...
            ModelResponse(
                parts=[
                    ToolCallPart(
                        tool_name="final_result", args={"a": 0}, tool_call_id=IsStr()
                    )
                ],
                usage=RequestUsage(input_tokens=53, output_tokens=9),
                model_name="test",
    ...
    assert result2.usage() == snapshot(
        RunUsage(requests=1, input_tokens=59, output_tokens=13)
    )
    new_msg_part_kinds = [
        (m.kind, [p.part_kind for p in m.parts]) for m in result2.all_messages()
    ]
...
def test_run_with_history_ending_on_model_request_and_no_user_prompt():
    ...
    messages: list[ModelMessage] = [
        ModelRequest(
            parts=[
                SystemPromptPart(
                    content="System prompt", dynamic_ref=system_prompt.__qualname__
                ),
                UserPromptPart(
                ...
                parts=[
                    ...
                    UserPromptPart(
                        content=[
                            "Hello",
                            ImageUrl(
                                url="https://example.com/image.jpg", identifier="39cfc4"
                            ),
                        ],
                        timestamp=IsDatetime(),
                    ),
...
def test_run_with_history_ending_on_model_response_with_tool_calls_and_no_user_prompt():
    ...
    message_history = [
        ...
        ModelResponse(
            parts=[
                ToolCallPart(tool_name="test_tool", args="{}", tool_call_id="call_123")
            ]
        ),
    ]
    ...
    assert result.all_messages() == snapshot(
        [
            ...
            ModelResponse(
                parts=[
                    ToolCallPart(
                        tool_name="test_tool", args="{}", tool_call_id="call_123"
                    )
                ],
                timestamp=IsDatetime(),
            ),
...
async def test_message_history_ending_on_model_response_with_instructions():
    model = TestModel(
        custom_output_text="James likes cars in general, especially the Fiat 126p that his parents had."
    )
    ...
    assert result.output == snapshot(
        "James likes cars in general, especially the Fiat 126p that his parents had."
    )
    assert result.new_messages() == snapshot(
        [
            ModelRequest(
...
""",
            ...
            ModelResponse(
                parts=[
                    TextPart(
                        content="James likes cars in general, especially the Fiat 126p that his parents had."
                    )
                ],
...
def test_empty_response_without_recovery():
    ...
    with capture_run_messages() as messages:
        with pytest.raises(
            UnexpectedModelBehavior,
            match=r"Exceeded maximum retries \(1\) for output validation",
        ):
            agent.run_sync("Hello")
...
async def test_agent_iter_metadata_surfaces_on_result() -> None:
    agent = Agent(
        TestModel(custom_output_text="iter metadata output"), metadata={"env": "tests"}
    )
...
async def test_agent_metadata_override_with_dict() -> None:
    agent = Agent(
        TestModel(custom_output_text="override dict base"), metadata={"env": "base"}
    )
...
async def test_agent_metadata_override_with_callable() -> None:
    agent = Agent(
        TestModel(custom_output_text="override callable base"), metadata={"env": "base"}
    )
...
async def test_agent_run_metadata_kwarg_merges_agent_metadata() -> None:
    agent = Agent(
        TestModel(custom_output_text="kwarg merge output"),
        metadata={"env": "base", "shared": "agent"},
    )

    result = await agent.run(
        "kwarg merge prompt", metadata={"run": "value", "shared": "run"}
    )
...
async def test_agent_run_metadata_kwarg_ignored_with_override() -> None:
    agent = Agent(
        TestModel(custom_output_text="kwarg override output"), metadata={"env": "base"}
    )
...
def test_unknown_tool():
    ...
    assert messages == snapshot(
        [
            ...
            ModelResponse(
                parts=[
                    ToolCallPart(tool_name="foobar", args="{}", tool_call_id=IsStr())
                ],
...
def test_unknown_tool_multiple_retries():
    ...
    with capture_run_messages() as messages:
        with pytest.raises(
            UnexpectedModelBehavior,
            match=r"Exceeded maximum retries \(2\) for output validation",
        ):
            agent.run_sync("Hello")
    assert messages == snapshot(
        [
...
def test_tool_exceeds_token_limit_but_complete_args():
    def return_complete_tool_but_hit_limit(
        messages: list[ModelMessage], info: AgentInfo
    ) -> ModelResponse:
        if len(messages) == 1:
            resp = ModelResponse(
                parts=[ToolCallPart("dummy_tool", args='{"foo": "bar"}')]
            )
            resp.finish_reason = "length"
            return resp
...
def test_override_model_no_model():
    ...
    with pytest.raises(
        UserError,
        match=r"`model` must either be set.+Even when `override\(model=...\)` is customiz",
    ):
        with agent.override(model="test"):
            agent.run_sync("Hello")
...
class TestMultipleToolCalls:
    ...
    def test_early_strategy_stops_after_first_final_result(self):
        ...
        agent = Agent(
            FunctionModel(return_model), output_type=OutputType, end_strategy="early"
        )
        ...
        assert result.all_messages() == snapshot(
            [
                ModelRequest(
                    parts=[
                        UserPromptPart(
                            content="test early output tools",
                            timestamp=IsNow(tz=timezone.utc),
                        )
                    ],
                    timestamp=IsNow(tz=timezone.utc),
        ...
        assert messages == snapshot(
            [
                ModelRequest(
                    parts=[
                        UserPromptPart(
                            content="test multiple final results",
                            timestamp=IsNow(tz=timezone.utc),
                        )
                    ],
                    timestamp=IsNow(tz=timezone.utc),
    ...
    def test_early_strategy_with_deferred_tool_call(self):
        ...
        assert result.output == snapshot(
            DeferredToolRequests(
                calls=[ToolCallPart(tool_name="deferred_tool", tool_call_id=IsStr())]
            )
        )
        messages = result.all_messages()
        ...
        assert messages == snapshot(
            [
                ModelRequest(
                    parts=[
                        UserPromptPart(
                            content="test early strategy with deferred tool call",
                            timestamp=IsNow(tz=timezone.utc),
                        )
                    ],
                    timestamp=IsNow(tz=timezone.utc),
        ...
        assert result.all_messages() == snapshot(
            [
                ModelRequest(
                    parts=[
                        UserPromptPart(
                            content="test exhaustive strategy",
                            timestamp=IsNow(tz=timezone.utc),
                        )
                    ],
                    timestamp=IsNow(tz=timezone.utc),
        ...
        assert result.all_messages() == snapshot(
            [
                ModelRequest(
                    parts=[
                        UserPromptPart(
                            content="test exhaustive output tools",
                            timestamp=IsNow(tz=timezone.utc),
                        )
                    ],
                    timestamp=IsNow(tz=timezone.utc),
        ...
        assert result.all_messages() == snapshot(
            [
                ModelRequest(
                    parts=[
                        UserPromptPart(
                            content="test invalid first valid second",
                            timestamp=IsNow(tz=timezone.utc),
                        )
                    ],
                    timestamp=IsNow(tz=timezone.utc),
        ...
        assert result.all_messages() == snapshot(
            [
                ModelRequest(
                    parts=[
                        UserPromptPart(
                            content="test valid first invalid second",
                            timestamp=IsNow(tz=timezone.utc),
                        )
                    ],
                    timestamp=IsNow(tz=timezone.utc),
        ...
        assert result.all_messages() == snapshot(
            [
                ModelRequest(
                    parts=[
                        UserPromptPart(
                            content="test exhaustive with tool retry",
                            timestamp=IsNow(tz=timezone.utc),
                        )
                    ],
                    timestamp=IsNow(tz=timezone.utc),
    ...
    def test_exhaustive_raises_unexpected_model_behavior(self):
        ...
        with pytest.raises(
            UnexpectedModelBehavior,
            match="Exceeded maximum retries \\(1\\) for output validation",
        ):
            agent.run_sync("test")
    ...
    def test_multiple_final_result_are_validated_correctly(self):
        ...
        def return_model(_: list[ModelMessage], info: AgentInfo) -> ModelResponse:
            ...
            return ModelResponse(
                parts=[
                    ToolCallPart(
                        "final_result", {"bad_value": "first"}, tool_call_id="first"
                    ),
                    ToolCallPart(
                        "final_result", {"value": "second"}, tool_call_id="second"
                    ),
                ],
            )
...
async def test_model_settings_override() -> None:
    ...
    assert (
        await my_agent.run("Hello", model_settings={"temperature": 0.5})
    ).output == IsJson({"temperature": 0.5})

    my_agent = Agent(
        FunctionModel(return_settings), model_settings={"temperature": 0.1}
    )
        ...
        [
            ModelRequest(
                parts=[
                    ...
                    SystemPromptPart(
                        content=dynamic_value, timestamp=IsNow(tz=timezone.utc)
                    ),
                    UserPromptPart(content="Hello", timestamp=IsNow(tz=timezone.utc)),
                ],
                timestamp=IsNow(tz=timezone.utc),
...
def test_dynamic_system_prompt_no_changes():
    ...
    manual_request = ModelRequest(
        parts=[SystemPromptPart(content="Static"), UserPromptPart(content="Manual")]
    )

    # Mix dynamic and non-dynamic messages to trigger branch coverage
    result2 = agent.run_sync(
        "Second call", message_history=result1.all_messages() + [manual_request]
    )
...
def test_capture_run_messages_tool_agent() -> None:
    ...
    assert messages == snapshot(
        [
            ModelRequest(
                parts=[
                    UserPromptPart(content="foobar", timestamp=IsNow(tz=timezone.utc))
                ],
                timestamp=IsNow(tz=timezone.utc),
                run_id=IsStr(),
            ),
            ModelResponse(
                parts=[
                    ToolCallPart(
                        tool_name="foobar", args={"x": "a"}, tool_call_id=IsStr()
                    )
                ],
...
def test_custom_output_type_sync() -> None:
    ...
    assert agent.run_sync("Hello", output_type=str).output == snapshot(
...
def test_custom_output_type_invalid() -> None:
    ...
    with pytest.raises(
        UserError,
        match="Cannot set a custom run `output_type` when the agent has output validators",
    ):
...
def test_tool_return_part_binary_content_serialization():
    ...
    tool_return = ToolReturnPart(
        tool_name="test_tool", content=binary_content, tool_call_id="test_call_123"
    )
...
def test_instructions_raise_error_when_system_prompt_is_set():
    ...
    assert result.all_messages()[0] == snapshot(
        ModelRequest(
            parts=[
                SystemPromptPart(
                    content="A system prompt!", timestamp=IsNow(tz=timezone.utc)
                ),
                UserPromptPart(content="Hello", timestamp=IsNow(tz=timezone.utc)),
            ],
            timestamp=IsNow(tz=timezone.utc),
...
def test_instructions_both_instructions_and_system_prompt_are_set():
    agent = Agent(
        "test", instructions="An instructions!", system_prompt="A system prompt!"
    )
    result = agent.run_sync("Hello")
    assert result.all_messages()[0] == snapshot(
        ModelRequest(
...
def test_instructions_parameter_with_sequence():
    ...
    agent = Agent(
        "test",
        instructions=("You are a helpful assistant.", empty_instructions, instructions),
    )
    result = agent.run_sync("Hello")
    assert result.all_messages()[0] == snapshot(
        ModelRequest(
...
def test_empty_final_response():
    def llm(messages: list[ModelMessage], info: AgentInfo) -> ModelResponse:
        if len(messages) == 1:
            return ModelResponse(
                parts=[TextPart("foo"), ToolCallPart("my_tool", {"x": 1})]
            )
        elif len(messages) == 3:
            return ModelResponse(
                parts=[TextPart("bar"), ToolCallPart("my_tool", {"x": 2})]
            )
        else:
            return ModelResponse(parts=[])
        ...
        [
            ...
            ModelResponse(
                parts=[
                    TextPart(content="foo"),
                    ToolCallPart(
                        tool_name="my_tool", args={"x": 1}, tool_call_id=IsStr()
                    ),
                ],
                usage=RequestUsage(input_tokens=51, output_tokens=5),
                model_name="function:llm:",
            ...
            ModelResponse(
                parts=[
                    ...
                    ToolCallPart(
                        tool_name="my_tool", args={"x": 2}, tool_call_id=IsStr()
                    ),
                ],
                    ...
                    "content": [
                        {
                            "type": "value_error",
                            "loc": ["bar"],
                            "msg": "Value error, bar cannot be 0",
                            "input": 0,
                        }
                    ],
                    "tool_name": "foo_tool",
...
def test_unsupported_output_mode():
    ...
    model = FunctionModel(
        hello,
        profile=ModelProfile(supports_tools=False, supports_json_schema_output=False),
    )

    agent = Agent(model, output_type=NativeOutput(Foo))

    with pytest.raises(
        UserError, match="Native structured output is not supported by this model."
    ):
        agent.run_sync("Hello")
...
def test_multimodal_tool_response():
    ...
    def llm(messages: list[ModelMessage], info: AgentInfo) -> ModelResponse:
        if len(messages) == 1:
            return ModelResponse(
                parts=[TextPart("Starting analysis"), ToolCallPart("analyze_data", {})]
            )
        else:
            return ModelResponse(
                parts=[
                ...
                parts=[
                    ...
                    UserPromptPart(
                        content=[
                            "Here are the analysis results:",
                            ImageUrl(
                                url="https://example.com/chart.jpg", identifier="672a5c"
                            ),
                            "The chart shows positive trends.",
                        ],
                        timestamp=IsNow(tz=timezone.utc),
...
def test_override_toolsets():
    ...
    assert result.output == snapshot(
        '{"baz":"Hello from baz","foo":"Hello from foo","bar":"Hello from bar"}'
    )
...
def test_toolset_factory():
    ...
    agent = Agent(
        FunctionModel(respond), toolsets=[via_toolsets_arg], prepare_tools=prepare_tools
    )
    ...
    async def via_toolset_decorator_for_entire_run(
        ctx: RunContext[None],
...
def test_sequential_calls(mode: Literal["argument", "contextmanager"]):
    ...
    agent = Agent(
        FunctionModel(call_tools_sequential),
        toolsets=[sequential_toolset],
        output_type=[str, DeferredToolRequests],
    )
    ...
    assert result.output == snapshot(
        DeferredToolRequests(
            approvals=[
                ToolCallPart(tool_name="requires_approval", tool_call_id=IsStr())
            ]
        )
    )
    assert integer_holder == 2
...
def test_set_mcp_sampling_model():
    ...
    server2 = MCPServerStdio(
        "python", ["-m", "tests.mcp_server"], sampling_model=test_model
    )
    toolset = CombinedToolset([server1, PrefixedToolset(server2, "prefix")])
    agent = Agent(None, toolsets=[toolset])

    with pytest.raises(
        UserError, match="No sampling model provided and no model set on the agent."
    ):
    ...
    function_model = FunctionModel(
        lambda messages, info: ModelResponse(parts=[TextPart("Hello")])
    )
    ...
    function_model2 = FunctionModel(
        lambda messages, info: ModelResponse(parts=[TextPart("Goodbye")])
    )
...
async def test_wrapper_agent():
    ...
    assert wrapper_agent.output_json_schema(output_type=str) == snapshot(
        {"type": "string"}
    )
...
async def test_hitl_tool_approval():
            ...
            return ModelResponse(
                parts=[
                    ...
                    ToolCallPart(
                        ...
                        args={"path": "ok_to_delete.py"},
                        tool_call_id="ok_to_delete",
                    ),
                    ToolCallPart(
                        tool_name="delete_file",
                        args={"path": "never_delete.py"},
    ...
    result = await agent.run(
        ...
        deferred_tool_results=DeferredToolResults(
            approvals={
                "ok_to_delete": True,
                "never_delete": ToolDenied("File cannot be deleted"),
            },
        ),
    )
    assert result.all_messages() == snapshot(
...
async def test_run_with_deferred_tool_results_errors():
    ...
    with pytest.raises(
        UserError,
        match="Tool call results need to be provided for all deferred tool calls.",
    ):
        await agent.run(
            message_history=message_history,
            deferred_tool_results=DeferredToolResults(),
    ...
    with pytest.raises(
        UserError,
        match="Tool call results were provided, but the message history is empty.",
    ):
        await agent.run(
            "Hello again",
            deferred_tool_results=DeferredToolResults(approvals={"create_file": True}),
    ...
    message_history: list[ModelMessage] = [
        ...
        ModelRequest(
            parts=[
                ToolReturnPart(
                    tool_name="run_me", tool_call_id="run_me", content="Success"
                ),
                RetryPromptPart(
                    tool_name="run_me_too", tool_call_id="run_me_too", content="Failure"
                ),
            ]
        ),
    ]
    ...
    with pytest.raises(
        UserError,
        match="Tool call 'run_me' was already executed and its result cannot be overridden.",
    ):
        await agent.run(
            message_history=message_history,
            deferred_tool_results=DeferredToolResults(
    ...
    with pytest.raises(
        UserError,
        match="Tool call 'run_me_too' was already executed and its result cannot be overridden.",
    ):
        await agent.run(
            message_history=message_history,
            deferred_tool_results=DeferredToolResults(
...
async def test_user_prompt_with_deferred_tool_results():
    ...
    def llm(messages: list[ModelMessage], info: AgentInfo) -> ModelResponse:
        ...
        # Second call: model responds to tool results and user prompt
            ...
            has_tool_return = any(
                isinstance(p, ToolReturnPart) for p in last_request.parts
            )
            has_user_prompt = any(
                isinstance(p, UserPromptPart) for p in last_request.parts
            )
            ...
            user_prompt_content = next(
                p.content for p in last_request.parts if isinstance(p, UserPromptPart)
            )
            return ModelResponse(
                parts=[TextPart(f"Approved and {user_prompt_content}")]
            )
    ...
    assert messages == snapshot(
        [
            ModelRequest(
                parts=[
                    UserPromptPart(content="Update .env file", timestamp=IsDatetime())
                ],
                timestamp=IsNow(tz=timezone.utc),
                run_id=IsStr(),
            ),
    ...
    results = DeferredToolResults(
        approvals={result.output.approvals[0].tool_call_id: True}
    )
        ...
        [
            ...
            ModelResponse(
                parts=[
                    ...
                    TextPart(content="Anything else I can help with?"),
                ],
                timestamp=IsDatetime(),
            ),
            ModelRequest(
...
def test_override_instructions_sequence_mixed_types():
    ...
    with agent.override(
        instructions=["OVERRIDE1", override_fn, "OVERRIDE2", override_fn_2]
    ):
        with capture_run_messages() as messages:
            agent.run_sync("Hello", model=TestModel(custom_output_text="ok"))
...
def test_continue_conversation_that_ended_in_output_tool_call(
    allow_model_requests: None,
):
    def llm(messages: list[ModelMessage], info: AgentInfo) -> ModelResponse:
        if any(
            isinstance(p, ToolReturnPart) and p.tool_name == "roll_dice"
    ...
    assert not any(
        isinstance(p, ToolReturnPart) and p.tool_name == "final_result"
...
async def test_run_with_unapproved_tool_call_in_history():
    ...
    assert result.output == snapshot(
        DeferredToolRequests(
            approvals=[ToolCallPart(tool_name="delete_file", tool_call_id=IsStr())]
        )
    )
...
async def test_message_history():
    ...
    async with agent.iter(
        ...
        assert run.new_messages_json().startswith(
            b'[{"parts":[{"content":"ok here is text",'
        )
...
async def test_mixed_static_and_dynamic_builtin_tools():
    ...
    agent = Agent(
        model, builtin_tools=[static_tool, prepared_web_search], deps_type=UserContext
    )
...
async def test_central_content_filter_handling():
    ...
    with pytest.raises(
        ContentFilterError,
        match="Content filter triggered. Finish reason: 'content_filter'",
    ):
