...
def test_output_validator():
    ...
    def validate_output(ctx: RunContext[object], o: Foo) -> Foo:
...
def test_output_function_retries():
    ...
    def get_weather(ctx: RunContext[object], text: str) -> str:
        retries_log.append(ctx.retry)
        max_retries_log.append(ctx.max_retries)
        if ctx.retry == target_retries:
...
def test_tool_output_max_retries_per_tool():
    ...
    def output_a(ctx: RunContext[object], value: str) -> str:
        a_max_retries_seen.append(ctx.max_retries)
        if ctx.retry < 3:
            raise ModelRetry(f'Retry A {ctx.retry}')
        return f'A: {value}'

    def output_b(ctx: RunContext[object], value: str) -> str:
...
def test_response_union_allow_str(input_union_callable: Callable[[], Any]):
    ...
    @agent.output_validator
    def validate_output(ctx: RunContext[object], o: Any) -> Any:
...
def test_output_type_function_with_run_context():
    ...
    def get_weather(ctx: RunContext[object], city: str) -> Weather:
...
def test_output_type_bound_instance_method_with_run_context():
    class Weather(BaseModel):
        ...
        def get_weather(self, ctx: RunContext[object], city: str) -> Self:
            assert ctx is not None
            return self
...
def test_agent_conversation_id_available_in_run_context() -> None:
    ...
    def capture_metadata(ctx: RunContext[object]) -> dict[str, Any]:
...
async def test_agent_run_metadata_kwarg_callable() -> None:
    ...
    def run_meta(ctx: RunContext[object]) -> dict[str, Any]:
...
class TestMultipleToolCalls:
    ...
    def test_exhaustive_skips_output_tool_exceeding_retries_on_execution(self):
        ...
        def process_second(ctx: RunContext[object], value: str) -> str:
...
def test_custom_output_type_invalid() -> None:
    ...
    @agent.output_validator
    def validate_output(ctx: RunContext[object], o: Any) -> Any:  # pragma: no cover
        return o
...
def test_override_toolsets():
    ...
    async def prepare_tools(ctx: RunContext[object], tool_defs: list[ToolDefinition]) -> list[ToolDefinition]:
...
def test_toolset_factory():
    ...
    def via_toolsets_arg(ctx: RunContext[object]) -> AbstractToolset[object]:
    ...
    @agent.toolset
    def via_toolset_decorator(ctx: RunContext[object]) -> AbstractToolset[object]:
        nonlocal toolset_creation_counts
    ...
    async def via_toolset_decorator_for_entire_run(ctx: RunContext[object]) -> AbstractToolset[object]:
        nonlocal toolset_creation_counts
...
def test_prepare_output_tools_receives_output_max_retries():
    ...
    async def prep(ctx: RunContext[object], tool_defs: list[ToolDefinition]) -> list[ToolDefinition]:
...
async def test_wrap_run_readiness_wait_cancels_wrapper_task_on_outer_cancellation():
    ...
    class WrapRunCapability(AbstractCapability[object]):
...
async def test_wrapper_agent():
    async def event_stream_handler(ctx: RunContext[object], events: AsyncIterable[AgentStreamEvent]):
...
async def test_agent_allows_none_output_validator_called():
    ...
    @agent.output_validator
    async def validate_output(ctx: RunContext[object], output: str | None) -> str | None:
...
async def test_agent_allows_none_output_validator_retry():
    ...
    @agent.output_validator
    async def reject_none(ctx: RunContext[object], output: str | None) -> str | None:
        if output is None:
...
class TestCallableAgentLevelSettings:
    ...
    def test_callable_agent_settings(self):
        def dynamic_settings(ctx: RunContext[object]) -> ModelSettings:
...
class TestCallableRunLevelSettings:
    ...
    def test_callable_run_sees_merged_agent_settings(self):
        ...
        def run_settings(ctx: RunContext[object]) -> ModelSettings:
            seen_settings.append(ctx.model_settings)
...
class TestMixedStaticAndCallableSettings:
    ...
    def test_callable_agent_static_run(self):
        def agent_settings(ctx: RunContext[object]) -> ModelSettings:
...
class TestPerStepSettingsResolution:
    ...
    def test_step_dependent_settings(self):
        ...
        def step_dependent_settings(ctx: RunContext[object]) -> ModelSettings:
            if ctx.run_step <= 1:
                return ModelSettings(max_tokens=100)
...
class TestOverrideWithModelSettings:
    ...
    def test_override_with_callable(self):
        def override_settings(ctx: RunContext[object]) -> ModelSettings:
...
async def test_concurrent_runs_output_retry_isolation():
    ...
    @agent.output_validator
    async def validate_output(ctx: RunContext[object], o: Foo) -> Foo:
...
def test_output_validator_retry_counter_with_tool_switch():
    ...
    @agent.output_validator
    def validate_output(ctx: RunContext[object], o: str) -> str:
...
def test_output_retry_budget_resolution(case: OutputRetryBudgetCase):
    ...
    @agent.output_validator
    def always_retry(ctx: RunContext[object], o: Foo) -> Foo:
        retries_log.append(ctx.retry)
        raise ModelRetry(f'retry {ctx.retry}')
...
def test_text_path_honors_output_retry_budget():
    ...
    @agent.output_validator
    def always_retry(ctx: RunContext[object], o: str) -> str:
        retries_log.append(ctx.retry)
        raise ModelRetry(f'retry {ctx.retry}')
...
async def test_image_output_validators_run():
    ...
    @agent.output_validator
    def validate_output(ctx: RunContext[object], output: BinaryImage) -> BinaryImage:
