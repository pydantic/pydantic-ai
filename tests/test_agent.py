...
async def test_test_model_provider_name_consistency():
    """TestModel should set provider_name on ModelResponse in both run() and run_stream() paths."""
    agent = Agent(TestModel())

    result = await agent.run('hello')
    run_responses = [m for m in result.all_messages() if isinstance(m, ModelResponse)]
    assert all(r.provider_name == 'test' for r in run_responses)

    async with agent.run_stream('hello') as stream:
        await stream.get_output()
        stream_responses = [m for m in stream.all_messages() if isinstance(m, ModelResponse)]
        assert all(r.provider_name == 'test' for r in stream_responses)
