...
# =============================================================================
# SGLang — three-layer merge, Hugging Face repo IDs, single-system-message merge
# =============================================================================


def test_sglang_gpt_oss_hf_namespace():
    from pydantic_ai.providers.sglang import SGLangProvider

    profile = SGLangProvider.model_profile('openai/gpt-oss-20b')
    assert _normalize(profile) == snapshot(
        {
            'json_schema_transformer': OpenAIJsonSchemaTransformer,
            'supports_json_schema_output': True,
...
def test_sglang_qwen_hf_namespace():
    from pydantic_ai.providers.sglang import SGLangProvider

    profile = SGLangProvider.model_profile('Qwen/Qwen3-32B')
    assert _normalize(profile) == snapshot(
        {
            'json_schema_transformer': InlineDefsJsonSchemaTransformer,
            'ignore_streamed_leading_whitespace': True,
...
def test_sglang_unknown_falls_back_to_overlay_only():
    from pydantic_ai.providers.sglang import SGLangProvider

    profile = SGLangProvider.model_profile('some-unknown-model')
    assert _normalize(profile) == snapshot(
        {
            'json_schema_transformer': OpenAIJsonSchemaTransformer,
            'openai_chat_supports_document_input': False,
