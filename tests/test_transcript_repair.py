...
for such dangling calls and drops calls whose args were cut off mid-stream, so any history —
interrupted or hand-built — is provider-valid by construction.
...
async def test_repair_is_idempotent_and_deterministic():
    ...
    # Idempotency: feeding the repaired history into a new run leaves it untouched.
