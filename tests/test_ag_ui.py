...
async def test_thinking_event_family_switches_at_reasoning_version(
    ag_ui_version: str, expected_types: list[str]
) -> None:
    """The `THINKING_*`/`REASONING_*` switch happens at exactly `REASONING_VERSION`.

    Pinned at the boundary: 0.1.11 is the first version exporting the `REASONING_*` events, so it is
    also the first that can carry a thinking signature over the stream. The threshold used to sit at
    0.1.13 — the version of the day when it landed, not a protocol requirement — which cost 0.1.11
    and 0.1.12 clients that signature for no reason.
    """
    run_input = create_input(UserMessage(id='msg_1', content='test'))
