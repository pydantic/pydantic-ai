...
async def test_run_stream_external_tools():
    ...
    assert events == snapshot(
        [
            ...
            '</request>',
            "<run-result>DeferredToolRequests(calls=[ToolCallPart(tool_name='external_tool', args={}, tool_call_id='pyd_ai_tool_call_id__external_tool')], approvals=[], context={}, metadata={})</run-result>",
            '</stream>',
        ]
    )
