"""Tests for the shipped minimal `LocalSandboxBackend` implementation of the sandbox protocol."""
...
pytestmark = [
    pytest.mark.anyio,
    pytest.mark.skipif(os.name != 'posix', reason='LocalSandboxBackend tests drive POSIX shell commands'),
]
...
async def test_relative_paths_are_rejected(tmp_path: Path, operation: str):
    ...
    with pytest.raises(ValueError, match='absolute'):
        if operation == 'root':
            LocalSandboxBackend('work')
        elif operation == 'cwd':
            await LocalSandboxBackend(tmp_path).run(['pwd'], cwd='subdir')
        else:
            await LocalSandboxBackend(tmp_path).fs.write_bytes('outside.txt', b'escape')
...
async def test_kill_tolerates_an_already_exited_group():
    ...
    await process.wait()
    LocalSandboxBackend._kill(process)  # pyright: ignore[reportPrivateUsage]
...
async def test_abandoned_spawn_kill_falls_back_to_direct_child_on_denied_killpg(
    ...
    LocalSandboxBackend._kill_abandoned_spawn(spawn)  # pyright: ignore[reportPrivateUsage]
    await process.wait()
...
async def test_fresh_facade_follows_backend_across_root_recreation():
    """A fresh facade observes each new environment created by re-entering a backend."""
    backend = LocalSandboxBackend()
    async with backend:
        first = Path(await Sandbox(backend).working_dir())
    async with backend:
...
async def test_symlinked_root_with_dotdot_keeps_one_environment(tmp_path: Path):
    ...
    sandbox = Sandbox(LocalSandboxBackend(repo / 'link' / '..'))
...
async def test_read_file_on_a_directory_raises(tmp_path: Path):
    (tmp_path / 'adir').mkdir()
    sandbox = Sandbox(LocalSandboxBackend(tmp_path))
...
async def test_filesystem_round_trip_with_parent_creation(tmp_path: Path):
    backend = LocalSandboxBackend(tmp_path)
...
async def test_filesystem_reports_missing_paths(tmp_path: Path, operation: str):
    fs = LocalSandboxBackend(tmp_path).fs
    with pytest.raises(FileNotFoundError):
...
async def test_unused_default_sandbox_creates_no_directory(monkeypatch: pytest.MonkeyPatch):
    ...
    async with LocalSandboxBackend():
...
async def test_caller_supplied_root_is_never_removed(tmp_path: Path):
    async with LocalSandboxBackend(tmp_path) as sandbox:
...
async def test_local_sandbox_capability_run_end_to_end(tmp_path: Path):
    def model_func(messages: list[ModelMessage], info: AgentInfo) -> ModelResponse:
        if len(messages) == 1:
    ...
    async def execute(ctx: RunContext[Any]) -> str:
        working_dirs.append(await ctx.sandbox.working_dir())
        return (await ctx.sandbox.run(['pwd'])).stdout

    result = await agent.run('show the working directory')

    assert result.output == 'done'
    assert working_dirs == [tmp_path.resolve().as_posix()]


async def test_local_sandbox_capability_removes_per_run_root():
    def model_func(messages: list[ModelMessage], info: AgentInfo) -> ModelResponse:
        if len(messages) == 1:
    ...
    @agent.tool
    async def record_root(ctx: RunContext[Any]) -> str:
        root = Path(await ctx.sandbox.working_dir())
        roots.append(root)
        await ctx.sandbox.write_text('created.txt', 'yes')
        return 'created'

    result = await agent.run('create a temporary sandbox')
