"""Tests for CodeModeToolset logic (description, caching, name collisions, deferred tools).
...
from .conftest import StubRuntime, WeatherResult, build_code_mode_toolset, build_run_context, get_weather
...
async def test_get_tools_produces_single_code_tool():
    """get_tools() returns exactly one tool named 'run_code_with_tools'."""
    _, tools = await build_code_mode_toolset(StubRuntime(), (_add, False))
    assert list(tools.keys()) == ['run_code_with_tools']
...
async def test_description_default():
    ...
    # Preamble present
    assert 'code mode' in description
...
async def test_description_custom_string():
    ...
    cm = CodeModeToolset(ts, runtime=StubRuntime(), description='My preamble')
...
async def test_description_custom_callback():
    ...
    ts: FunctionToolset[None] = FunctionToolset()
    ts.add_function(_add, takes_ctx=False)
    cm = CodeModeToolset(ts, runtime=StubRuntime(), description=my_desc)
    tools = await cm.get_tools(build_run_context())
...
async def test_deferred_tools_raise_user_error():
    ...
    cm = CodeModeToolset(ts, runtime=StubRuntime())
...
async def test_cached_signature_reused_across_get_tools_calls():
    ...
    code_mode, tools1 = await build_code_mode_toolset(StubRuntime(), (_add, False))
...
async def test_dedup_correctness_after_cache_backed_deepcopy():
    ...
    _code_mode, tools = await build_code_mode_toolset(StubRuntime(), (get_weather, False), (_get_weather_alias, False))
