...
class TestReflectOnToolCall:
        ...
        def model_func(messages: list[ModelMessage], info: AgentInfo) -> ModelResponse:
            if len(messages) == 1:
                ...
                return ModelResponse(
                    parts=[ToolCallPart('tool_a', {'value': 'a'}), ToolCallPart('tool_b', {'value': 'b'})]
                )
            elif len(messages) == 3:
    ...
    def test_result_schema_with_reflection(self) -> None:
        ...
        def model_func(messages: list[ModelMessage], info: AgentInfo) -> ModelResponse:
            if len(messages) == 1:
                ...
                return ModelResponse(parts=[ToolCallPart(info.result_tools[0].name, {'value': 'structured_value'})])
            else:
                return ModelResponse(parts=[TextPart('Unexpected message sequence')])

        agent = Agent(FunctionModel(model_func), result_type=ResultData, reflect_on_tool_call=True)
    ...
    def test_result_schema_without_reflection(self) -> None:
        ...
        agent = Agent(FunctionModel(model_func), result_type=ResultData, reflect_on_tool_call=False)
