...
async def test_stdio_server(run_context: RunContext[int]):
    ...
    async with server:
        ...
        result = await server.call_mcp_tool('celsius_to_fahrenheit', {'celsius': 0})
...
async def test_stdio_server_with_tool_prefix(run_context: RunContext[int]):
    ...
    async with server:
        tool_names = (await server.get_tools(run_context)).keys()
        assert all(name.startswith('foo_') for name in tool_names)
...
async def test_log_level_unset(run_context: RunContext[int]):
    ...
    async with server:
        ...
        result = await server.call_mcp_tool('get_log_level', {})
...
async def test_client_sampling(run_context: RunContext[int]):
    ...
    async with server:
        result = await server.call_mcp_tool('use_sampling', {'foo': 'bar'})
...
async def test_client_sampling_disabled(run_context: RunContext[int]):
    ...
    async with server:
        with pytest.raises(ModelRetry, match='Error executing tool use_sampling: Sampling not supported'):
            await server.call_mcp_tool('use_sampling', {'foo': 'bar'})
    ...
    async with agent:
        with patch.object(
            ...
            with pytest.raises(ModelRetry, match='Test MCP error conversion'):
                await mcp_server.call_mcp_tool('test_tool', {})
