...
class TestMCPToolsetIntegration:
    ...
    async def test_get_tools_caches_and_lists(self, fastmcp_server: FastMCP[None], run_context: RunContext[object]):
    ...
    async def test_get_instructions_when_enabled(self, fastmcp_server: FastMCP[None], run_context: RunContext[object]):
        toolset = MCPToolset(fastmcp_server, include_instructions=True)
        async with toolset:
    ...
    async def test_get_instructions_returns_none_when_disabled(
        self, fastmcp_server: FastMCP[None], run_context: RunContext[object]
    ):
        toolset = MCPToolset(fastmcp_server)
        async with toolset:
    ...
    async def test_tools_no_caching_when_disabled(self, fastmcp_server: FastMCP[None], run_context: RunContext[object]):
    ...
    async def test_call_tool_returns_text(self, fastmcp_server: FastMCP[None], run_context: RunContext[object]):
        toolset = MCPToolset(fastmcp_server)
        async with toolset:
            tools = await toolset.get_tools(run_context)
    ...
    async def test_tool_error_raises_model_retry(self, fastmcp_server: FastMCP[None], run_context: RunContext[object]):
        toolset = MCPToolset(fastmcp_server)
        async with toolset:
            tools = await toolset.get_tools(run_context)
    ...
    async def test_process_tool_call_hook_runs(self, fastmcp_server: FastMCP[None], run_context: RunContext[object]):
    ...
    async def test_call_tool_returns_image(self, fastmcp_server: FastMCP[None], run_context: RunContext[object]):
    ...
    async def test_call_tool_returns_embedded_blob(
        self, fastmcp_server: FastMCP[None], run_context: RunContext[object]
    ):
        from pydantic_ai.messages import BinaryContent
