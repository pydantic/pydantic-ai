...
def model(openai_api_key: str) -> Model:
    return OpenAIChatCompletionsModel('gpt-4o', provider=OpenAIProvider(api_key=openai_api_key))
...
async def test_agent_with_prefix_tool_name(openai_api_key: str):
    server = MCPServerStdio('python', ['-m', 'tests.mcp_server'], tool_prefix='foo')
    model = OpenAIChatCompletionsModel('gpt-4o', provider=OpenAIProvider(api_key=openai_api_key))
    agent = Agent(
        model,
        toolsets=[server],
    )
