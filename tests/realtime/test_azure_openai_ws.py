...
with try_import() as imports_successful:
    ...
    from pydantic_ai.realtime.azure_openai import AzureOpenAIRealtimeModel
...
async def test_text_in_audio_out_turn(
    azure_openai_ws_cassette: tuple[AzureProvider, RealtimeCassette],
) -> None:
    """A text turn uses the GA session shape and produces Azure-hosted audio and transcript."""
    provider, cassette = azure_openai_ws_cassette
    model = AzureOpenAIRealtimeModel('gpt-realtime', provider=provider)
