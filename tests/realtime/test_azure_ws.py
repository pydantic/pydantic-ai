...
async def test_audio_in_server_vad_turn(
    azure_ws_cassette: tuple[AzureVoiceLiveProvider, RealtimeCassette], assets_path: Path
) -> None:
...
async def test_tool_call_round(
    ...
    model = AzureRealtimeModel(
        'gpt-realtime', provider=provider, settings=AzureRealtimeModelSettings(output_modality='text')
    )
