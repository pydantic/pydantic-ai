...
def _build_agent(model_name: str) -> Agent[object, str]:
    ...
    agent: Agent[object, str] = Agent(model=model)
...
async def test_tool_search_toolset_custom_search_fn_is_used():
    ...
    def custom_search(ctx: RunContext[object], queries: Sequence[str], tools: Sequence[ToolDefinition]) -> list[str]:
...
async def test_anthropic_promotes_local_search_history_round_trip(
    ...
    model = AnthropicModel('claude-sonnet-4-5', provider=AnthropicProvider(api_key=anthropic_api_key))
    agent: Agent[object, str] = Agent(model=model, capabilities=[ToolSearch()])
...
async def test_anthropic_native_tool_search_regex_strategy(allow_model_requests: None, anthropic_api_key: str) -> None:
    ...
    model = AnthropicModel('claude-sonnet-4-5', provider=AnthropicProvider(api_key=anthropic_api_key))
    agent: Agent[object, str] = Agent(model=model, capabilities=[ToolSearch(strategy='regex')])
...
async def test_openai_rejects_anthropic_named_strategy(allow_model_requests: None):
    ...
    agent: Agent[object, str] = Agent(model=model, capabilities=[ToolSearch(strategy='bm25')])
...
async def test_tool_search_named_strategy_agent_run_raises_on_unsupported_model(strategy: str):
    ...
    agent: Agent[object, str] = Agent(TestModel(), capabilities=[ToolSearch(strategy=cast(Any, strategy))])
...
async def test_tool_search_keywords_agent_run_falls_back_on_unsupported_model():
    ...
    agent: Agent[object, str] = Agent(TestModel(), capabilities=[ToolSearch(strategy='keywords')])
...
async def test_local_tool_search_stream_emits_typed_call_part_from_first_event() -> None:
    ...
    async def event_stream_handler(_ctx: RunContext[object], stream: AsyncIterable[AgentStreamEvent]) -> None:
...
async def test_tool_search_toolset_async_search_fn_is_awaited() -> None:
    ...
    async def async_match(
        _ctx: RunContext[object], _queries: Sequence[str], tools: Sequence[ToolDefinition]
    ) -> Sequence[str]:
