...
- Don't swap a *concrete* snapshot value for a matcher (`IsInt()`, `IsFloat()`, …) just to make a changed test pass — a small scalar (token counts, costs, ids) *is* the drift signal, so root-cause why it changed and re-pin it (`--inline-snapshot=fix`); reserve matchers for genuinely non-deterministic values or large opaque noise
