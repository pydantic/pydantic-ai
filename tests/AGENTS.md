...
- Do not run subprocesses from tests. Starting another interpreter and importing the project again makes the full suite materially slower. Move executable logic into importable functions and test it in-process. If imported modules must stay outside coverage, pause and restart the active `Coverage` collector around the call.
