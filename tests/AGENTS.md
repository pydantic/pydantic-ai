...
- Gate a suite on a missing optional dependency with `pytest.skip(..., allow_module_level=True)` in the test modules, never in a `conftest.py`, and keep that conftest importable without the dependency — deferring the imports its fixtures need into the fixture bodies. pytest loads the conftest of every command-line argument's directory before collection, and `_importconftest` catches only `Exception` while `Skipped` is a `BaseException`, so a gate there escapes as a traceback with exit 1 the moment someone names that directory (or a node id inside it) instead of a parent. See `tests/durable_exec/temporal/conftest.py` and the guard in `tests/durable_exec/test_temporal_skip_gating.py`
