...
async def test_adapter_load_tool_return_binary_data_unrecognized_shape_passes_through(data_payload: Any):
    """Unrecognized binary `data` shapes pass through `_js_binary_to_bytes` unchanged rather than the
    helper raising `KeyError`/`TypeError` on malformed input. Multimodal validation of the un-coerced
    `data` then fails, and the `ToolReturnContent` discriminator's passthrough
    (`_validate_multimodal_or_passthrough`) keeps the whole dict as a plain mapping instead of erroring."""
    ui_messages: list[UIMessage] = [
