...
_EXPECTED_EXCEPTION_EVENTS: dict[str, list[tuple[str, str, str]]] = {
    'retry': [
        ('running tool', 'pydantic_ai.exceptions.ToolRetryError', 'True'),
        ('running tool', 'pydantic_ai.exceptions.UnexpectedModelBehavior', 'True'),
        ('agent run', 'pydantic_ai.exceptions.UnexpectedModelBehavior', 'False'),
    ],
    'exception': [
        ('running tool', 'ValueError', 'True'),
        ('agent run', 'ValueError', 'False'),
    ],
}
...
@pytest.mark.skipif(not logfire_installed, reason='logfire not installed')
@pytest.mark.parametrize('include_content', [True, False])
@pytest.mark.parametrize('failure', ['retry', 'exception'])
def test_exception_events_honor_include_content(
    capfire: CaptureLogfire, include_content: bool, failure: Literal['retry', 'exception']
) -> None:
    """Exception events on tool and agent run spans carry a message and stack trace only when content is included.

    A `ToolRetryError` message is the retry prompt the model sees. When a tool runs out of retries, the
    `UnexpectedModelBehavior` that ends the run is chained from the last retry, and the OTel SDK's stack
    trace formatting repeats the cause's text on the second tool span and on the run span (the test
    exporter trims stack traces, so only the message is checked here). A plain exception from tool code
    is user-authored but may quote arguments, so it is treated the same way. With `include_content=False`,
    every event keeps only the exception type.
    """
    model_calls = 0
    ...
    if include_content:
        assert descriptions == [IsStr(regex=r'\w+: [\s\S]+')]
    else:
        assert descriptions == []
