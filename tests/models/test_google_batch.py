...
class TestGoogleBatchEmptyResults:
    ...
    async def test_batch_create_per_request_settings(self, allow_model_requests: None):
        ...
        # Verify first request has temperature 0.2 and max_output_tokens 100 (per-request settings)
        req_1_config = batch_call['src'][0]['config']
        assert req_1_config['temperature'] == 0.2
        assert req_1_config['max_output_tokens'] == 100

        # Verify second request has temperature 0.9 and max_output_tokens 200 (per-request settings)
        req_2_config = batch_call['src'][1]['config']
        assert req_2_config['temperature'] == 0.9
        assert req_2_config['max_output_tokens'] == 200
    ...
    async def test_batch_create_batch_wide_fallback(self, allow_model_requests: None):
        ...
        assert batch_call['src'][0]['config']['temperature'] == 0.7
        assert batch_call['src'][0]['config']['max_output_tokens'] == 500
        assert batch_call['src'][1]['config']['temperature'] == 0.7
        assert batch_call['src'][1]['config']['max_output_tokens'] == 500
    ...
    async def test_batch_create_settings_merging(self, allow_model_requests: None):
        ...
        # Per-request setting only overrides temperature, not max_output_tokens
        ...
        # First request: temperature from per-request (0.9), max_output_tokens from batch-wide (300)
        req_1_config = batch_call['src'][0]['config']
        assert req_1_config['temperature'] == 0.9
        assert req_1_config['max_output_tokens'] == 300

        # Second request: both from batch-wide
        req_2_config = batch_call['src'][1]['config']
        assert req_2_config['temperature'] == 0.5
        assert req_2_config['max_output_tokens'] == 300
