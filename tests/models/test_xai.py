...
def test_x_search_tool_validation():
    ...
    # from_date/to_date must match the xAI SDK contract
    invalid_from_date: Any = '2024-06-01'
    with pytest.raises(TypeError, match='`from_date` must be a `datetime` object'):
        XSearchTool(from_date=invalid_from_date)

    invalid_to_date: Any = '2024-12-31'
    with pytest.raises(TypeError, match='`to_date` must be a `datetime` object'):
        XSearchTool(to_date=invalid_to_date)
