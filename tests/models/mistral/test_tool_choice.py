"""Tests for Mistral tool_choice setting."""
...
async def test_tool_choice_auto_live(allow_model_requests: None, mistral_api_key: str):
    """Test tool_choice='auto' allows model to decide whether to use tools."""
    m = MistralModel('mistral-small-latest', provider=MistralProvider(api_key=mistral_api_key))
    agent = Agent(m)
...
async def test_tool_choice_required_live(allow_model_requests: None, mistral_api_key: str):
...
async def test_tool_choice_none_live(allow_model_requests: None, mistral_api_key: str):
...
async def test_tool_choice_specific_live(allow_model_requests: None, mistral_api_key: str):
