...
def test_tool_choice_none_with_multiple_output_tools() -> None:
    """tool_choice='none' with multiple output tools falls back to 'required'."""
    func_tool = ToolDefinition(
        name='func_tool',
        description='Function tool',
        parameters_json_schema={'type': 'object', 'properties': {}},
    )
    output_tool_a = ToolDefinition(
        name='output_a',
        description='Output tool A',
        parameters_json_schema={'type': 'object', 'properties': {}},
    )
    output_tool_b = ToolDefinition(
        name='output_b',
        description='Output tool B',
        parameters_json_schema={'type': 'object', 'properties': {}},
    )
    mrp = ModelRequestParameters(
        output_mode='tool',
        ...
        output_tools=[output_tool_a, output_tool_b],
    )
    ...
    # Both output tools are kept
    assert len(tools) == 2
    # Falls back to 'required' since HuggingFace can't force a specific tool from multiple
    assert result_tool_choice == 'required'
