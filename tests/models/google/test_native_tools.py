...
These unit tests pin request and response payload shapes that VCR matching does not validate.
"""
...
async def test_gemini_streamed_response_grounding_metadata_does_not_add_text_separator():
    streamed_response = _gemini_streamed_response_from_chunks(
        [
            _generate_stream_response('stream-1', parts=[{'text': 'Pydantic AI '}]),
            _generate_stream_response('stream-2', parts=[{'text': 'supports agents.'}]),
    ...
    deltas = [
        delta async for delta in _text_output_agent_stream(streamed_response).stream_text(delta=True, debounce_by=None)
    ]

    assert deltas == ['Pydantic AI ', 'supports agents.']
...
async def test_gemini_streamed_response_emits_non_grounded_tool_return_immediately():
    streamed_response = _gemini_streamed_response_from_chunks(
        [
            _generate_stream_response(
                'stream-1',
...
def test_web_search_none_tool_response_round_trips_unchanged():
    ...
    [round_tripped] = ModelMessagesTypeAdapter.validate_json(ModelMessagesTypeAdapter.dump_json([response]))
    assert isinstance(round_tripped, ModelResponse)
    mapped = _content_model_response(round_tripped, frozenset({'google-gla'}), supports_tool_combination=True)
...
async def test_file_search_multiple_calls_all_filled_streaming():
    ...
    assert [item.tool_call_id for item in returns] == ['call_1', 'call_2']
    assert all(item.content is not None for item in returns)
