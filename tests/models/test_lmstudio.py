...
def test_init():
    m = LMStudioModel('llama3.2', base_url='foobar/')
    assert m.openai_model.client.api_key == 'lmstudio'
    assert m.openai_model.client.base_url == 'foobar/'
    assert m.name() == 'lmstudio:llama3.2'


async def test_request_simple_success(allow_model_requests: None):
    c = completion_message(ChatCompletionMessage(content='world', role='assistant'))
    mock_client = MockOpenAI.create_mock(c)
    m = LMStudioModel('llama3.2', openai_client=mock_client, base_url=None)
    agent = Agent(m)
