...
class MockBedrockClient:
    ...
    @classmethod
    def create_mock(cls, completions: ConverseResponseTypeDef | list[ConverseResponseTypeDef]) -> MockBedrockClient:
...
def completion_message(content: list[ContentBlockOutputTypeDef], usage: TokenUsageTypeDef) -> ConverseResponseTypeDef:
    return {
        'ResponseMetadata': {
...
async def test_stream_tool(allow_model_requests: None):
    ...
    async with agent.run_stream('What is the capital of the UK?') as result:
        ...
        assert result.usage() == snapshot(Usage(requests=1, request_tokens=417, response_tokens=76, total_tokens=493))
...
async def test_stream_text(allow_model_requests: None):
    ...
    async with agent.run_stream('What is the capital of the UK?') as result:
        ...
        assert result.is_complete
        assert result.usage() == snapshot(Usage(requests=1, request_tokens=7, response_tokens=27, total_tokens=34))
