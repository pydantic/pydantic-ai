...
with try_import() as imports_successful:
    ...
    from pydantic_ai.models.openai import (
        ...
        _native_tool_names_factory,  # pyright: ignore[reportPrivateUsage]
...
def test_openai_responses_native_tool_names_factory():
    assert _native_tool_names_factory() == {}
    assert _native_tool_names_factory() is not _native_tool_names_factory()
...
async def _text_editor_execute(command: TextEditorCommand) -> TextEditorOutput:
    return TextEditorOutput(output=f'Handled {command["command"]}')
...
async def test_apply_patch_execute_helper_variants() -> None:
    ...
    edited = await _text_editor_execute({'command': 'view', 'path': '/tmp/example.py'})
    assert edited == TextEditorOutput(output='Handled view')
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            TextEditorToolset(execute=_text_editor_execute),
            'text_editor',
            id='text-editor',
        ),
    ],
...
async def test_hosted_shell_multi_turn_round_trip(allow_model_requests: None):
    """Multi-turn hosted shell: NativeToolCallPart/NativeToolReturnPart round-trip through ShellCall/ShellCallOutput."""
    ...
    # Second turn: pass message_history back — triggers NativeToolCallPart→ShellCall and NativeToolReturnPart→ShellCallOutput round-trip
...
async def test_shell_tool_param_with_network_policy(allow_model_requests: None):
    ...
    agent = Agent(
        m,
        capabilities=[NativeTool(ShellTool(network_policy=CodeExecutionNetworkPolicy(mode='disabled')))],
    )
    await agent.run('hello')
    ...
    agent2 = Agent(
        ...
        capabilities=[
            NativeTool(
                ShellTool(network_policy=CodeExecutionNetworkPolicy(mode='allowlist', allowed_domains=['example.com']))
            ),
        ],
    )
    await agent2.run('hello')
...
async def test_shell_call_output_in_response(allow_model_requests: None):
    """ResponseFunctionShellToolCallOutput in non-streaming response maps to NativeToolReturnPart (lines 3549-3565)."""
    from openai.types.responses import (
        ResponseFunctionShellToolCall,
        ResponseFunctionShellToolCallOutput,
        ResponseOutputMessage,
...
async def test_hosted_shell_without_container_id_round_trip(allow_model_requests: None):
    """NativeToolCallPart without container_id skips environment (line 2268->2273)."""
    from openai.types.responses import (
        ResponseFunctionShellToolCall,
        ResponseFunctionShellToolCallOutput,
        ResponseOutputMessage,
...
async def test_shell_output_edge_cases_in_response(allow_model_requests: None):
    ...
    returns = [p for msg in result.all_messages() for p in msg.parts if isinstance(p, NativeToolReturnPart)]
...
async def test_code_interpreter_with_network_policy(allow_model_requests: None):
    ...
    m = OpenAIResponsesModel('gpt-5.4', provider=OpenAIProvider(openai_client=mock_client))
    agent = Agent(
        m, capabilities=[NativeTool(CodeExecutionTool(network_policy=CodeExecutionNetworkPolicy(mode='disabled')))]
    )
    await agent.run('hello')
