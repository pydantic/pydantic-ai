...
async def test_system_prompt_role(
    ...
    assert OpenAIModelProfile.from_profile(m.profile).openai_system_prompt_role == system_prompt_role
...
async def test_openai_pass_custom_system_prompt_role(allow_model_requests: None, openai_api_key: str):
    ...
    resolved = OpenAIModelProfile.from_profile(model.profile)
    assert resolved.openai_system_prompt_role == 'user'
    assert resolved.supports_tools is False
