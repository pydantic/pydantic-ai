...
async def test_openai_web_search_tool_model_not_supported(
    ...
    with pytest.raises(
        UserError,
        match=r"WebSearchTool is not supported with `OpenAIChatModel` and model.*",
    ):
    ...
    def model_profile(model_name: str) -> ModelProfile:
        return ModelProfile(
            json_schema_transformer=(
                InlineDefsJsonSchemaTransformer if model_name == "gpt-4o" else None
            )
        )
...
def test_openai_response_timestamp_milliseconds(allow_model_requests: None):
    ...
    assert response.timestamp == snapshot(
        datetime(2025, 6, 1, 3, 7, 48, tzinfo=timezone.utc)
    )
...
async def test_openai_model_settings_temperature_ignored_on_gpt_5(
    ...
    result = await agent.run(
        "What is the capital of France?", model_settings=ModelSettings(temperature=0.0)
    )
