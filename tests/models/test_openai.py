...
def test_openai_image_generation_tool_aspect_ratio_invalid() -> None:
    ...
    with pytest.raises(
        UserError, match="OpenAI image generation only supports `aspect_ratio` values"
    ):
...
async def test_response_with_created_timestamp_but_no_provider_details(
    ...
    class MinimalOpenAIChatModel(OpenAIChatModel):
        def _process_provider_details(
            self, response: chat.ChatCompletion
        ) -> dict[str, Any] | None:
            return None

    c = completion_message(ChatCompletionMessage(content="world", role="assistant"))
    mock_client = MockOpenAI.create_mock(c)
    m = MinimalOpenAIChatModel(
        "gpt-4o", provider=OpenAIProvider(openai_client=mock_client)
    )
    agent = Agent(m)
...
async def test_openai_chat_image_detail_vendor_metadata(allow_model_requests: None):
    ...
    image_url = ImageUrl(
        "https://example.com/image.png", vendor_metadata={"detail": "high"}
    )
    binary_image = BinaryContent(
        b"\x89PNG", media_type="image/png", vendor_metadata={"detail": "high"}
    )
    ...
    image_parts = [
        item["image_url"]
        for item in request_kwargs[0]["messages"][0]["content"]
...
async def test_request_tool_call(allow_model_requests: None):
    responses = [
        ...
        completion_message(
            ChatCompletionMessage(content="final response", role="assistant")
        ),
    ]
...
def chunk(
    ...
    return chat.ChatCompletionChunk(
        id="123",
        choices=[
            ChunkChoice(index=index, delta=delta, finish_reason=finish_reason)
...
def text_chunk(
    text: str, finish_reason: FinishReason | None = None
) -> chat.ChatCompletionChunk:
    return chunk(
        [ChoiceDelta(content=text, role="assistant")], finish_reason=finish_reason
    )
...
async def test_stream_text(allow_model_requests: None):
    ...
    async with agent.run_stream("") as result:
        ...
        assert result.usage() == snapshot(
            RunUsage(requests=1, input_tokens=6, output_tokens=3)
        )
...
async def test_stream_structured(allow_model_requests: None):
    ...
    async with agent.run_stream("") as result:
        assert not result.is_complete
        assert [
            dict(c) async for c in result.stream_output(debounce_by=None)
        ] == snapshot(
            [
                {},
                {"first": "One"},
        ...
        assert result.is_complete
        assert result.usage() == snapshot(
            RunUsage(requests=1, input_tokens=20, output_tokens=10)
        )
...
async def test_stream_tool_call_with_empty_text(allow_model_requests: None):
                ...
                ChoiceDelta(
                    ...
                    tool_calls=[
                        ChoiceDeltaToolCall(
                            index=0,
                            function=ChoiceDeltaToolCallFunction(
                                name="final_result", arguments=None
                            ),
                        )
                    ],
                ),
    ...
    mock_client = MockOpenAI.create_mock_stream(stream)
    m = OpenAIChatModel(
        "gpt-oss:20b", provider=OllamaProvider(openai_client=mock_client)
    )
...
@pytest.mark.filterwarnings(
    "ignore:Set the `system_prompt_role` in the `OpenAIModelProfile` instead."
)
...
async def test_system_prompt_role_o1_mini(
...
async def test_openai_pass_custom_system_prompt_role(
    allow_model_requests: None, openai_api_key: str
):
...
async def test_parallel_tool_calls(
    allow_model_requests: None, parallel_tool_calls: bool
) -> None:
    c = completion_message(
        ChatCompletionMessage(
            content=None,
            ...
            tool_calls=[
                ChatCompletionMessageFunctionToolCall(
                    id="123",
                    function=Function(
                        arguments='{"response": [1, 2, 3]}', name="final_result"
                    ),
                    type="function",
                )
            ],
    ...
    await agent.run("Hello")
    assert (
        get_mock_chat_completion_kwargs(mock_client)[0]["parallel_tool_calls"]
        == parallel_tool_calls
    )
...
async def test_openai_audio_url_input(allow_model_requests: None, openai_api_key: str):
    m = OpenAIChatModel(
        "gpt-4o-audio-preview", provider=OpenAIProvider(api_key=openai_api_key)
    )
    agent = Agent(m)

    result = await agent.run(
        ["Hello", AudioUrl(url="https://cdn.openai.com/API/docs/audio/alloy.wav")]
    )
...
async def test_document_url_input(allow_model_requests: None, openai_api_key: str):
    ...
    assert result.output == snapshot(
        'The document contains the text "Dummy PDF file" on its single page.'
    )
...
async def test_image_url_force_download_chat() -> None:
    ...
    with patch(
        "pydantic_ai.models.openai.download_item", new_callable=AsyncMock
    ) as mock_download:
...
async def test_document_url_force_download_responses() -> None:
    ...
    m = OpenAIResponsesModel(
        "gpt-4.5-nano", provider=OpenAIProvider(api_key="test-key")
    )
...
async def test_image_url_tool_response(allow_model_requests: None, openai_api_key: str):
    ...
    result = await agent.run(
        ["What food is in the image you can get from the get_image tool?"]
    )
    assert result.all_messages() == snapshot(
        [
            ModelRequest(
...
async def test_image_as_binary_content_tool_response(
    ...
    result = await agent.run(
        ["What fruit is in the image you can get from the get_image tool?"]
    )
    assert result.all_messages() == snapshot(
        [
            ModelRequest(
...
async def test_document_as_binary_content_input(
    ...
    result = await agent.run(
        ["What is the main content on this document?", document_content]
    )
    assert result.output == snapshot(
        'The main content of the document is "Dummy PDF file."'
    )
...
async def test_text_document_url_input(allow_model_requests: None, openai_api_key: str):
    ...
    result = await agent.run(
        ["What is the main content on this document, in one sentence?", document_url]
    )
    assert result.output == snapshot(
...
async def test_text_document_as_binary_content_input(
    ...
    result = await agent.run(
        ["What is the main content on this document?", text_document_content]
    )
    assert result.output == snapshot(
...
async def test_document_as_binary_content_input_with_tool(
    ...
    assert result.output == snapshot(
        'The main content of the document is "DUMMY PDF FILE" in uppercase.'
    )
...
def test_model_status_error(allow_model_requests: None) -> None:
    mock_client = MockOpenAI.create_mock(
        APIStatusError(
            ...
            response=httpx.Response(
                status_code=500, request=httpx.Request("POST", "https://example.com/v1")
            ),
    ...
    assert str(exc_info.value) == snapshot(
        "status_code: 500, model_name: gpt-4o, body: {'error': 'test error'}"
    )
...
def test_model_connection_error(allow_model_requests: None) -> None:
    ...
    assert "Connection to http://localhost:11434/v1 timed out" in str(
...
def test_responses_model_connection_error(allow_model_requests: None) -> None:
    ...
    m = OpenAIResponsesModel(
        "o3-mini", provider=OpenAIProvider(openai_client=mock_client)
    )
    agent = Agent(m)
    with pytest.raises(ModelAPIError) as exc_info:
        agent.run_sync("hello")
...
async def test_max_completion_tokens(
    allow_model_requests: None, model_name: str, openai_api_key: str
):
...
async def test_multiple_agent_tool_calls(
    allow_model_requests: None, gemini_api_key: str, openai_api_key: str
):
    gemini_model = GoogleModel(
        "gemini-2.0-flash-exp", provider=GoogleProvider(api_key=gemini_api_key)
    )
    openai_model = OpenAIChatModel(
        "gpt-4o-mini", provider=OpenAIProvider(api_key=openai_api_key)
    )
...
async def test_message_history_can_start_with_model_response(
    ...
    openai_model = OpenAIChatModel(
        "gpt-4.1-mini", provider=OpenAIProvider(api_key=openai_api_key)
    )

    message_history = [
        ModelResponse(parts=[TextPart("Where do you want to go today?")])
    ]

    agent = Agent(model=openai_model)

    result = await agent.run(
        "Answer in 5 words only. Who is Tux?", message_history=message_history
    )
            ...
            snapshot(
                {
                    "additionalProperties": False,
                    "properties": {
                        "x": {"format": "uri", "minLength": 1, "type": "string"}
                    },
                    "required": ["x"],
                    "type": "object",
                }
...
async def test_strict_mode_cannot_infer_strict(
    ...
    async def assert_strict(
        expected_strict: bool | None, profile: ModelProfile | None = None
    ):
        mock_client = MockOpenAI.create_mock(c)
        m = OpenAIChatModel(
            "gpt-4o",
...
def test_strict_schema():
    ...
    assert OpenAIJsonSchemaTransformer(
        MyModel.model_json_schema(), strict=True
    ).walk() == snapshot(
        {
            "$defs": {
                "Apple": {
...
def test_native_output_strict_mode(allow_model_requests: None):
    ...
    c = completion_message(
        ChatCompletionMessage(
            content='{"city": "Mexico City", "country": "Mexico"}', role="assistant"
        ),
    )
    mock_client = MockOpenAI.create_mock(c)
    model = OpenAIChatModel(
    ...
    assert (
        get_mock_chat_completion_kwargs(mock_client)[-1]["response_format"][
            "json_schema"
        ]["strict"]
        is True
    )
...
async def test_openai_web_search_tool(allow_model_requests: None, openai_api_key: str):
    m = OpenAIChatModel(
        "gpt-4o-search-preview", provider=OpenAIProvider(api_key=openai_api_key)
    )
    agent = Agent(
        m,
        instructions="You are a helpful assistant.",
...
async def test_openai_web_search_tool_with_user_location(
    ...
    agent = Agent(
        ...
        instructions="You are a helpful assistant.",
        builtin_tools=[
            WebSearchTool(user_location={"city": "Utrecht", "country": "NL"})
        ],
    )
...
def test_openai_model_profile_function():
    ...
    m = OpenAIChatModel(
        "gpt-4o", provider=OpenAIProvider(api_key="foobar"), profile=model_profile
    )
    assert isinstance(m.profile, ModelProfile)
    assert m.profile.json_schema_transformer is InlineDefsJsonSchemaTransformer

    m = OpenAIChatModel(
        "gpt-4o-mini", provider=OpenAIProvider(api_key="foobar"), profile=model_profile
    )
    assert isinstance(m.profile, ModelProfile)
...
async def test_compatible_api_with_tool_calls_without_id(
    ...
    assert result.all_messages() == snapshot(
        [
            ...
            ModelResponse(
                parts=[
                    ToolCallPart(
                        tool_name="get_user_country", args="{}", tool_call_id=IsStr()
                    )
                ],
                usage=RequestUsage(
                    input_tokens=68,
...
async def test_openai_text_output_function(
    allow_model_requests: None, openai_api_key: str
):
    m = OpenAIChatModel("gpt-4o", provider=OpenAIProvider(api_key=openai_api_key))
...
async def test_openai_native_output_multiple(
    allow_model_requests: None, openai_api_key: str
):
    m = OpenAIChatModel("gpt-4o", provider=OpenAIProvider(api_key=openai_api_key))
...
async def test_openai_prompted_output_multiple(
    allow_model_requests: None, openai_api_key: str
):
    m = OpenAIChatModel("gpt-4o", provider=OpenAIProvider(api_key=openai_api_key))
...
async def test_tool_choice_fallback(allow_model_requests: None) -> None:
    profile = OpenAIModelProfile(openai_supports_tool_choice_required=False).update(
        openai_model_profile("stub")
    )

    mock_client = MockOpenAI.create_mock(
        completion_message(ChatCompletionMessage(content="ok", role="assistant"))
    )
    model = OpenAIChatModel(
        "stub", provider=OpenAIProvider(openai_client=mock_client), profile=profile
    )

    params = ModelRequestParameters(
        function_tools=[ToolDefinition(name="x")], allow_text_output=False
    )
...
async def test_openai_gpt_5_2_temperature_warns_when_reasoning_enabled(
    ...
    with pytest.warns(
        ...
        await agent.run(
            "What is the capital of France?",
            model_settings=OpenAIChatModelSettings(
                temperature=0.5, openai_reasoning_effort="medium"
            ),
        )
...
async def test_openai_model_cerebras_provider(
    allow_model_requests: None, cerebras_api_key: str
):
    m = OpenAIChatModel(
        "llama3.3-70b", provider=CerebrasProvider(api_key=cerebras_api_key)
    )
    agent = Agent(m)
...
async def test_openai_model_cerebras_provider_qwen_3_coder(
    ...
    m = OpenAIChatModel(
        "qwen-3-coder-480b", provider=CerebrasProvider(api_key=cerebras_api_key)
    )
...
async def test_openai_model_cerebras_provider_harmony(
    allow_model_requests: None, cerebras_api_key: str
):
    m = OpenAIChatModel(
        "gpt-oss-120b", provider=CerebrasProvider(api_key=cerebras_api_key)
    )
    agent = Agent(m)
...
async def test_openai_custom_reasoning_field_sending_back_in_thinking_tags(
    ...
    c = completion_message(
        ChatCompletionMessage.model_construct(
            content="response", reasoning_content="reasoning", role="assistant"
        )
    )
    m = OpenAIChatModel(
        "foobar",
    ...
    resp = await m.request(
        messages=[], model_settings=settings, model_request_parameters=params
    )
...
async def test_openai_reasoning_in_thinking_tags(allow_model_requests: None):
    c = completion_message(
        ChatCompletionMessage.model_construct(
            content="<think>reasoning</think>response", role="assistant"
        )
    )
    m = OpenAIChatModel(
        "foobar",
...
def test_azure_prompt_filter_error(allow_model_requests: None) -> None:
    ...
    mock_client = MockOpenAI.create_mock(
        APIStatusError(
            "content filter",
            response=httpx.Response(
                status_code=400, request=httpx.Request("POST", "https://example.com/v1")
            ),
            body=body,
        )
    )
...
async def test_openai_response_filter_error(allow_model_requests: None):
    ...
    mock_client = MockOpenAI.create_mock(c)
    m = OpenAIChatModel(
        "gpt-5-mini", provider=OpenAIProvider(openai_client=mock_client)
    )
    agent = Agent(m)
...
def test_openai_chat_audio_default_base64(allow_model_requests: None):
    ...
    mock_client = MockOpenAI.create_mock(c)
    model = OpenAIChatModel(
        "gpt-4o-audio-preview", provider=OpenAIProvider(openai_client=mock_client)
    )
    agent = Agent(model)
    ...
    audio_part = next(
        part for part in user_message["content"] if part["type"] == "input_audio"
    )
...
def test_openai_chat_audio_uri_encoding(allow_model_requests: None):
    ...
    # Expect Data URI
    expected_data = (
        f"data:audio/wav;base64,{base64.b64encode(audio_data).decode('utf-8')}"
    )
    assert audio_part["input_audio"]["data"] == expected_data
    assert audio_part["input_audio"]["format"] == "wav"
