...
- Map hooks by firing point, inputs, ordering, mutation, blocking, and error behavior. Claude hook matchers may run concurrently; Pydantic AI hooks run in capability order. Require a golden trace for any hook-visible contract.
...
- Harness command controls are best-effort policy, not an OS security boundary. Use application-owned container, VM, or sandbox isolation for untrusted execution.
