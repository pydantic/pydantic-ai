...
3. Run this check from the candidate worktree, `cd "${CANDIDATE_DIR:?substitute the literal path noted in Step 1}" || { echo "candidate worktree gone; stop"; exit 1; }` first; the policy base always answers "missing". Verify the branch-context files are missing or still the unfilled templates — on a clean checkout `.claude/skills/branch-context/` holds only the templates and helpers, and the live files do not exist yet:
...
or a worktree-relative path — with
`cd "${CANDIDATE_DIR:?substitute the literal path noted in Step 1}" || { echo "candidate worktree gone; stop"; exit 1; }`,
which stops instead of staying put when the variable is gone or the directory is not, and keeps
the `|| { … exit 1; }` clause when the literal path replaces the variable: the helpers write to whatever
...
Start with `cd "${CANDIDATE_DIR:?substitute the literal path noted in Step 1}" || { echo "candidate worktree gone; stop"; exit 1; }`. Use the same schema as `/initialize-worktree` Step 3 (see
