...
- **The SDK bump is usually blocked by the 7-day quarantine.** `exclude-newer = "7 days"` in the root `pyproject.toml` keeps a same-day `anthropic` release out of the lock, so use the local-`Literal` bridge below rather than an `exclude-newer-package` override.
