  ...
  async fetch(request, env): Promise<Response> {
    ...
    if (r.status == 404) {
      ...
      if (contentType && contentType.includes('text/html')) {
        const handler = new InsertVersionNoticeHandler(env)
...
class InsertVersionNoticeHandler {
  ...
  async element(element: Element): Promise<void> {
    try {
      const warning = await getVersionNotice(this.env)
...
async function getVersionNotice(env: Env): Promise<string> {
  const cacheKey = `version-notice-${env.GIT_BRANCH}-${env.GIT_COMMIT_SHA}`
  let versionNotice = await env.VERSION_NOTICE_CACHE.get(cacheKey, {cacheTtl: 60})
  if (versionNotice === null) {
    versionNotice = await fetchVersionNotice(env)
    await env.VERSION_NOTICE_CACHE.put(cacheKey, versionNotice, {expirationTtl: 300})
  }
...
async function fetchVersionNotice(env: Env): Promise<string> {
  const headers = {
  ...
  const r2 = await fetch(
    `https://api.github.com/repos/pydantic/pydantic-ai/compare/${tag_name}...${env.GIT_COMMIT_SHA}`,
    {headers}
  )
  if (!r2.ok) {
