...
async function getChangelog(
  ...
  while (typeof url === "string") {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to fetch changelog: ${response.status} ${response.statusText} ${text}`,
      );
    }
...
function prepRelease(release: Release): string {
  const body = release.body
    .replace(/(#+)/g, (m) => `##${m}`)
    .replace(
      /https:\/\/github.com\/pydantic\/pydantic-ai\/pull\/(\d+)/g,
      (url, id) => `[#${id}](${url})`,
    )
    .replace(
      /(\s)@([\w-]+)/g,
      (_, s, u) => `${s}[@${u}](https://github.com/${u})`,
    )
    .replace(
      /\*\*Full Changelog\*\*: (\S+)/,
      (_, url) => `[${githubIcon} Compare diff](${url}).`,
    );
  return `
### ${release.name}
 ...
 */
async function maybeGetTextResponse(
  request: Request,
  env: Env,
