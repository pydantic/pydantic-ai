...
class AnthropicModelSettings(ModelSettings, total=False):
    ...
    anthropic_download_code_execution_files: bool
    """Whether to automatically download files generated by the Anthropic code execution tool.

    When enabled, any files created during code execution will be downloaded from the Anthropic
    Files API and returned as `FilePart` objects in the model response.
    This adds additional API calls for each generated file.
    """
...
class AnthropicModel(Model[AsyncAnthropicClient]):
    ...
    async def request(
        ...
        try:
            ...
            process_response = self._process_response(response)

            if model_settings and model_settings.get('anthropic_download_code_execution_files'):
                new_parts = list(process_response.parts)
                for part in process_response.parts:
                    if isinstance(part, BuiltinToolReturnPart) and part.tool_name == CodeExecutionTool.kind:
                        if isinstance(part.content, dict) and (content := part.content.get('content')):  # type: ignore
                            for file_output in content:  # type: ignore
                                file_id = cast(str, file_output.get('file_id'))  # type: ignore
                                if not file_id:
                                    continue
                                file_part = await _download_anthropic_file(
                                    self.client, file_id=file_id, provider_name=self._provider.name
                                )
                                new_parts.append(file_part)
                process_response.parts = new_parts
    ...
    async def request_stream(
        ...
        async with response:
            yield await self._process_streamed_response(
                response, model_request_parameters, cast(AnthropicModelSettings, model_settings or {})
            )
...
class AnthropicStreamedResponse(StreamedResponse):
    ...
    async def _get_event_iterator(self) -> AsyncIterator[ModelResponseStreamEvent]:  # noqa: C901
        with _map_api_errors(self._model_name):
            ...
            async for event in self._response:
                if isinstance(event, BetaRawMessageStartEvent):
                    ...
                    if isinstance(current_block, BetaTextBlock) and current_block.text:
                        ...
                        if isinstance(current_block.content, BetaCodeExecutionResultBlock):
                            for content in current_block.content.content:
                                _file_ids.append(content.file_id)
            ...
            if self._model_settings.get('anthropic_download_code_execution_files'):
                for file_id in _file_ids:
                    if not file_id:
                        continue
                    file_part = await _download_anthropic_file(
                        self._client, file_id=file_id, provider_name=self._provider_name
                    )
                    yield self._parts_manager.handle_part(vendor_part_id=file_id, part=file_part)
...
async def _download_anthropic_file(client: AsyncAnthropicClient, *, file_id: str, provider_name: str) -> FilePart:
    """Download a file from the Anthropic Files API and return it as a FilePart."""
    file_content = await client.beta.files.download(file_id=file_id)  # type: ignore[attr-defined]
    file_bytes = await file_content.read()  # type: ignore

    metadata = await client.beta.files.retrieve_metadata(file_id=file_id)  # type: ignore[attr-defined]
    content = BinaryContent(media_type=metadata.mime_type, data=file_bytes)  # type: ignore
