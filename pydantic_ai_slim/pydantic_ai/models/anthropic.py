...
def _reraise_anthropic_error(model_name: AnthropicModelName, exc: BaseException) -> None:
    """Map Anthropic SDK HTTP/connection errors to pydantic-ai model errors (streaming and non-streaming)."""
    if isinstance(exc, APIStatusError):
        if (status_code := exc.status_code) >= 400:
            raise ModelHTTPError(status_code=status_code, model_name=model_name, body=exc.body) from exc
        raise ModelAPIError(model_name=model_name, message=exc.message) from exc  # pragma: lax no cover
    if isinstance(exc, APIConnectionError):
        raise ModelAPIError(model_name=model_name, message=exc.message) from exc
    raise exc
...
class AnthropicModel(Model):
    ...
    async def _messages_create(
        ...
        try:
            ...
            _reraise_anthropic_error(self.model_name, e)
...
class AnthropicStreamedResponse(StreamedResponse):
    ...
    async def _get_event_iterator(self) -> AsyncIterator[ModelResponseStreamEvent]:  # noqa: C901
        ...
        try:
            ...
            _reraise_anthropic_error(self._model_name, e)
