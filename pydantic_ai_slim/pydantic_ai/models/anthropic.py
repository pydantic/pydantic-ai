        ...
        try:
            ...
            if model_settings.get('anthropic_download_code_execution_files'):
                ...
                for part in process_response.parts:
                    if isinstance(part, NativeToolReturnPart) and part.tool_name == CodeExecutionTool.kind:
                        for file_id in _code_execution_file_ids(part.content):
            ...
            async for event in self._response:
                if isinstance(event, BetaRawMessageStartEvent):
                    ...
                    if isinstance(current_block, BetaTextBlock) and current_block.text:
                        ...
                        _file_ids.extend(_code_execution_file_ids(part.content))
                        yield self._parts_manager.handle_part(vendor_part_id=event.index, part=part)
...
def _code_execution_file_ids(content: Any) -> list[str]:
    """Extract the file IDs referenced by a code execution tool result's (dumped) content."""
    if not _utils.is_str_dict(content) or not isinstance(outputs := content.get('content'), list):
        return []
    return [
        file_id
        for output in cast('list[Any]', outputs)
        if _utils.is_str_dict(output) and (file_id := output.get('file_id'))
    ]
