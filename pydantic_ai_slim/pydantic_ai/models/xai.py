    ...
    def _map_response_parts(self, parts: Sequence[ModelResponsePart]) -> chat_types.chat_pb2.Message | None:
        ...
        for item in parts:
            if isinstance(item, TextPart):
                ...
                # xAI handles built-in tool calls server-side, they're not sent back
                pass
        ...
        if texts or reasoning_texts or tool_calls:
            if reasoning_texts or tool_calls:
                # Start with assistant helper and add reasoning_content or tool_calls
                if texts:
                    msg = assistant('\n\n'.join(texts))
                else:
                    # Create empty assistant message if no text content
                ...
                # Use helper for simple text-only messages
                return assistant('\n\n'.join(texts))
