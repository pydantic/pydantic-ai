...
<!-- rule:26 -->
- Don't add preemptive client-side guards that reject provider settings based on assumed capability limits; verify the limitation by testing and let the provider API surface the actual incompatibility — this governs settings only, not content/part types a provider can't represent (those still raise, to avoid silently dropping user data) — Prevents degrading functionality with workarounds based on outdated assumptions, since the API is the authority on what it currently supports
<!-- rule:478 -->
