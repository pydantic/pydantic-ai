...
def _otel_message_role(part: ModelRequestPart, version: int) -> _otel_messages.Role:
    ...
    `system`, because that is what it renders as: feedback that belongs to no tool call reaches the
    model in the system voice, so the span shows the role the model actually saw. Earlier versions
    keep it on `user`, so a consumer written against them keeps reading the role it was built for.
