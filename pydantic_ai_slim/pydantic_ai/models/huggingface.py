...
class HuggingFaceModel(Model[AsyncInferenceClient]):
    ...
    async def _map_messages(
        ...
        for message in messages:
            if isinstance(message, ModelRequest):
                ...
                hf_messages.append(self._map_response_message(message))
            else:
    ...
    def _map_response_message(self, message: ModelResponse) -> ChatCompletionInputMessage:
        texts: list[str] = []
        ...
        for item in message.parts:
            if isinstance(item, TextPart):
                ...
                if item.content and self.profile.get('huggingface_send_back_thinking_parts', False):
