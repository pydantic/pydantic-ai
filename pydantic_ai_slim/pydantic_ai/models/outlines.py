...
class OutlinesModelSettings(ModelSettings, total=False):
    ...
    The Outlines model settings are a bit different from the other model settings
    as Outlines is used to run various different models that do not support the same settings.
    All settings in the base model settings are explicitly treated: either provided to the
    model if supported or removed with a warning to avoid causing an error. You should add
    additional settings specific to the model you are using yourself.
    """
...
class OutlinesModel(Model):
    ...
    def _format_transformers_inference_kwargs(self, model_settings: dict[str, Any]) -> dict[str, Any]:
        ...
        for arg in [
            ...
            if model_settings.get(arg):
                ...
                warnings.warn(
                    f'The transformers model does not support the `{arg}` argument. '
        ...
        if logit_bias := model_settings.pop('logit_bias', None):
            sequence_bias = {}
        ...
        if model_settings.get('stop_sequences'):
            model_settings['stop_strings'] = model_settings.pop('stop_sequences', None)
    ...
    def _format_llama_cpp_inference_kwargs(self, model_settings: dict[str, Any]) -> dict[str, Any]:
        ...
        for arg in [
            ...
            if model_settings.pop(arg, None):
                warnings.warn(
                    f'The llama_cpp model does not support the `{arg}` argument. '
                    + 'The argument has been removed to avoid causing an error.',
                    UserWarning,
                )
        ...
        if model_settings.get('logit_bias'):
            if any(not isinstance(key, int) for key in model_settings['logit_bias'].keys()):
                model_settings.pop('logit_bias', None)
                warnings.warn(
        ...
        if model_settings.get('stop_sequences'):
            model_settings['stop'] = model_settings.pop('stop_sequences', None)
    ...
    def _format_mlxlm_inference_kwargs(  # pragma: no cover
        ...
        for arg in [
            ...
            if model_settings.get(arg):
                ...
                warnings.warn(
                    f'The mlxlm model does not support the `{arg}` argument. '
                    + 'The argument has been removed to avoid causing an error.',
                    UserWarning,
                )
    ...
    def _format_sglang_inference_kwargs(self, model_settings: dict[str, Any]) -> dict[str, Any]:
        ...
        for arg in [
            ...
            if model_settings.pop(arg, None):
                warnings.warn(
                    f'The SgLang model does not support the `{arg}` argument. '
                    + 'The argument has been removed to avoid causing an error.',
                    UserWarning,
                )
    ...
    def _format_vllm_offline_inference_kwargs(  # pragma: no cover
        ...
        for arg in [
            ...
            if model_settings.pop(arg, None):
                warnings.warn(
                    f'The vllm offline model does not support the `{arg}` argument. '
                    + 'The argument has been removed to avoid causing an error.',
                    UserWarning,
                )
