...
try:
    ...
    from outlines.models.dottxt import Dottxt, from_dottxt  # pyright: ignore[reportUnknownVariableType]
    ...
    from outlines.models.transformers import (  # pyright: ignore[reportUnknownVariableType]
        Transformers,
        from_transformers,
    )
    from outlines.models.vllm_offline import (
        VLLMOffline,  # pyright: ignore[reportUnknownVariableType]
...
class OutlinesModelSettings(ModelSettings, total=False):
    """Settings used for a Outlines model request."""

    # ALL FIELDS MUST BE `outlines_` PREFIXED SO YOU CAN MERGE THEM WITH OTHER MODELS.

    # This class is a placeholder for any future outlines-specific settings
...
class OutlinesModel(Model):
    ...
    @classmethod
    def from_llama_cpp(  # pragma: no cover
        cls,
        llama_model: Any,
        *,
        provider: Literal['outlines'] | Provider[OutlinesBaseModel] = 'outlines',
    ...
    @classmethod
    def from_sglang(  # pragma: no cover
        cls,
    ...
    @classmethod
    def from_dottxt(  # pragma: no cover
        cls,
        dottxt_client: Any,
        model_name: str,
        *,
    ...
    async def request(
        ...
        inference_kwargs = self._format_inference_kwargs(model_settings)
        if isinstance(self.model, OutlinesAsyncBaseModel):  # pragma: no cover
            response: str = await self.model(prompt, output_type, None, **inference_kwargs)
        else:
            response: str = self.model(prompt, output_type, None, **inference_kwargs)
    ...
    def _format_inference_kwargs(self, model_settings: ModelSettings | None) -> dict[str, Any]:
        ...
        if isinstance(self.model, Transformers):
            settings_dict = self._format_transfomers_inference_kwargs(settings_dict)
        elif isinstance(self.model, LlamaCpp):  # pragma: no cover
        ...
        elif isinstance(self.model, SGLang | AsyncSGLang):  # pragma: no cover
        ...
        elif isinstance(self.model, Dottxt):  # pragma: no cover
            settings_dict = self._format_dottxt_inference_kwargs(settings_dict)
        return settings_dict

    def _format_transfomers_inference_kwargs(  # pragma: no cover
        ...
        for arg in [
            ...
            if model_settings.get(arg):
                model_settings.pop(arg)
                warnings.warn(
                    'The transformers model does not support the `{arg}` argument. '
                    + 'The argument has been removed to avoid causing an error.',
                    UserWarning,
                )
        if model_settings.get('presence_penalty') or model_settings.get('frequency_penalty'):
            presence_penalty = model_settings.pop('presence_penalty')
            frequency_penalty = model_settings.pop('frequency_penalty')
            model_settings['repetition_penalty'] = frequency_penalty or presence_penalty
            warnings.warn(
                'The transformers model has a single argument `repetition_penalty` that combines '
                + ' both `presence_penalty` and `frequency_penalty`. The value has been set to '
                + f'that of the {"frequency_penalty" if frequency_penalty else "presence_penalty"}.',
                UserWarning,
            )
        if model_settings.get('logit_bias'):
            logit_bias = model_settings.pop('logit_bias')
            sequence_bias = {}
            for key, value in logit_bias.items():
                if isinstance(key, tuple):
                    sequence_bias[key] = value
                elif isinstance(key, int):
                    sequence_bias[(key,)] = value
                else:
                    warnings.warn(
                        'The transformers model expects the keys of the `logits_bias` argument '
                        + '(that it calls `sequence_bias`) to be a tuple of integers. '
                        + f'As the format you provided is not compatible ({type(key)}), the argument '
                        + 'has been removed to avoid causing an error.',
                        UserWarning,
                    )
                    break
            if sequence_bias:
                model_settings['sequence_bias'] = sequence_bias
        if model_settings.get('stop_sequences'):
            model_settings['stop_strings'] = model_settings.pop('stop_sequences')

        return model_settings

    def _format_llama_cpp_inference_kwargs(  # pragma: no cover
        self, model_settings: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        for arg in [
            ...
            if model_settings.get(arg):
                model_settings.pop(arg)
                warnings.warn(
                    'The mlxlm model does not support the `{arg}` argument. '
                    + 'The argument has been removed to avoid causing an error.',
                    UserWarning,
                )
        if model_settings.get('logit_bias'):
            if any(not isinstance(key, int) for key in model_settings['logit_bias'].keys()):
                model_settings.pop('logit_bias')
                warnings.warn(
                    'The llama_cpp model expects the keys of the `logits_bias` argument '
                    + '(that it calls `sequence_bias`) to be integers. '
                    + 'The argument has been removed to avoid causing an error.',
                    UserWarning,
                )
        if model_settings.get('stop_sequences'):
            model_settings['stop'] = model_settings.pop('stop_sequences')
    ...
    def _format_sglang_inference_kwargs(  # pragma: no cover
        self, model_settings: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _format_vllm_offline_inference_kwargs(  # pragma: no cover
        ...
        for key in model_settings.keys():
            if key in ['use_tqdm', 'lora_request', 'priority']:
                continue
            setattr(sampling_params, key, model_settings.pop(key))

        model_settings['sampling_params'] = sampling_params

        return model_settings

    def _format_dottxt_inference_kwargs(  # pragma: no cover
        self, model_settings: dict[str, Any]
    ) -> dict[str, Any]:
        """Map the model settings to the dottxt inference kwargs."""
        for arg in [
            'timeout',
            'parallel_tool_calls',
            'logit_bias',
            ...
            if model_settings.get(arg):
                model_settings.pop(arg)
                warnings.warn(
                    'The dottxt model does not support the `{arg}` argument. '
                    + 'The argument has been removed to avoid causing an error.',
                    UserWarning,
                )
        if model_settings.get('stop_sequences'):
