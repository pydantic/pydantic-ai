...
class Model(AbstractModel, Generic[InterfaceClient]):
    ...
    def _resolve_native_tool_swap(self, params: ModelRequestParameters) -> ModelRequestParameters:
        ...
        return resolve_native_tool_swap(
            params,
            self.profile.get('supported_native_tools', SUPPORTED_NATIVE_TOOLS),
            can_defer_tool_schemas=self._can_defer_tool_schemas,
        )
...
def resolve_native_tool_swap(
    ...
    *,
    can_defer_tool_schemas: Callable[[Sequence[AbstractNativeTool]], bool] | None = None,
    ...
    3. `defer_loading` → the tool is hidden until something reveals it, and `can_defer_tool_schemas`
       ...
       tool, and withheld entirely where it can't and nothing has. A caller without a
       `can_defer_tool_schemas` answer (the realtime session path) can't withhold schemas at all.
    ...
    * The corpus-empty drop is specific to the framework-managed tool-search native tool's
    ...
    path (via `Model._resolve_native_tool_swap`, which passes its profile-derived
    `_can_defer_tool_schemas`) and the realtime session path can share it — `RealtimeModel` is
    ...
    # Computed after the two steps above so it names the native tools this request really
    ...
    can_defer = can_defer_tool_schemas(supported_natives) if can_defer_tool_schemas is not None else False
