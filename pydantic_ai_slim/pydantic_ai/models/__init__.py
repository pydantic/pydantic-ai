...
class Model(ABC, Generic[InterfaceClient]):
    ...
    def prepare_request(
        ...
        if params.builtin_tools or any(t.prefer_builtin for t in params.function_tools):
            supported_types = self.profile.get('supported_builtin_tools', SUPPORTED_BUILTIN_TOOLS)
    ...
    def profile(self) -> ModelProfile:
        ...
        After resolution we compute the intersection of the profile's `supported_builtin_tools`
        and the model class's implemented tools, ensuring `model.profile['supported_builtin_tools']`
        ...
        # Step 4: builtin tools intersection
        model_supported = self.__class__.supported_builtin_tools()
        profile_supported = resolved.get('supported_builtin_tools', SUPPORTED_BUILTIN_TOOLS)
        effective_tools = profile_supported & model_supported
        if effective_tools != profile_supported:
            resolved = merge_profile(resolved, ModelProfile(supported_builtin_tools=effective_tools))
