...
UNSUPPORTED_GATEWAY_MODEL_NAMES = frozenset(
    {
        # Removed from Gateway: live smoke tests against a configured gateway route returned stable
        # unsupported-model errors (for example `model_not_found`, deprecated/EOL models, invalid Bedrock
        # model IDs, or models that only work with non-chat endpoints/modalities).
