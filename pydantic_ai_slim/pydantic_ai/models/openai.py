...
class OpenAIResponsesModelSettings(OpenAIChatModelSettings, total=False):
    ...
    """Whether to automatically download files generated by the code interpreter.

    When enabled, pydantic-ai will use the container files API to download
    all generated files and return them as FileParts.
...
class OpenAIResponsesModel(Model[AsyncOpenAI]):
    ...
    async def request(
        ...
        if model_settings and model_settings.get('openai_download_code_execution_files'):
            ...
            for part in process_response.parts:
                if isinstance(part, TextPart) and part.provider_details:
                    if annotations := part.provider_details.get('annotations'):
                        new_parts.extend(
                            await _download_container_files(self.client, annotations, provider_name=part.provider_name)
                        )
            ...
            if isinstance(item, responses.ResponseReasoningItem):
                ...
                for content in item.content:
                    if isinstance(content, responses.ResponseOutputRefusal):
                        ...
                        if (
                            model_settings.get('openai_include_raw_annotations')
                            or model_settings.get('openai_download_code_execution_files')
        ...
        with _map_api_errors(self._model_name):
            ...
            async for chunk in self._response:
                ...
                if isinstance(chunk, responses.ResponseCompletedEvent):
                    ...
                    if self._model_settings.get('openai_include_raw_annotations') or self._model_settings.get(
                    ...
                    if provider_details:
                        ...
                        if self._model_settings.get('openai_download_code_execution_files'):
                            if annotations := provider_details.get('annotations'):
                                for file_part in await _download_container_files(
                                    self._client, annotations, provider_name=self._provider_name
                                ):
                                    yield self._parts_manager.handle_part(
                                        vendor_part_id=f'{file_part.id}-file',
                                        part=file_part,
                                    )
...
async def _download_container_files(
    client: AsyncOpenAI, annotations: list[dict[str, Any]], *, provider_name: str | None
) -> list[FilePart]:
    """Download the container files referenced by output-text annotations as `FilePart`s."""
    file_parts: list[FilePart] = []
    for annotation in annotations:
        ...
        if not file_id or not container_id:
            continue
        file_parts.append(
            await _download_container_file(
                client, file_id=file_id, container_id=container_id, provider_name=provider_name
            )
        )
    return file_parts
...
async def _download_container_file(
    client: AsyncOpenAI, *, file_id: str, container_id: str, provider_name: str | None
) -> FilePart:
    """Download a file from an OpenAI container and return it as a FilePart."""
    file_metadata = await client.containers.files.retrieve(file_id=file_id, container_id=container_id)
    file = await client.containers.files.content.retrieve(file_id=file_id, container_id=container_id)
    media_type = mimetypes.guess_type(file_metadata.path)[0] or 'application/octet-stream'
