            ...
            if isinstance(message, ModelRequest):
                for part in message.parts:
                    if isinstance(part, SystemPromptPart):
                        ...
                        if self.profile.get('tool_availability_delta') != 'with_definitions':
                        ...
                        if call_id in client_replay_call_ids and isinstance(part, ToolSearchReturnPart):
                            ...
                            if (
                                ...
                                and profile.get('tool_availability_delta') == 'with_definitions'
                            ):
...
def _introduced_tool_names(
    ...
    if profile.get('tool_availability_delta') == 'with_definitions' and not _has_tool_search(model_request_parameters):
        names.update(
            match['name']
            for message in messages
            if isinstance(message, ModelRequest)
