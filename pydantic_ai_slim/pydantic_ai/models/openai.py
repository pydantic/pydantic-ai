...
class OpenAIChatModel(Model[AsyncOpenAI]):
    ...
    class _MapModelResponseContext:
        ...
        """Whether the conversation being mapped contains any `ThinkingPart`.

        Set by `_map_messages` (a single message can't see the whole conversation) and used by
        `_into_message_param` to repair synthesized tool-call turns that lack thinking.
        """
        ...
        def _into_message_param(self) -> chat.ChatCompletionAssistantMessageParam | None:
            ...
            elif self.tool_calls and self.thinking_active:
                # A tool-calling turn with no thinking, mid-thinking-run, is one the framework
                # synthesized (e.g. the deferred-capability tool-search exchange), not the model.
                # DeepSeek-style providers reject it unless it carries their thinking field, so send
                # it empty (the model produced no reasoning). Only `'field'` mode names the field;
                # `'auto'` infers it from a received part this turn never had.
                profile = OpenAIModelProfile.from_profile(self._model.profile)
                if profile.openai_chat_send_back_thinking_parts == 'field' and (
                    thinking_field := profile.openai_chat_thinking_field
                ):
                    message_param[thinking_field] = ''
            if self.texts:
    ...
    def _map_model_response(
        self, message: ModelResponse, *, thinking_active: bool = False
        ...
        `thinking_active` reports whether any `ThinkingPart` exists across the whole conversation,
        so a tool-calling turn that lacks thinking can be repaired for providers that require a
        thinking field (see `_MapModelResponseContext._into_message_param`).

        Subclasses of `OpenAIChatModel` may override this method to provide their own mapping logic.
        """
        return self._MapModelResponseContext(self, thinking_active=thinking_active).map_assistant_message(message)
    ...
    async def _map_messages(
        ...
        for message in messages:
            if isinstance(message, ModelRequest):
                ...
                if (mapped := self._map_model_response(message, thinking_active=thinking_active)) is not None:
