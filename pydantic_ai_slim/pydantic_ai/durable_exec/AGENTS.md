...
`CapabilityOperationId`, and `WorkspaceOperationId`. Capability methods marked with
`@durable_operation` and workspace methods both arrive through the same backend and config resolver
as framework operations, so do not maintain a second registration path for them.
