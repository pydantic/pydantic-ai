...
| `onStepFinish`, response `onFinish`, or v7 `onEnd` lifecycle callbacks | **Core/Application:** hooks or event stream handling; `VercelAIAdapter.on_complete` for completed-run persistence | Assert firing point, per-step versus terminal behavior, errors, persisted messages, and exactly-once writes. |
