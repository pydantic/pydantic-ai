...
Structured non-tool output (`NativeOutput`, `PromptedOutput`, or image output): the final result comes from the text/image the model returns, not an output tool. Some models (e.g. Claude on Bedrock) occasionally return such an output *and* a function tool call in one response. Under `'early'` a valid structured output ends the run and the function tools are skipped (output that fails validation falls through to the tools); under `'graceful'` / `'exhaustive'` the function tools run and (outside streaming) the run continues.

Plain text output (`output_type=str` / `TextOutput`, incl. a `str` fallback) is treated differently: the model isn't told its text is the final output, so text alongside a tool call is usually preamble, not an answer. Plain text does NOT end an `'early'` run when a function tool is co-emitted — the tool runs, as under `'graceful'`. Only structured (native/prompted) output or an image ends early. (Streaming still commits the first text as it streams, regardless of `end_strategy`.)
