...
`openai-codex:` uses ChatGPT subscription credentials created by `clai auth login openai-codex` or supplied through `OpenAICodexProvider`; do not recommend `OPENAI_API_KEY` for it. Use a model instance instead of a string when the user needs provider-specific constructor arguments.
