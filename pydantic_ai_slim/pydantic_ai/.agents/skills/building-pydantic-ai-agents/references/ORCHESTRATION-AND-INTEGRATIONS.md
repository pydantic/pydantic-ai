...
Tools wrapped as durable units by `TemporalDurability`, `DBOSDurability`, or `PrefectDurability` can call `ctx.enqueue()`: messages are recorded with the result and restored after the unit completes, with IDs and priorities preserved and duplicates suppressed within each reconstructed run. An attempt that fails without a recorded tool result does not deliver its messages. Other durable operations still reject enqueueing. This does not change `ctx.emit()`: emitted events are not replayed by Prefect or DBOS, and emitting from a Temporal activity remains unsupported.
