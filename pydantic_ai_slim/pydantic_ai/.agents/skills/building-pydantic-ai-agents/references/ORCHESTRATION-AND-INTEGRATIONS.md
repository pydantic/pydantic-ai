...
Capability hooks that must behave differently in deterministic workflow code check `ctx.in_durable_context`: `True` inside the durable workflow or flow, `False` in activities, steps, and tasks (where tools run) and outside durable execution. Don't detect the engine by module or class name.
