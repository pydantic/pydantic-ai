...
| Delegating to another agent from a tool or output function | `await agent.run()` in an `async def` callback; sync methods are rejected from Pydantic AI worker threads because they can deadlock |
