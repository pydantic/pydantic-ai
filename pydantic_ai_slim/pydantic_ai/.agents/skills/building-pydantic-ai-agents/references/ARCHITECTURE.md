...
**Additional prefixes:** `litellm:`, `nebius:`, `ovhcloud:`, `alibaba:`, `sambanova:`, `vercel:`, `moonshotai:`. For any other OpenAI-compatible endpoint, point `OpenAIChatModel` at it with `provider=OpenAIProvider(base_url=..., api_key=...)`. For anything that isn't OpenAI-compatible, subclass `Model`. The `pydantic_ai.models.babel` drop-ins (`BabelOpenAIChatModel`, `BabelAnthropicModel`, `BabelGoogleModel`, `BabelBedrockConverseModel`, extra `babel`) are constructed like their parents and map the wire through [babel](https://github.com/pydantic/babel) instead of the hand-written mappers.
