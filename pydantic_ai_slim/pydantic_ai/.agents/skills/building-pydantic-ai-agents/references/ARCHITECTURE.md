...
| Delegating to another agent from a tool or output function | `await agent.run()` in an `async def` callback — `run_sync()` and `run_stream_sync()` are rejected inside another agent run because they can deadlock |
