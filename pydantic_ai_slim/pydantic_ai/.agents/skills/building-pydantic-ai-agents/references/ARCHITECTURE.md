...
| Codex subscription | `openai-codex:` | `openai-codex:gpt-5.5` (requires managed or injected Codex credentials) |
...
**Model string format:** `"provider:model-name"` (e.g., `"openai:gpt-5.2"`, `"openai-codex:gpt-5.5"`, `"anthropic:claude-sonnet-4-6"`, `"google:gemini-3-pro-preview"`). The `openai-codex:` prefix uses ChatGPT subscription credentials managed by `clai auth login openai-codex` or an injected credential source; it does not use `OPENAI_API_KEY`.
