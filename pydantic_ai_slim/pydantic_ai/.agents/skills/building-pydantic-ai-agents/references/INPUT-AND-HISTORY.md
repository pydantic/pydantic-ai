...
- provider-returned web and document citations are available on `TextPart.citations`; `ContentCitationAnchor` and
  `MarkerCitationAnchor` use Python character indices into that part's content, while an absent anchor means only
  text-part-level attribution is available; each source's `excerpts` list contains provider-selected supporting passages
...
- persisted citations replay through native provider annotations for Anthropic Messages, Bedrock Converse, and OpenAI
  Responses when history is continued with the same provider; OpenAI Responses also requires
  `openai_send_reasoning_ids=True`; other providers and cross-provider histories replay the text without synthesizing
  citation objects; citations remain in the stored Pydantic AI messages for application rendering, but the model may
  not know what a visible marker refers to, so include the source again when a follow-up requires it rather than
  injecting a synthetic citation list into history
