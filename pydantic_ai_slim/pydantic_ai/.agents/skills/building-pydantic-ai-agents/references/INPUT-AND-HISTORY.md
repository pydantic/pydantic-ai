...
An enqueued `SystemPromptPart` is a mid-conversation instruction: it's sent at its position in the history rather than hoisted into the provider's top-level system prompt, so it doesn't invalidate a cached prefix ahead of it. This does not enable caching by itself; configure the model's prompt caching or include a `CachePoint`. On models that honor `CachePoint`, one at the end of an `enqueue(...)` batch covers every preceding item in that batch, including a `SystemPromptPart`. Where the provider's API accepts a system message inline it's sent as one, with real operator authority; if its native placement rules cannot represent an exact cache boundary, or the transport doesn't accept inline system messages, it's rendered as `<system>`-tagged user content at that position instead. A boundary between a tool call and its required result is invalid in either representation and raises a `UserError`. A model treats the tagged fallback as a strong preference rather than a system-level rule. Support varies by model *and* transport, and Pydantic AI picks the rendering automatically — don't gate your own code on a model list.
