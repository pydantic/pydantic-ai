...
An enqueued `SystemPromptPart` is a mid-conversation instruction: it's sent at its position in the history rather than hoisted into the provider's top-level system prompt, so it doesn't invalidate the cached prefix ahead of it. Models whose API honors a system message inline (Claude Opus 4.8, Claude Opus 5, Claude Fable 5, Claude Mythos 5) give it real operator authority; everywhere else it's rendered as `<system>`-tagged user content, which a model treats as a strong preference rather than a system-level rule. Claude Sonnet 5 belongs to the second group despite returning `200` for the inline entry — it accepts it and then doesn't act on it — so don't read "newer model" as "supported".
