...
In streamed runs, the model request hooks (`wrap_model_request`, `before_model_request`, `after_model_request`) run in a separate asyncio task. A `ContextVar` set there works within that request lifecycle but never reaches tools, output/run hooks, or caller code — a task's context-variable writes don't reach its parent. Share such state through a mutable attribute on `ctx.deps` (e.g. set `ctx.deps.request_message_count` in `before_model_request`, read it from a tool or after the run): object writes cross tasks. Alternatively set the var in `wrap_run`/`before_run`, which run in the outer task.
