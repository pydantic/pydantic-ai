...
- **Async agent delegation**: Any tool or output function that calls another agent must be `async def` and use `await delegate.run(...)`. Never generate `run_sync()` or `run_stream_sync()` inside a Pydantic AI callback. The parent may still use `run_sync()` at a synchronous application boundary, and ordinary non-delegating tools may remain synchronous.
