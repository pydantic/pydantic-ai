...
- **Workspace boundaries**: `Workspace` only carries an execution environment; applications choose which tools expose it. An agent has one workspace capability: a second `LocalWorkspace` with the default id replaces the first (its settings do not carry over), and any other second supplier raises `UserError`. `LocalWorkspace` / `LocalWorkspaceBackend` isolate nothing and are only for trusted workloads; use a sandbox provider for untrusted code.
