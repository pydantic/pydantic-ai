...
**Quick reference — model string format:** `"provider:model-name"` (e.g., `"openai:gpt-5.2"`, `"openai-codex:gpt-5.5"`, `"anthropic:claude-sonnet-4-6"`, `"google:gemini-3-pro-preview"`). `openai-codex:` requires `clai auth login openai-codex` or an injected Codex credential source; it does not use `OPENAI_API_KEY`.
