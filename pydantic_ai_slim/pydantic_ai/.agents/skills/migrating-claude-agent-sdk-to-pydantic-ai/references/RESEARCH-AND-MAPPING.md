...
| `ClaudeAgentOptions.system_prompt`, model, thinking, limits | **Core:** instructions, model/provider settings, `UsageLimits(request_limit=...)`; **Harness:** `SpendLimits` only for observed cross-run spend policy | Assert that request exhaustion raises `UsageLimitExceeded` and translate it if callers expect `error_max_turns`; prove spend reporting separately. |
