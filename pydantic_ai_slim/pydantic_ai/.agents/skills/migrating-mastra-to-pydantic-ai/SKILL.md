...
- Mastra workflow suspension persists a resumable snapshot. Select an application persistence design or supported durable runtime and prove restart at the promised step. Pydantic AI deferred tools preserve agent tool calls, not arbitrary workflow state.
...
2. Start core-only. Add `pydantic_graph`, Harness, or a durable runtime only after a source contract requires it.
