...
class GenerateToolJsonSchema(GenerateJsonSchema):
    def typed_dict_schema(self, schema: core_schema.TypedDictSchema) -> JsonSchemaValue:
        ...
        # PR #12179 partially addressed this in pydantic 2.12, but typed extras_schema
        # (e.g. **kwargs: int) still produces additionalProperties: true instead of the typed schema.
        extra = schema.get('extra_behavior') or schema.get('config', {}).get('extra_fields_behavior')
        if extra == 'allow':
            extras_schema = schema.get('extras_schema', None)
            if extras_schema is not None:
                # Always use typed additionalProperties when extras_schema exists
        ...
        elif extra == 'forbid' and 'additionalProperties' not in json_schema:  # pragma: no cover
