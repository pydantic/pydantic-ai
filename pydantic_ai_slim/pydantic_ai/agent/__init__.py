...
class Agent(AbstractAgent[AgentDepsT, OutputDataT]):
    ...
    def to_responses(
        ...
        *,
        ...
        runs its own tool loop server-side and is exposed as a single model: only the assistant's
        text is returned, not its internal tool calls or reasoning. Both streaming (`stream=True`)
        ...
        Note that `deps` and `model_settings` are the same for each request. To provide different
        `deps` per request, use [`handle_responses_request`][pydantic_ai._responses.handle_responses_request]
        ...
        Example:
            ...
            response = client.responses.create(model='gpt-5', input='Where does the sun rise?')
