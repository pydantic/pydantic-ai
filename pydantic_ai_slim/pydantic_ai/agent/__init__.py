...
def _validate_capability_ids(capabilities: Sequence[AbstractCapability[Any]]) -> set[str]:
    ...
    [`combine`][pydantic_ai.capabilities.AbstractCapability.combine] are allowed to repeat: the
    duplication is resolved for the whole composed tree at run setup by
    `combine_duplicate_capabilities`,
    which is the only place `combine` is called. Rejecting the rest here means the common mistake
    still surfaces in `Agent(...)` rather than on the first run.
