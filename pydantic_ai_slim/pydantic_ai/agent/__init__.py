...
class Agent(AbstractAgent[AgentDepsT, OutputDataT]):
    ...
    async def realtime_session(
        ...
        *,
        ...
        # Get tool definitions from the agent's toolset
        deps = self._get_deps(deps)
        ...
        async with toolset:
            ...
            async def tool_runner(name: str, args: dict[str, Any]) -> str:
                ...
                ctx = dataclasses.replace(run_context, tool_name=name)
                try:
        ...
        async with self._enter_lock:
            if self._entered_count == 0:
                async with AsyncExitStack() as exit_stack:
                    ...
                    await exit_stack.enter_async_context(toolset)  # pragma: no cover
                    self._exit_stack = exit_stack.pop_all()  # pragma: no cover
