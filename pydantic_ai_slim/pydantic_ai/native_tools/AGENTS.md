...
- Put portable options directly on the native tool. Put provider-only options in a tool-specific `provider_settings` `TypedDict` whose closed model-system keys each map to a system-specific `TypedDict`. Each adapter reads only its own system entry. Keep existing public fields and model-setting alternatives for backward compatibility.
