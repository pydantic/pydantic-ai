...
- Put portable options directly on the native tool. Put provider-only options in a provider-specific `TypedDict` that extends the tool's base settings type, and accept it through the tool's `settings` field, mirroring the `ModelSettings` pattern — including its `{provider}_` key prefix: every adapter reads the same runtime dict, so unprefixed keys from different providers would collide silently. Keep existing public fields and model-setting alternatives for backward compatibility.
