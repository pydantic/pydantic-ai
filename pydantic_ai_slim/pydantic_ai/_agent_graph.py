...
from .tools import RunContext, ToolCallResult, ToolCallResults, ToolDefinition, ToolKind
...
class GraphAgentDeps(Generic[DepsT, OutputDataT]):
    ...
    tool_call_results: ToolCallResults | None
...
class UserPromptNode(AgentNode[DepsT, NodeRunEndT]):
    ...
    async def run(  # noqa: C901
        ...
        tool_call_results = ctx.deps.tool_call_results
        if (
            tool_call_results is not None
            and messages
            ...
            for part in last_message.parts:
                if isinstance(part, _messages.ToolReturnPart):
                    ...
                    if part.tool_call_id not in tool_call_results:
        ...
        if messages and (last_message := messages[-1]):
            if isinstance(last_message, _messages.ModelRequest) and self.user_prompt is None:
                ...
                unprocessed_tool_calls = any(isinstance(part, _messages.ToolCallPart) for part in last_message.parts)
                ...
                if self.user_prompt is None or unprocessed_tool_calls:
...
class CallToolsNode(AgentNode[DepsT, NodeRunEndT]):
    ...
    async def _handle_tool_calls(
        ...
        ) and (deferred_tool_calls := ctx.deps.tool_manager.get_deferred_tool_calls(tool_calls_without_results)):
...
async def process_function_tools(  # noqa: C901
    ...
    if calls_to_run:
        ...
        with ctx.deps.tracer.start_as_current_span(
            ...
            tasks = [
                asyncio.create_task(
                    _call_function_tool(tool_manager, call, tool_call_results.get(call.tool_call_id)),
                    name=call.tool_name,
                )
                for call in calls_to_run
            ]
...
async def _call_function_tool(
    ...
    try:
        if tool_call_result is None or tool_call_result == 'call':
