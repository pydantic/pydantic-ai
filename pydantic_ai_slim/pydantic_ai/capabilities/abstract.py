...
def auto_capability_id() -> str:
    """Generate an implementation-owned capability id."""
    return f'{_AUTO_CAPABILITY_ID_PREFIX}{uuid4()}'


def is_auto_capability_id(id: str) -> bool:
    """Return whether `id` was generated by `auto_capability_id`."""
    return id.startswith(_AUTO_CAPABILITY_ID_PREFIX)
...
class AbstractCapability(ABC, Generic[AgentDepsT]):
    ...
    id: str = field(default_factory=auto_capability_id)
    ...
    def __post_init__(self) -> None:
        ...
        try:
            ...
            raise UserError(
                'Deferred capabilities must initialize `id` for message history replay. '
                'Use the generated dataclass `__init__`, or set `self.id` before calling '
                '`self.__post_init__()` from a custom `__init__`.'
            ) from e

        if is_auto_capability_id(capability_id):
            raise UserError(
