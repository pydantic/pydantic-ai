...
try:
    ...
    raise ImportError(
        'Please install the `starlette` package to use `responses_app`, '
        'you can use the `responses` optional group — `pip install "pydantic-ai-slim[responses]"`'
    ) from e


__all__ = ['responses_app', 'gateway']
...
def responses_app(
    ...
    *,
    ...
    """Build an ASGI application that handles Responses requests by running the agent and streaming the response.
    ...
    Returns:
        A Starlette ASGI application that exposes the agent over the OpenAI Responses API.
    """
    app = Starlette(
        debug=debug,
        routes=routes,
