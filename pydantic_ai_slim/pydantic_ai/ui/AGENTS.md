...
The inbound half of that last rule lives in `ag_ui/_forward_compat.py`: AG-UI's `Message`, `InputContent` and a media part's `source` are discriminated unions, so a `role`, `type` or `source.type` added after the installed version is a hard validation failure for the whole request unless it's skipped first. A tag the SDK retired, rather than one it has not learned yet, is translated to its replacement (`binary` on 1.0); a retired item with nothing to translate is malformed and still fails. Skipping is scoped to exactly that — an unknown tag only counts as new functionality when the item also satisfies the contract every member of its union shares (for `Message`, a string `id`), so anything else malformed must still fail.

Outbound, three inputs decide what the event stream emits. The `HAS_*` constants (try-imports in `_lifecycle_1_0.py`, `_interrupt.py`, `_binary_0_14.py`) say what the installed SDK can construct and are the ceiling. `ag_ui_version` defaults to the installed version and can only lower that ceiling, for a client older than the server. From 1.0 the client also declares its own version on `RunAgentInput.protocolVersion`. A field a pre-1.0 client passes through (`protocolVersion` on `RUN_STARTED`, `usage`) needs only the first two; a shape its strict outcome schema rejects (the `cancelled` outcome, `pendingToolCallIds`) also needs the client to declare 1.0, or it drops the whole stream at `RUN_FINISHED`. No declaration means a pre-versioning peer; an unreadable one counts as newer, with a warning, as the spec requires. `dump_messages` is the one place the installed SDK beats `ag_ui_version`: 1.0's `UserMessage.content` union no longer admits the retired `binary` part, so a 1.0 install writes typed media for every negotiated version.

Tests for version-gated behavior belong behind `requires_ag_ui('<version>')` in `tests/test_ag_ui.py`, never behind the module-level `imports_successful()` gate: a name that only exists above the floor in that import block skips the entire module on CI's `test-lowest-versions` job, which is the only job that exercises the floor these gates exist for. Expectations that differ by installed SDK go through a version-aware helper (`run_finished_outcome()`-style) so the same test runs at the floor; a test that constructs a shape a newer SDK removed is skipped on that SDK with a dedicated marker (`skip_if_ag_ui_1_0`).
