...
class AGUIAdapter(UIAdapter[RunAgentInput, Message, BaseEvent, AgentDepsT, OutputDataT]):
    ...
    def _dump_model_response(
        ...
        - ToolReturnPart / RetryPromptPart: Looked up from tool_results and emitted as ToolMessage
        ...
        for part in msg.parts:
            if isinstance(part, BuiltinToolReturnPart) or isinstance(part, ThinkingPart) or isinstance(part, FilePart):
                ...
                result.append(
                    ToolMessage(id=uuid.uuid4().hex, content=part.model_response_str(), tool_call_id=part.tool_call_id)
                )
            elif isinstance(part, RetryPromptPart):
                if part.tool_name:
                    result.append(
                        ToolMessage(id=uuid.uuid4().hex, content=part.model_response(), tool_call_id=part.tool_call_id)
                    )
                else:
    ...
    def _dump_model_request(
        ...
        for part in parts:
            if isinstance(part, SystemPromptPart):
                ...
                # Emitted as a separate ToolMessage
                result.append(
                    ToolMessage(id=uuid.uuid4().hex, content=part.model_response_str(), tool_call_id=part.tool_call_id)
                )
            elif isinstance(part, RetryPromptPart):
