...
class Provider(ABC, Generic[InterfaceClient]):
    ...
    def _owned_http_client(self) -> httpx.AsyncClient:
        """Create an HTTP client owned by this provider and tracked for the context-manager lifecycle.

        Providers that build their own SDK client call this when the user didn't supply an
        `http_client`, so the client is closed on context-manager exit and recreated on re-entry.
        """
