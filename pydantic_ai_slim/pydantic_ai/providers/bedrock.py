...
def bedrock_amazon_model_profile(model_name: str) -> ModelProfile | None:
    ...
    if 'nova-2' in model_name:
        profile = merge_profile(profile, ModelProfile(supported_builtin_tools=frozenset({CodeExecutionTool})))
...
def _strip_builtin_tools(profile: ModelProfile | None) -> ModelProfile:
    return merge_profile(profile, ModelProfile(supported_builtin_tools=frozenset()))
