...
try:
    ...
    raise ImportError(
        'Please install the `openai` package to use the SGLang provider, '
...
class SGLangProvider(_OpenAICompatibleProvider):
    """Provider for local or remote SGLang API."""
    ...
    def model_profile(model_name: str) -> ModelProfile | None:
        # This Hugging Face repo ID mapping is duplicated from `VLLMProvider.model_profile`: both servers
        # identify models by HF repo ID, but no shared home for the mapping exists yet. Extract a common
        # helper once both providers have landed.
        prefix_to_profile = {
        ...
        # Unlike `VLLMProvider`, no `supports_thinking` overlay is applied here: SGLang toggles reasoning
        # through `--reasoning-parser` and `chat_template_kwargs` rather than `reasoning_effort`, and the
        # mapping has not been verified live yet. Family profiles still contribute their own thinking flags.
        #
        ...
        #   grammar-constrained decoding (xgrammar). Like vLLM's guided decoding, that is pure token masking,
        #   so the model only sees the schema if it is also injected into the instructions.
        #   See https://github.com/pydantic/pydantic-ai/issues/3490.
        # - File content parts and native tool return schemas are not supported.
        # - Chat templates are applied server-side, so templates that reject more than one leading system
        #   message fail the same way as on vLLM. See https://github.com/pydantic/pydantic-ai/issues/5812.
    ...
    def __init__(
        ...
        *,
        ...
        """Create a new SGLang provider.

        Args:
            base_url: The base url for the SGLang requests. If not provided, the `SGLANG_BASE_URL` environment
                variable will be used if available.
            api_key: The API key to use for authentication, if not provided, the `SGLANG_API_KEY` environment
                variable will be used if available.
        ...
        if openai_client is not None:
            ...
            base_url = base_url or os.getenv('SGLANG_BASE_URL')
            if not base_url:
                raise UserError(
            ...
            api_key = api_key or os.getenv('SGLANG_API_KEY') or 'api-key-not-set'
