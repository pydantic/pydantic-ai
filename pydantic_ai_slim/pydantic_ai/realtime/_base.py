...
class RealtimeModelSettings(TypedDict, total=False):
    ...
    Absent uses [`ServerVAD`][pydantic_ai.realtime.openai.ServerVAD]; `None` enables manual
    turn-taking. [`SemanticVAD`][pydantic_ai.realtime.openai.SemanticVAD] is supported by OpenAI;
    xAI silently falls back to `ServerVAD` when it is supplied, following the model-settings
    convention of ignoring unsupported provider-specific detail.
    """
