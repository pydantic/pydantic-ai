...
__all__ = ('AzureOpenAIRealtimeModel',)


@dataclass
class AzureOpenAIRealtimeModel(OpenAIRealtimeModel):
    ...
    def __post_init__(self, provider: Provider[AsyncOpenAI] | str) -> None:
        if isinstance(provider, str):
            ...
            if not isinstance(resolved, AzureProvider):
                raise UserError("`AzureOpenAIRealtimeModel` requires an `AzureProvider` or `provider='azure'`.")
            provider = resolved
