    ...
    * all fields in properties must be marked as required
    ...
    def walk(self) -> JsonSchema:
        ...
        if self.root_ref is not None:
            ...
            # Use the transformed schema from $defs (which has been processed by _handle())
            # rather than the original untransformed schema from self.defs
            if '$defs' in result and root_key in result['$defs']:
                result.update(result['$defs'].get(root_key) or {})
