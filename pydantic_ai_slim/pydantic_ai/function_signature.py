...
def _schema_to_type_expr(
    ...
    if '$ref' in schema:
        ...
        if ref_name in defs and ref_name not in referenced_types:
            ...
            elif ('enum' in ref_schema or 'const' in ref_schema) and '$ref' not in ref_schema:
                # Collapse value-bearing defs (enums, consts) to a `Literal` inline — a bare name
                # would be undefined in the rendered signature, hiding the allowed values from
                # the model. The `$ref` check keeps this recursion at depth one even for
                # degenerate defs that pair `$ref` with `enum`/`const`.
