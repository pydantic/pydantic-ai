...
- Providers should call `profile.with_origin(provider, model_name)` in `model_profile()` only when they can supply a more canonical `genai-prices` provider/model lookup than `Model.profile` can infer from `system`, `base_url`, and the raw model name — use this for proxy or rewritten model names (e.g. OpenRouter `"anthropic/claude-sonnet-4-5"`, Bedrock `"anthropic.claude-3-5-sonnet-v2:0"`, or providers that normalize Hugging Face-style model refs for local profile selection) — Do not add `with_origin()` when the provider's native `system` or `base_url` already resolves correctly in `genai-prices`, since redundant origin hints add maintenance burden and can be less accurate than the native lookup.
<!-- rule:264 -->
