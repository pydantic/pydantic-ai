    ...
    if args.list_models:
        ...
        if last_config and last_config[0]:
            try:
                ...
                console.print('[dim]Use "clai -d" to reconnect to last endpoint[/dim]')
    ...
    if args.discover_models is not None:
        ...
        if not endpoint:
            ...
            if last_config and last_config[0]:
                ...
                console.print('[dim]Usage: clai -d <api_url> (e.g., clai -d http://localhost:1234/v1)[/dim]')
                return 1
        ...
        try:
            ...
            if last_config and last_config[0] == endpoint:
                try:
                    timestamp = datetime.fromisoformat(last_config[2])
                    time_str = timestamp.strftime('%Y-%m-%d %H:%M')
                    console.print(f'[dim]Last used: {last_config[1]} ({time_str})[/dim]')
                except (ValueError, TypeError):
            ...
            if discovered_models:
                ...
                default_index = None
                if last_config and last_config[0] == endpoint and last_config[1] in discovered_models:
                ...
                for i, model in enumerate(discovered_models, 1):
                    if default_index is not None and i - 1 == default_index:
                        console.print(f'  {i}. {model} [dim](last used)[/dim]', highlight=False)
                    else:
                ...
                try:
                    ...
                    if not choice:
                        if default_index is not None:
                            ...
                            console.print(f'[dim]Using default: {discovered_models[model_index]}[/dim]')
                        else:
                    ...
                    if 0 <= model_index < len(discovered_models):
                        ...
                        console.print(f'[green]Starting chat with {selected_model} at {endpoint}[/green]')
                    else:
                ...
                console.print('[yellow]No models found at the specified endpoint.[/yellow]')
                return 1
