"""CodeMode Demo: Batch Calendar Event Creation with Modal Runtime.

This demo is identical to ``batch_operations_demo`` but uses ModalRuntime instead
of MontyRuntime. Code mode reduces LLM roundtrips when creating multiple calendar
events — the LLM writes a loop that creates all events in one go, and the code
executes in a gVisor-isolated cloud sandbox via Modal.

Requires:
    - ``pip install "pydantic-ai-slim[modal]"``
    - ``MODAL_TOKEN_ID`` and ``MODAL_TOKEN_SECRET`` environment variables
      (get them from https://modal.com/settings)

Run:
    uv run -m pydantic_ai_examples.code_mode.modal_runtime_demo
"""
...
def create_code_mode_agent(toolset: FunctionToolset[None]) -> Agent[None, str]:
    """Create agent with CodeMode (tools as Python functions via Modal)."""
    runtime = ModalRuntime(app_name='pydantic-ai-code-mode-demo')
...
async def run_code_mode(toolset: FunctionToolset[None]) -> RunMetrics:
    """Run with CodeMode tool calling via Modal."""
    global _calendar_events
    _calendar_events = []  # Reset calendar

    with logfire.span('code_mode_modal'):
        agent = create_code_mode_agent(toolset)
        code_toolset = agent.toolsets[0]
        async with code_toolset:
            result = await agent.run(PROMPT)
    return extract_metrics(result, 'code_mode_modal')
...
async def main() -> None:
    logfire.configure(service_name='code-mode-modal-demo')
    ...
    with logfire.span('demo_code_mode_modal'):
