...
COPY_PROMPT_PREFIX = (
    'Write one short social-media sentence based on this image description: '
)
...
def _api_key() -> str | None:
    """Same precedence as [`AlibabaProvider`][pydantic_ai.providers.alibaba.AlibabaProvider]."""
    return os.getenv('ALIBABA_API_KEY') or os.getenv('DASHSCOPE_API_KEY')


def _print_step(title: str, result: AgentRunResult[OutputT]) -> None:
    print(f'\n--- {title} ---')
    print(result.output)
...
def _agent(
    model: str, *, output_type: type[ImageCaption] | None = None
) -> Agent[Any, Any]:
    ...
    if base_url:
        api_key = _api_key()
        assert api_key is not None  # validated in main() before any agent runs
...
def main() -> None:
    if not _api_key():
        raise SystemExit(
            'Set ALIBABA_API_KEY or DASHSCOPE_API_KEY to run this example.'
        )
    ...
    image_bytes = httpx.get(
        SAMPLE_IMAGE_URL, follow_redirects=True, timeout=10.0
    ).content
