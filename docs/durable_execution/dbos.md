...
workflow holds the session until the run ends. Tools are still listed through the `get_tools` step,
never from workflow-side state, so which steps run doesn't depend on process warmth; the server's own
[`cache_tools`][pydantic_ai.mcp.MCPToolset.cache_tools] then answers those steps from the session the
run is already holding. Concurrent runs in one process share a server's session, as they do outside a
