...
For handlers with I/O side effects, pass `event_stream_handler=` to [`DBOSDurability`][pydantic_ai.durable_exec.dbos.DBOSDurability]. Model events are delivered live inside each model-request step. Ready tool events are collected into bursts and each burst crosses the durable boundary in one event-handler step. The handler is still invoked once for each event, in order. Events yielded while a step is running accumulate for the next step, so tool execution can proceed while call events are being handled. As with any DBOS step, a handler may run more than once if the workflow recovers before its step is checkpointed, so keep its side effects idempotent.
