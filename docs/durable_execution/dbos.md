...
Toolsets that implement their own tool listing and calling but aren't one of the types above — a [custom toolset](../toolsets.md#building-a-custom-toolset) subclassing [`AbstractToolset`][pydantic_ai.toolsets.AbstractToolset] — cannot be turned into steps, so attaching `DBOSDurability` to an agent that uses one raises a `UserError`. Return it from a `DynamicToolset` or a [`DynamicCapability`][pydantic_ai.capabilities.DynamicCapability] (whose tools are listed and called inside steps), or expose its tools on a `FunctionToolset` (and decorate them with `@DBOS.step` as described above). See [Custom toolsets and durable execution](../toolsets.md#custom-toolsets-and-durable-execution).
