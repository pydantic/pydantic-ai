...
A [workspace](../workspace.md) supplied by a capability works inside a workflow without further setup. `ctx.workspace` in a tool is the plain workspace, rebuilt inside the activity from the run's serialized [`WorkspaceRef`][pydantic_ai.workspaces.WorkspaceRef] through the agent's capabilities, policy wrappers such as a read-only `LocalWorkspace` included; its calls reach the provider directly. In workflow code (capability hooks, output functions, `result.workspace`), each operation is its own activity, named `agent__{name}__workspace__{method}`, and one `agent__{name}__workspace__ensure` activity at the start of the run creates or attaches the environment and records its ref and working directory, so every activity of the run — parallel tool calls included — reattaches to the same environment, and replay never touches the provider. `working_dir()` and `resolve()` answer from that recorded value from then on.
...
- `workspace_activity_config`: The Temporal activity config to use for [workspace](#workspaces) activities. This is merged with the base activity config. Unless it sets a `retry_policy`, the activities for `run`, `write_bytes`, `write_text`, `make_dir` and `remove` are attempted once, so a command or write is never repeated by a retry; the read activities keep the base policy. A `run(timeout=...)` longer than the activity's `start_to_close_timeout` widens that call's deadline to fit the command.
