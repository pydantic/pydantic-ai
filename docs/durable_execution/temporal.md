...
Specifically, only the `deps`, `run_id`, `conversation_id`, `metadata`, `retries`, `tool_call_id`, `tool_name`, `retry`, `max_retries`, `run_step`, `partial_output`, `usage`, `usage_limits`, `trace_include_content`, `instrumentation_version`, `loaded_capability_ids`, `discovered_tool_names`, and `capability_loaded` fields are serialized on the generic run-context blob. `tool_call_approved` and `tool_call_metadata` are **not**: a value that decides whether a tool executes is not honored as ambient serialized run-context state. On a tool-call activity they are applied from the activity's `CallToolParams` (computed workflow-side from [`DeferredToolResults`][pydantic_ai.tools.DeferredToolResults]); on every other activity they default to unapproved. Flipping `tool_call_approved` on the run-context blob cannot elevate an unapproved call. `agent` and `root_capability` are re-attached from the worker's agent instance, `tool_manager` is `None` (so [`available_tool_names`][pydantic_ai.tools.RunContext.available_tool_names] returns the snapshot resolved when the activity was dispatched, and falls back to `discovered_tool_names` only for a subclass whose `serialize_run_context` doesn't carry it; [`available_capability_ids`][pydantic_ai.tools.RunContext.available_capability_ids] is carried the same way, which is what lets [`is_tool_available`][pydantic_ai.tools.RunContext.is_tool_available] answer for a tool owned by a capability), and `pending_messages` holds a guard that makes [`ctx.enqueue()`][pydantic_ai.tools.RunContext.enqueue] raise inside an activity, since the activity's recorded result is replayed without re-running your code and the enqueued messages would be dropped.
...
!!! note "HITL approval and exactly-once side effects"
    Temporal activities are at-least-once. [`ApprovalRequired`][pydantic_ai.exceptions.ApprovalRequired] stops an unapproved call, but it does not make an approved call execute once: if the tool's HTTP request succeeds and the activity then times out, Temporal retries the same activity and the tool runs again. [`RunContext.idempotency_key`][pydantic_ai.tools.RunContext.idempotency_key] is a replay-stable key bound to `run_id` and `tool_call_id` — pass it to downstream APIs that support idempotency (payment processors, provisioners). It is not an approval token and does not authorize the call.
