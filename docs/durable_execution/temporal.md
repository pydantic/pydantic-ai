...
!!! warning "Deprecated"
    ...
    [`TemporalDurability`][pydantic_ai.durable_exec.temporal.TemporalDurability] accepts the activity names and payload shapes recorded by `TemporalAgent` (as long as the agent's `name` and toolset `id`s stay the same), so workflows started under `TemporalAgent` will replay correctly after switching — there's no need to drain or version your workflows first.
