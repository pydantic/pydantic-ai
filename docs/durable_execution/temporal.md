...
Emitting [custom events](../agent.md#custom-events) via [`ctx.emit_event()`][pydantic_ai.tools.RunContext.emit_event] from a tool is not currently supported, as tools run inside activities that cannot reach the run's event stream; doing so raises a `UserError`. Emit custom events from [capability](../capabilities/overview.md) hooks, which run in the workflow, instead.
