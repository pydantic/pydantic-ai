...
A toolset contributed by a [capability](../capabilities/overview.md) — like the local fallback of [`WebSearch`][pydantic_ai.capabilities.WebSearch] — takes its `id` from the capability, so set it there: `WebSearch(local='duckduckgo', id='search')`.
