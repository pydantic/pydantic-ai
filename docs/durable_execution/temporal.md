...
[Capabilities](../capabilities/overview.md) that contribute a toolset — a [`Capability`][pydantic_ai.capabilities.Capability] with `tools=`, a [`NativeOrLocalTool`][pydantic_ai.capabilities.NativeOrLocalTool] like [`WebSearch`][pydantic_ai.capabilities.WebSearch] falling back to a local implementation, or an [`MCP`][pydantic_ai.capabilities.MCP] server running locally — derive the toolset's `id` from the capability's own [`id`][pydantic_ai.capabilities.AbstractCapability.id], so set `Capability(id='...', tools=[...])` or `MCP(id='...', url='...')`. The single-purpose built-ins ([`WebSearch`][pydantic_ai.capabilities.WebSearch], [`WebFetch`][pydantic_ai.capabilities.WebFetch], [`ImageGeneration`][pydantic_ai.capabilities.ImageGeneration], and [`XSearch`][pydantic_ai.capabilities.XSearch]) default their `id` to their own name, so they work here as-is. (`MCP` falls back to an id derived from the server URL's host and path when no `id` is given.) A toolset you pass in yourself — via `toolsets=` on a capability, or as the `local=` fallback of a `NativeOrLocalTool` — keeps its own `id`, which must be set on the toolset itself.
