...
!!! warning "Persisted payload schemas"
    Temporal may deserialize persisted workflow and activity payloads using the models and type annotations available in the currently deployed worker. Treat models used in persisted Temporal payloads as durable contracts: adding an optional field with a default may continue to validate, while adding a required field or making another incompatible change may cause payload decoding to fail before the workflow or activity body executes. This is especially relevant to application-owned workflow inputs and dependency models. Pydantic AI does not own or migrate Temporal workflow history, so applications with long-running workflows should use an appropriate versioning or migration strategy when changing these models.
