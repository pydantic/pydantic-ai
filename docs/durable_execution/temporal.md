...
8. The [`PydanticAIPlugin`][pydantic_ai.durable_exec.temporal.PydanticAIPlugin] tells Temporal to use Pydantic for serialization and deserialization, and automatically registers activities for agents listed in `__pydantic_ai_agents__`. Activity retry policies treat [`UserError`][pydantic_ai.exceptions.UserError], `PydanticUserError`, [`UnexpectedModelBehavior`][pydantic_ai.exceptions.UnexpectedModelBehavior], and [`FallbackExceptionGroup`][pydantic_ai.exceptions.FallbackExceptionGroup] as non-retryable, along with Temporal's own `PayloadSizeError` failure type (see [Payload Size and Binary Content](#payload-size-and-binary-content)), while the worker registers `UserError`, `PydanticUserError`, [`AgentRunError`][pydantic_ai.exceptions.AgentRunError], and `UnsupportedEventLoopError` as `workflow_failure_exception_types`.
...
This limit is easiest to hit with a tool that returns media, as the tool's return value becomes the activity's result payload. A tool returning a [`BinaryImage`][pydantic_ai.messages.BinaryImage] — whether the subagent fallback behind the [`ImageGeneration`][pydantic_ai.capabilities.ImageGeneration] capability or your own `local=` generator — sends those bytes back across the boundary, and Pydantic AI raises a `UserError` naming the tool if Temporal rejects the payload.
...
The same cap applies to an image a model returns on its response, as with a [native image generation tool](../native-tools.md#image-generation-tool), since the model request is itself an activity. Pydantic AI raises a `UserError` naming the model there too, but returning a reference isn't something you can do from your own code: the bytes come back from the provider. Ask the model for a smaller image through its [model settings](../agent.md#model-run-settings), or offload the payload as below. A [streamed](#streaming) segment can also overflow on its buffered events alone, without any image involved.

To offload large payloads without changing what your tools or models return, configure Temporal's [external storage](https://docs.temporal.io/develop/python/data-handling/external-storage) or a hand-written [claim check codec](https://docs.temporal.io/ai-cookbook/claim-check-pattern-python) on your `DataConverter`: both replace an oversized payload with a reference *before* Temporal checks its size, and [`PydanticAIPlugin`][pydantic_ai.durable_exec.temporal.PydanticAIPlugin] preserves them because it replaces only the payload converter.
...
- **Payload size**: whenever [streaming](#streaming) is used — an `event_stream_handler`, a `ProcessEventStream` capability, or a per-run `event_stream_handler` — each segment's buffered events are shipped back to the workflow and must fit within Temporal's [payload size limit](#payload-size-and-binary-content) (2MB by default). A segment that overflows raises the same `UserError` as any other oversized model response.
