...
For handlers with I/O side effects, pass `event_stream_handler=` to [`TemporalDurability`][pydantic_ai.durable_exec.temporal.TemporalDurability]. Model events are delivered live inside each model-request activity. Ready tool events are collected into bursts and each burst crosses the durable boundary in one event-handler activity. The handler is still invoked once for each event, in order. Events yielded while an activity is running accumulate for the next activity, so tool execution can proceed while call events are being handled. As with any Temporal activity, a handler may run more than once if an activity retries, so keep its side effects idempotent.

Temporal records the batching command change with the `pydantic-ai-batched-event-dispatch` patch marker. New workflows use batched activities. When an older in-flight history is replayed, the missing marker selects the original per-event activity sequence so replay remains deterministic.
