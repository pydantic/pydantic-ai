...
A [custom toolset](../toolsets.md#building-a-custom-toolset) subclassing [`AbstractToolset`][pydantic_ai.toolsets.AbstractToolset] isn't one of the types above, so its own `get_tools()` and `call_tool()` run in workflow code rather than in an activity. That's fine if they perform no I/O and are deterministic. If they do perform I/O, implement the toolset as a `FunctionToolset` subclass instead, or return it from a dynamic toolset or a [`DynamicCapability`][pydantic_ai.capabilities.DynamicCapability], whose tool listing and tool calls both run inside activities. See [Custom toolsets and durable execution](../toolsets.md#custom-toolsets-and-durable-execution).
