...
Specifically, only the `deps`, `run_id`, `conversation_id`, `metadata`, `retries`, `tool_call_id`, `tool_name`, `tool_call_approved`, `tool_call_metadata`, `retry`, `max_retries`, `run_step`, `partial_output`, `usage`, `usage_limits`, `trace_include_content`, `instrumentation_version`, `loaded_capability_ids`, `discovered_tool_names`, and `capability_active` fields are available by default. `agent` and `root_capability` are re-attached from the worker's agent instance, and `tool_manager` is `None` (so [`available_tool_names`][pydantic_ai.tools.RunContext.available_tool_names] returns the snapshot resolved when the activity was dispatched, and falls back to `discovered_tool_names` only for a subclass whose `serialize_run_context` doesn't carry it; [`active_capability_ids`][pydantic_ai.tools.RunContext.active_capability_ids] is carried the same way, which is what lets [`is_tool_available`][pydantic_ai.tools.RunContext.is_tool_available] answer for a tool owned by a capability).

Tools wrapped by `TemporalDurability`, including MCP `process_tool_call` hooks, can call [`ctx.enqueue()`][pydantic_ai.tools.RunContext.enqueue]. Their messages are recorded with the activity result and restored in workflow code after the activity completes or its result is replayed. Other activity types keep a `pending_messages` guard that rejects enqueueing because their results do not carry messages back to the workflow. See [enqueue messages from tools](overview.md#enqueue-messages-from-tools) for retry and deduplication behavior.
