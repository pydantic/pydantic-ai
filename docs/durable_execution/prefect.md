...
For handlers with I/O side effects, pass `event_stream_handler=` to [`PrefectDurability`][pydantic_ai.durable_exec.prefect.PrefectDurability]. Model events are delivered live inside each model-request task. Ready tool events are collected into bursts and each burst crosses the durable boundary in one event-handler task. The handler is still invoked once for each event, in order. Events yielded while a task is running accumulate for the next task, so tool execution can proceed while call events are being handled. Configure these tasks with `event_stream_handler_task_config=`. As with any Prefect task, a handler may run more than once if a task retries, so keep its side effects idempotent.

The replay-stable event sequence counts batches. A flow run recorded before batching and recovered with batching enabled has a cache miss at the recovery boundary and re-delivers those events once. Event handlers should already tolerate duplicate delivery because Prefect can retry tasks.
