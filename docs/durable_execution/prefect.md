...
Under Prefect, a `validation_context` builder must derive its value from the run's `deps` or be constant, because the task cache key covers `deps` but not the built context, so two calls differing only in validation context share a cache entry.
