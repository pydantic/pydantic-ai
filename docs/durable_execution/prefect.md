...
A [custom toolset](../toolsets.md#building-a-custom-toolset) subclassing [`AbstractToolset`][pydantic_ai.toolsets.AbstractToolset] isn't one of those types, so its own `get_tools()` and `call_tool()` run in flow code rather than in a task. That's fine if they perform no I/O and are deterministic. If they do perform I/O, implement the toolset as a `FunctionToolset` subclass instead, or return it from a dynamic toolset or a [`DynamicCapability`][pydantic_ai.capabilities.DynamicCapability], whose tool *calls* do run inside tasks. Note that Prefect resolves a dynamic toolset's tool *listing* in flow code rather than in a task (unlike Temporal and DBOS, which journal it), so a flow retry re-executes the returned toolset's `get_tools()` — keep non-deterministic I/O out of it and confine it to the tools themselves. This divergence is tracked in [#6910](https://github.com/pydantic/pydantic-ai/issues/6910). See [Custom toolsets and durable execution](../toolsets.md#custom-toolsets-and-durable-execution).
