...
A tool with an [`args_validator`](../tools-advanced.md#args-validator) gets a `Validate Tool Args: {name}` task of its own, so the validator's I/O is checkpointed like the tool call's — and a `DynamicToolset`'s validators are no longer lost when its tools cross into flow code. A tool without one gets no extra task, and a tool that opted out of task wrapping with `metadata={'prefect': False}` is validated in flow code alongside its call. Validation runs before the [approval and deferral](../deferred-tools.md) gate, which is why it gets a task of its own rather than sharing the tool-call one: a tool with `requires_approval=True` whose arguments its validator rejects is turned into a retry prompt without ever asking a human to approve them. A validator that [defers the call itself](../tools-advanced.md#args-validator) works from inside the task too — the call joins the run's `DeferredToolRequests` output without a tool-call task, and resuming with an approval runs the validation task again with `tool_call_approved` set.

Under Prefect, a built `validation_context` forks the task cache key when Prefect can hash it. Non-serializable parts such as live HTTP clients, database connections, and locks collapse to a stable sentinel, so they must not be relied on to distinguish cached calls.
