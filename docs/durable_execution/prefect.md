...
A toolset of any other type that implements its own tool listing and calling — a [custom toolset](../toolsets.md#building-a-custom-toolset) subclassing [`AbstractToolset`][pydantic_ai.toolsets.AbstractToolset] — cannot be turned into tasks, so attaching `PrefectDurability` to an agent that uses one raises a `UserError`. Return it from a `DynamicToolset` or a [`DynamicCapability`][pydantic_ai.capabilities.DynamicCapability] (whose tools are listed and called inside tasks), or expose its tools on a `FunctionToolset`. See [Custom toolsets and durable execution](../toolsets.md#custom-toolsets-and-durable-execution).
