...
Code execution is a [toolset](toolsets.md) that replaces individual [tool](tools.md) calls with a single `run_code` tool, allowing the model to write and execute Python code that orchestrates multiple tools at once — with loops, conditionals, variables, and parallel execution.

With standard tool calling, each tool invocation is a separate round-trip to the model. If an agent needs to fetch 10 items and look up details for each one, that's 11 model calls. With code execution, the model writes a script that does it all in one step:
...
[`CodeExecutionToolset`][pydantic_ai.toolsets.code_execution.CodeExecutionToolset] optionally wraps an existing toolset and exposes its tools as callable Python functions to the model:
...
Code execution works with any [toolset](toolsets.md), including [MCP servers](mcp/client.md). Tool names that aren't valid Python identifiers (e.g. `search-records`) are automatically sanitized (to `search_records`) so the model can call them naturally from code.
...
- **Tool approval and deferral** — [deferred tools](deferred-tools.md) (tools that require approval or external execution) are not yet supported in code execution.
...
- [Function Tools](tools.md) — defining the tools that code execution wraps
- [MCP Client](mcp/client.md) — using MCP servers as toolsets with code execution
- [Dependencies](dependencies.md) — dependency injection, which works through code execution's tool pipeline
