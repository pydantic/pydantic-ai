...
| xAI | `reasoning_effort='low'` on Grok 4.3 | `reasoning_effort='high'` | Grok 4.3 supports `'none'`, `'low'`, `'medium'`, and `'high'`; Grok 3 Mini only supports `'low'` and `'high'` and silently ignores `thinking=False` |
