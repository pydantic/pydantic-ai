...
To fully comply with the GenAI semantic conventions for multimodal inputs (as specified in [the multimodal examples](https://opentelemetry.io/docs/specs/semconv/gen-ai/non-normative/examples-llm-calls/#multimodal-inputs-example)), use [`InstrumentationSettings(version=4)`][pydantic_ai.models.instrumented.InstrumentationSettings]. This formats image, audio, video, and document URLs with `type='uri'`, `modality`, `uri`, and `mime_type` fields instead of the previous `type='image-url'` (etc.) and `url` fields. Inline binary content is formatted with `type='blob'`, `modality`, `mime_type`, and `content` fields instead of the previous `type='binary'` and `media_type` fields.
