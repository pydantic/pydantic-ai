...
When `include_content=False` is set, Pydantic AI will exclude sensitive content from telemetry, including user prompts and model completions, tool call arguments and responses, and any other message content. Exceptions recorded on agent run, model request, fallback attempt and tool spans keep only their type, since their message and stack trace can quote that content.
