# Audio, transcripts, images, and events
...
Each audio iterator buffers up to 32 chunks and each transcript iterator up to 512 items. A slow
consumer drops its oldest item instead of stalling tools, turn tracking, or other consumers.
...
configured with `google_input_transcription`, so it accepts only `'auto'` or `None` for the shared
setting. Provider-specific defaults and deployment constraints live on the provider pages.
...
- No useful speech usually means the PCM rate or channel count is wrong. Use mono PCM16 and the
  sample rates from `session.profile`.
