...
Deferred capability loading is unsupported. A capability with `defer_loading=True` raises
[`UserError`][pydantic_ai.exceptions.UserError] before connecting because there is no in-session
loading boundary. History-processing capabilities also do not transform seeded history.
