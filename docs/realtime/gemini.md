...
tools, since a fast result can cut off speech that has barely started. What the model says while the
tool runs, until the result is in, is recorded in the calling
[`ModelResponse`][pydantic_ai.messages.ModelResponse], after the call; see
[Tool calls in history](history.md#tool-calls-in-history). Models that don't support it ignore the
setting, and `gemini-3.8-live-extended-thinking`, which has no blocking mode, runs every tool call
this way regardless — see [Extended thinking](#extended-thinking).
