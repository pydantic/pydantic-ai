...
Voice reports the input transcript as cumulative snapshots that can correct earlier text, so a revised
partial arrives with
[`SpeechPartDelta.replaces_transcript`][pydantic_ai.messages.SpeechPartDelta.replaces_transcript] set
rather than as an appended delta; see [Transcribing user input](index.md#transcribing-user-input).
