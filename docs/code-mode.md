...
The prompt that describes available functions and the execution model to the model is generated by [`build_code_mode_prompt`][pydantic_ai.toolsets.code_mode.build_code_mode_prompt]. You can replace it by passing a custom `prompt_builder` to [`CodeModeToolset`][pydantic_ai.toolsets.code_mode.CodeModeToolset]:
