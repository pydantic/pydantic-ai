...
| Model | Description |
|-------|-------------|
| `MiniMax-M3` | Latest flagship model with 512K context, up to 128K output, and image input support (default) |
...
agent = Agent('minimax:MiniMax-M3')
...
```
...
model = OpenAIChatModel('MiniMax-M3', provider='minimax')
agent = Agent(model)
...
```
...
model = OpenAIChatModel(
    'MiniMax-M3', provider=MiniMaxProvider(api_key='your-api-key')
)
agent = Agent(model)
...
```
