...
!!! note "v2 default change"
    In Pydantic AI v2, the bare `'openai:'` prefix will resolve to `OpenAIResponsesModel` instead of `OpenAIChatModel`. The 1.x line emits a `DeprecationWarning` whenever bare `'openai:'` is used; pick an explicit prefix to silence the warning and pin behavior:

    - `'openai-chat:gpt-5.2'` keeps the current Chat Completions routing.
