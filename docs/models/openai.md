...
On the Responses API the instructions are sent as leading input messages, because the top-level `instructions` field cannot carry a breakpoint. With [`openai_previous_response_id`][pydantic_ai.models.openai.OpenAIResponsesModelSettings.openai_previous_response_id]`='auto'`, a chained response reuses that relocated prefix only when the current [instructions](../agent.md#instructions) equal the ones stored on the chained request — so a later agent, or the same agent with new instruction text, does not keep running under the previous policy. When they differ, the chain is dropped and the current instructions are relocated onto a full-history request. An explicit `openai_previous_response_id` whose response is not in `message_history` cannot be checked that way and raises. When [`openai_conversation_id`][pydantic_ai.models.openai.OpenAIResponsesModelSettings.openai_conversation_id] is set, or the history has been [compacted](../capabilities/compaction.md), the instructions stay in the top-level field and no breakpoint is added.
