...
All OpenAI SDK-based providers — including [`OpenAIProvider`][pydantic_ai.providers.openai.OpenAIProvider], [`AzureProvider`][pydantic_ai.providers.azure.AzureProvider], [`DeepSeekProvider`][pydantic_ai.providers.deepseek.DeepSeekProvider], and the [OpenAI-compatible providers](#openai-compatible-models) — accept a `max_retries` parameter that controls how many times the underlying SDK retries failed requests. It defaults to `2`, matching the OpenAI SDK. Set it to `0` to disable SDK-level retries, which is useful when you rely on custom retry logic or a durable execution framework like [Temporal](../durable_execution/temporal.md):
...
model = OpenAIChatModel(
    'gpt-5.2',
    provider=OpenAIProvider(api_key='your-api-key', max_retries=0),
)
agent = Agent(model)
...
```
...
If `max_retries` is the only setting you need to customise, pass it directly to the provider instead of constructing a custom client — see [Configuring retries](#configuring-retries) above.
