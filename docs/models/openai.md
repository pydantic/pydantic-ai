...
Use [`MoonshotAIModel`][pydantic_ai.models.moonshotai.MoonshotAIModel] for Moonshot AI,
including native dynamic tool loading on Kimi K3. See [Moonshot AI](moonshotai.md) for setup.
`OpenAIChatModel` with `MoonshotAIProvider` remains supported for ordinary Chat Completions.
