...
Agent('typesafe:jev-1.13.0', output_type=Handling)
```
...
| `float` with `ge=0` and `le=1` | yes or no | Jev's probability, unrounded |
| whole numbers `0, 1, 2, …`, each level described in the schema | score against a rubric | the nearest level |
...
Anything else is a [`UserError`][pydantic_ai.exceptions.UserError] before a request is sent, and the message names the field and lists what is supported.
...
A rubric is a set of ordered levels rather than a set of alternatives: the whole numbers from 0 upwards, in order, at least two of them, and every level needs a description in the schema saying what it means. Jev answers with a position along the rubric, which lands between levels, and the field gets the nearest one — a half rounds up. The unrounded position is in `provider_details['scores']`.
...
The threshold applies to every `bool` field and to each option of a `list` of a `Literal` or `Enum`, which is one yes/no per option. It does not apply to a `float` bounded with `ge=0` and `le=1`, which asks for the probability itself and hands it back unrounded: a field whose threshold you would want to vary per call is often better declared that way, and compared in your own code.

Confidence moves with the threshold, because it is the margin over the bar that was actually used: a yes at 0.8 under a threshold of 0.75 reports 0.2, not the 0.6 it would report against a coin flip. A [fallback on low confidence](#falling-back-on-low-confidence) therefore keeps meaning what it meant.
...
irreversible action. For a guard the two mistakes rarely cost the same — a missed irreversible command costs more
than a second look at a safe one — so set [what `True` has to mean](#what-true-has-to-mean) accordingly.
...
| a function tool, below the threshold | Jev fills the fields, or with only output functions takes the likeliest of them; the lean is in `provider_details['tool']` | none |
