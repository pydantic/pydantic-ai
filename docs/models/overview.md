...
The resolved model exposes its [`context_window`][pydantic_ai.models.Model.context_window]: the maximum number of tokens it can handle in a single request, or `None` when unknown. The value comes from the model profile's [`context_window`][pydantic_ai.profiles.ModelProfile.context_window]. Inside a run, [`ctx.context_window_used`][pydantic_ai.tools.RunContext.context_window_used] reports the fraction in use. It returns `None` when that fraction cannot be calculated. For example, it returns `None` for a fallback model because its candidates may have different window sizes. See [Compact when the context window fills](../message-history.md#compact-when-the-context-window-fills) for an example that handles this case.
