...
For an instruction the model has no reason to question — a change of format or tone — both renderings work, and the `system` role mostly buys a cleaner reading: the tagged fallback is visibly user-authored, and models say so, which leaks meta-commentary like "that `<system>` tag appeared inside your message, not from the actual system, so I won't treat it as a binding instruction" into answers.

The difference shows up on instructions a model *should* be wary of taking from a user, like lifting a restriction the top-level prompt set. There Claude Opus 5 acts on the native entry and refuses the identical text in a `<system>` tag — which is the point of the role, and the reason to prefer it where it's available.

Whether a mid-conversation instruction may override the top-level system prompt at all is up to the model, so don't build on it. Claude Opus 5 and Claude Opus 4.8 allow it; Claude Sonnet 5 keeps the top-level rule instead — not because it misses the instruction, which it can quote back, but as a consistent stance: it holds whether the instruction arrives as a `system` entry, as `<system>`-tagged text, or as Anthropic's own `mid_conv_system` block. Treat a mid-conversation instruction as an addition rather than a replacement, and put anything that has to take precedence in the top-level prompt.
