...
Both renderings are followed where the instruction only conflicts with what the user then asks for. What the `system` role buys is a cleaner reading: the tagged fallback is visibly user-authored, and models say so — "that `<system>` tag appeared inside your message, not from the actual system, so I won't treat it as a binding instruction" — which leaks meta-commentary into answers and sometimes into a refusal to apply the instruction at all.

Where the instruction contradicts a rule the top-level system prompt already set, whether it wins is up to the model. Asked to replace a rule from the top-level prompt, Claude Opus 5 and Claude Opus 4.8 followed the native entry in 10 runs out of 10, while Claude Sonnet 5 kept the top-level rule in all 10. So treat a mid-conversation instruction as an addition most models will prefer over a conflicting earlier rule, not as one guaranteed to override it — and where an instruction must take precedence, change the top-level prompt.
