...
!!! note
    A file is only downloadable when the model **runs code** that writes it to disk. Asking the model to "create a file" doesn't always produce a downloadable file, so if you rely on this feature, prompt it to run code that saves the result to disk (e.g. "write and run Python code that saves the result to a CSV file").
