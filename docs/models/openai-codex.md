...
The provider calls `load()` on first use and `save()` after every rotation. It also narrows the multi-replica race over a shared grant: because refresh tokens rotate, two replicas refreshing the same stored set would invalidate each other, so before refreshing, the provider re-reads storage and adopts a peer's newer credentials instead of spending its own. This is best-effort rather than distributed mutual exclusion (replicas can still race between `load()` and `save()`), so deployments that need a guaranteed single refresher should serialize refreshes themselves, for example with a per-user lease held around calls that may refresh.
