...
1. **Cache User Messages with [`CachePoint`][pydantic_ai.messages.CachePoint]**: Insert a `CachePoint` marker to cache everything before it in the current user message. A `CachePoint` at the start of a user prompt part marks the boundary at the end of the preceding user content instead, so parts injected behind a cache boundary (e.g. by capabilities) work as expected. Pass `CachePoint(ttl='1h')` to opt into the extended cache duration.
