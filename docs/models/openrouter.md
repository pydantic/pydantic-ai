...
Anthropic models routed through OpenRouter don't support a forced [`tool_choice`][pydantic_ai.settings.ModelSettings.tool_choice] while [thinking](../capabilities/thinking.md) is enabled: the route has no equivalent of the adaptive thinking that lifts the restriction on [the direct Anthropic API](anthropic.md#forced-tool-choice), since `reasoning` is the only knob it exposes. And where Anthropic rejects an incompatible combination outright, OpenRouter silently drops the `reasoning` field from the request instead, so the response comes back with no thinking at all. With thinking enabled on an `anthropic/` model:
