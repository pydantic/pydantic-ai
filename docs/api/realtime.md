...
Support for **realtime, bidirectional speech-to-speech models** — any provider that streams audio
in and out over a persistent connection.
...
instructions into a session and runs the tool loop for you.
...
provider-specific, as reported by
[`RealtimeModelProfile.supports_async_tool_calls`][pydantic_ai.realtime.RealtimeModelProfile.supports_async_tool_calls].
