...
!!! warning "Tool approvals and results are submitted by the client"
    ...
    Authenticate the endpoint (as above) and enforce authorization for sensitive actions inside the tool function itself — where you can read [`RunContext.tool_call_approved`][pydantic_ai.tools.RunContext.tool_call_approved] and which runs regardless of how the call entered the history — or persist paused runs server-side and pass your own `deferred_tool_results` rather than honoring the client's.
