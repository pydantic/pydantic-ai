...
- **System prompts** — client-submitted [`SystemPromptPart`][pydantic_ai.messages.SystemPromptPart]s, [`InstructionDeltaPart`][pydantic_ai.messages.InstructionDeltaPart]s, and instruction prefix metadata are stripped by default and replaced with the agent's configured instructions. Control with [`UIAdapter.manage_system_prompt`][pydantic_ai.ui.UIAdapter.manage_system_prompt]; see each adapter's docs for details. Persist [append-on-change instruction history](../agent.md#appending-instruction-changes) server-side; UI message dumps omit its operator records.
