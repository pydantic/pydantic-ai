...
- **Workspace references** — [`ModelResponse.workspace_ref`][pydantic_ai.messages.ModelResponse.workspace_ref] is reset to `None` on client-submitted messages by default, since the most recent reference in history is offered to a [workspace](../workspace.md) capability, which would attach to that environment using your server's provider credentials. Keep them by setting [`UIAdapter.strip_workspace_refs`][pydantic_ai.ui.UIAdapter.strip_workspace_refs] to `False` when the client is trusted to hold the conversation's workspace identity.
