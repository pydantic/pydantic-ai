...
- **Tool-return provenance** — [`UserPromptPart.source`][pydantic_ai.messages.UserPromptPart.source] is cleared on client-submitted messages. It records which tool call produced content that travels as a user message, and the server authors it; honoring a client-supplied value would let ordinary user content be presented to the model as tool output. Unlike the settings above this one has no opt-in, and it does not make the rendered [provenance marker](../tools-advanced.md#tool-return-provenance) unforgeable — that marker is ordinary prompt text.
