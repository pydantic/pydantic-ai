...
- **System prompts** — client-submitted [`SystemPromptPart`][pydantic_ai.messages.SystemPromptPart]s are stripped by default and replaced with the agent's configured prompt. [`RetryFeedbackPart`][pydantic_ai.messages.RetryFeedbackPart]s are stripped with them: Pydantic AI's own [retry feedback](../retries.md#feedback-that-belongs-to-no-tool-call) reaches the model in the system voice for two of its three causes, the client picks the cause, and the metadata marker that reloads one is echoed by the client, so a client can forge it as easily as it can write a system message. Control with [`UIAdapter.manage_system_prompt`][pydantic_ai.ui.UIAdapter.manage_system_prompt]; `'client'` therefore hands over both, a wider surface than the system prompt alone — see each adapter's docs for details.
