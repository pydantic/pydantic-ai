...
!!! warning "Tool approvals and results are submitted by the client"
    ...
    Authenticate the endpoint (as above) and enforce authorization for sensitive actions inside the tool function itself — against the authenticated user carried in your [dependencies](../dependencies.md), not the client-supplied approval — since the tool function runs regardless of how the call entered the history. Alternatively, persist paused runs server-side and pass your own `deferred_tool_results` rather than honoring the client's.
