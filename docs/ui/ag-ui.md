...
[Retry feedback that belongs to no tool call](../retries.md#feedback-that-belongs-to-no-tool-call) is shown to the model in the system voice, so [`AGUIAdapter.dump_messages`][pydantic_ai.ui.ag_ui.AGUIAdapter.dump_messages] writes each [`RetryFeedbackPart`][pydantic_ai.messages.RetryFeedbackPart] as a `SystemMessage` holding the same rendered text. A dumped history therefore carries a `SystemMessage` wherever the agent retried its own output, including for an agent with no configured `system_prompt` at all. The part itself rides in that message's `encrypted_value` under the same namespaced payload as [failed tool outcomes](#preserving-failed-tool-outcomes), because a `SystemMessage` has no other metadata slot, and the rendered text alone can't give back the part's `cause` or its raw content.

On the way back in, only that marker rebuilds a `RetryFeedbackPart`: a `SystemMessage` the frontend wrote loads as a [`SystemPromptPart`][pydantic_ai.messages.SystemPromptPart]. The marker separates provenance rather than proving it — the frontend echoes it, so it can also forge it — which is why a rebuilt part is stripped again by default, along with the frontend's system messages, under `manage_system_prompt='server'`.
