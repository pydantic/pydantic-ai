...
| `ResumeEntry.payload`   | `{ "approved": bool, "editedArgs"?: object, "reason"?: string }`, validated against `Interrupt.response_schema`; a payload that fails validation denies, including when the offending field is not `approved` itself (a non-dict `editedArgs` or a non-string `reason` denies even alongside `approved=True`) |
