...
The carrier here is the `encrypted_value` *field*, which `ag-ui-protocol` added to `BaseMessage` (and so to both `SystemMessage` and `UserMessage`) in 0.1.11 — the same floor as [tool outcomes](#preserving-failed-tool-outcomes), which ride the same payload on the live event stream via [`ReasoningEncryptedValueEvent`](https://github.com/ag-ui-protocol/ag-ui/blob/11f03fa65c4fa22a8637d3f6e06e77d8c1b9ae78/docs/sdk/python/core/events.mdx#L555-L577). Below 0.1.11 the field doesn't exist, so the feedback reloads as a plain `SystemPromptPart` holding the rendered text and `dump_messages` warns. That warning previously fired only for parts carrying a `tool_kind`, so an app on 0.1.10 that never touched lazy capabilities or tool search can now see it for the first time on a run that retried its output; upgrading to 0.1.11 or later is what silences it.
