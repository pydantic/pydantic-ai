...
A run that completes, fails or is [cancelled](../agent.md#cancelling-a-run) closes with a `result` record and nothing follows it, so consumers read the last line of the stream to find it. A run torn down from outside — the consuming task itself being cancelled, raising `asyncio.CancelledError` — is the exception: that propagates, truncating the stream with no `result` record at all.

On a run that completes, the record reports the turn count, duration, token usage and final answer. The final answer is the last *text* the model produced, so it is empty only for a run that produced no text at all: a [structured output](../output.md#structured-output) answers with a tool call rather than text, but any text the model wrote before that call is still reported.

How the run ended is named by `terminal_reason`, not by `is_error`, which is `true` for a cancelled run (`terminal_reason: 'cancelled'`) as much as for a failed one (`'error'`). A run that stops awaiting [tool approvals or external execution](../deferred-tools.md) is neither, and reports `terminal_reason: 'deferred_tool_requests'` with `is_error: false`. Exhausting a [usage limit](../agent.md#usage-limits) additionally reports `subtype: 'error_max_turns'`. The fields shift on the failure paths too: `result` carries the error text instead of an answer, `usage` is omitted, and a run that failed adds an `errors` array.
