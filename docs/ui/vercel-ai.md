...
`load_messages` also keeps each `UIMessage.id` in that reserved namespace, and `dump_messages` uses it as the id again, so a history the browser sent comes back with the ids the browser assigned. Pass `generate_message_id` to `dump_messages` to choose ids yourself instead.
