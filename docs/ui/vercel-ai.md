...
With the default `preserve_file_data=False`, file content in tool returns — including files nested inside dicts or lists — is dropped on `dump_messages` (any text is kept), mirroring the AG-UI adapter and how client-uploaded files ([`UploadedFile`][pydantic_ai.messages.UploadedFile]) are handled. On load, `load_messages` rehydrates file references from client-submitted history, then the shared sanitization step strips disallowed file URL schemes before the agent runs, per the [Trust model](#trust-model) below.
