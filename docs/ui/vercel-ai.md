...
`dump_messages` also writes a `role='system'` `UIMessage` wherever a run produced [retry feedback that belongs to no tool call](../retries.md#feedback-that-belongs-to-no-tool-call), including for an agent with no configured `system_prompt` at all. It holds the same text the model was shown, with the [`RetryFeedbackPart`][pydantic_ai.messages.RetryFeedbackPart] itself under a `retry_feedback` claim in the reserved `pydantic_ai` key of that text part's `providerMetadata` (not `UIMessage.metadata`), and `load_messages` rebuilds the part from that claim alone, so a system message the frontend wrote stays a [`SystemPromptPart`][pydantic_ai.messages.SystemPromptPart]. Ordering within a single [`ModelRequest`][pydantic_ai.messages.ModelRequest] is lossy as a result: system-voice parts and user content go out as two `UIMessage`s and the `role='system'` one always comes first, so feedback authored after a [`UserPromptPart`][pydantic_ai.messages.UserPromptPart] reloads before it. The [AG-UI adapter](ag-ui.md#preserving-retry-feedback) keeps the authored order; this adapter's split-by-role dump cannot.
...
- `'client'`: the frontend owns the system prompt. Frontend system messages are preserved as-is, and the agent's configured `system_prompt` is not injected — the caller is fully responsible for sending it on every turn if desired. To opt into fallback-to-configured behavior, add the [`ReinjectSystemPrompt`][pydantic_ai.capabilities.ReinjectSystemPrompt] capability to your agent. [Retry feedback](#message-metadata) is preserved along with them, so a frontend that attaches the `retry_feedback` claim to a system message of its own writing also reaches the model in the system voice. Reach for `'client'` only when the frontend is as trusted as the server.
