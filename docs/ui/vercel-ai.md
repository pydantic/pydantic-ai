...
`load_messages` also keeps each `UIMessage.id` in that reserved namespace on the message it produces, and `dump_messages` restores it, so a history the browser sent comes back with the ids the browser assigned rather than generated ones. Pass `generate_message_id` to `dump_messages` to choose ids yourself instead.
