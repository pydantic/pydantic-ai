...
- **Removes** a tool call whose arguments were cut off mid-stream (unparsable JSON): it never executed and can't be replayed. If this empties a [`ModelResponse`][pydantic_ai.messages.ModelResponse], the response is removed too.
...
- **Removes** a dangling builtin (server-side) tool call, such as an Anthropic `code_execution` call with no result. Builtin results are produced by the provider inline, so a dangling one can never be answered and can't be given a meaningful synthetic result.
...
Some provider-invalid shapes are still out of scope and may be rejected: duplicate tool results for one call, and provider-specific ordering rules beyond call/result pairing.
