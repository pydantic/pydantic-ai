...
Tool availability changes are stored as [`ToolAvailabilityDeltaPart`][pydantic_ai.messages.ToolAvailabilityDeltaPart] request parts. A tool returning [`ToolReturn(tools_added=[...])`][pydantic_ai.messages.ToolReturn] authors the part immediately after its [`ToolReturnPart`][pydantic_ai.messages.ToolReturnPart] in the same request, with the call's `tool_call_id` as a causal link. Replaying history keeps each `added` name revealed; tool definitions continue to come from the current run, so names absent from the current definitions are ignored when rendering a request.
