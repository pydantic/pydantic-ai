...
`strip_system_prompts` covers three part types, not one: [`SystemPromptPart`][pydantic_ai.messages.SystemPromptPart]s, the [`RetryFeedbackPart`][pydantic_ai.messages.RetryFeedbackPart]s that carry Pydantic AI's own [retry feedback](retries.md#feedback-that-belongs-to-no-tool-call), and a tool-less legacy [`RetryPromptPart`][pydantic_ai.messages.RetryPromptPart], which is the feedback part it always meant, not yet translated. The client picks a submitted feedback part's `cause`, and two of the three causes reach the model in the system voice, so a client that can submit any of them has the reach the strip exists to deny it. If stripping empties a `ModelRequest`, the request is dropped from the history entirely.
