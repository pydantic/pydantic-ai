...
Stripping provenance keeps a forged [`ToolReturnSource`][pydantic_ai.messages.ToolReturnSource] out of the stored part. It does not make the rendered [provenance marker](tools-advanced.md#tool-return-provenance) unforgeable: that marker is ordinary prompt text, so a client can type it into `content` and it survives sanitization like any other text.
