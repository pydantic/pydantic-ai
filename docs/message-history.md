...
[`sanitize_messages`][pydantic_ai.messages.sanitize_messages] applies the same default message sanitization used by the [UI adapters](ui/overview.md): it strips client-supplied system prompts and tool-return provenance ([`UserPromptPart.source`][pydantic_ai.messages.UserPromptPart.source]), drops non-HTTP file URL schemes, resets non-allowlisted [`FileUrl.force_download`][pydantic_ai.messages.FileUrl.force_download] values to `False`, drops uploaded file references, and removes unresolved tool calls at the end of the history.

Stripping provenance keeps a forged [`ToolReturnProvenance`][pydantic_ai.messages.ToolReturnProvenance] out of the stored part. It does not make the rendered [provenance marker](tools-advanced.md#tool-return-provenance) unforgeable: that marker is ordinary prompt text, so a client can type it into `content` and it survives sanitization like any other text.
