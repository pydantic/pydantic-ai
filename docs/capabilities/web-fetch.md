...
`max_content_tokens`, and Anthropic's `use_cache` and `response_inclusion`. `max_uses`, `use_cache`,
and `response_inclusion` require native support; domain filters are enforced locally when native
isn't available.
