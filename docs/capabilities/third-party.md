...
* [`pydantic-ai-atomicmail`](https://github.com/Atomic-Mail/atomic-mail-agentic/tree/main/py/pydantic_ai) - `AtomicMailCapability` gives an agent its own inbox on [Atomic Mail](https://atomicmail.ai), rather than wrapping a person's existing mailbox. The agent provisions the address itself through a proof-of-work `register` call — no signup, domain or card — and then reads, sends and searches over [JMAP](https://jmap.io/) (RFC 8620/8621) through a single `jmap_request` tool that takes either inline method calls or a named preset, with local-file attachments and validated placeholder substitution. A third `help` tool serves the bundled docs, so the model can look up presets and method shapes instead of guessing them. `credentials_dir` sets a default inbox per capability instance and stays overridable per call, so one process can drive several mailboxes. Also available as a lower-level [toolset](../toolsets.md) via `AtomicMailToolset`, and as a LangChain package sharing the same runtime.
