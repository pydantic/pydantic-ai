...
* [`piighost`](https://github.com/Athroniaeth/piighost) - Keeps personal data (PII) out of the model while tools and the user still get the real values. It hides sensitive values before the model sees them, then restores them in the reply and at the tool boundary. Added as `capabilities=[pii_hooks(...)]`, with pluggable regex, NER, and LLM detectors and thread-consistent tokens across a multi-turn history. Available through the `piighost[pydantic-ai]` extra.
