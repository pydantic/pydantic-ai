...
| [`acquire_sandbox()`][pydantic_ai.capabilities.AbstractCapability.acquire_sandbox] | [`SandboxRef`][pydantic_ai.sandboxes.SandboxRef] `| None` | Optionally acquire a [sandbox](../sandbox.md) for the run. The first capability returning a ref wins; the framework stamps its ID in `SandboxRef.capability_id` and routes `resolve_sandbox()` and `release_sandbox()` back to it. |
| [`resolve_sandbox()`][pydantic_ai.capabilities.AbstractCapability.resolve_sandbox] | [`SandboxBackend`][pydantic_ai.sandboxes.SandboxBackend] `| None` | Construct a backend for a sandbox reference without connecting or performing I/O |
...
`for_agent()` → bootstrap model selection and resolution → first non-`None` `acquire_sandbox()` → routed `resolve_sandbox()` → `get_wrapper_sandbox()` → `for_run()` → per-step selection and preparation → model request → tool/output processing → run completion → routed `release_sandbox()`
...
| Run bootstrap | [`get_model()`][pydantic_ai.capabilities.AbstractCapability.get_model], then [`resolve_model_id()`][pydantic_ai.capabilities.AbstractCapability.resolve_model_id] if the selection is a string; [`acquire_sandbox()`][pydantic_ai.capabilities.AbstractCapability.acquire_sandbox], [`resolve_sandbox()`][pydantic_ai.capabilities.AbstractCapability.resolve_sandbox], and [`get_wrapper_sandbox()`][pydantic_ai.capabilities.AbstractCapability.get_wrapper_sandbox] | The run's initial [`RunContext`][pydantic_ai.tools.RunContext] (no tool manager or validation context yet) |
