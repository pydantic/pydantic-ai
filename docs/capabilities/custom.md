...
`for_agent()` → bootstrap model selection and resolution → `acquire_sandbox()` → `for_run()` → per-step selection and preparation → model request → tool/output processing → run completion → `release_sandbox()`
