...
| [`create_sandbox()`][pydantic_ai.capabilities.AbstractCapability.create_sandbox] | [`SandboxRef`][pydantic_ai.sandboxes.SandboxRef] `| None` | Supply a [sandbox](../sandbox.md) for the run — do not override unless the capability supplies one; [`get_sandbox()`][pydantic_ai.capabilities.AbstractCapability.get_sandbox] connects refs and [`destroy_sandbox()`][pydantic_ai.capabilities.AbstractCapability.destroy_sandbox] destroys them |
...
`for_agent()` → `wrap_entire_run` entry → bootstrap model selection and resolution → `create_sandbox()` → initial metadata → `for_run()` → `get_toolset()` → model and toolset entry → `wrap_run` → `before_run` → per-step work → run completion → teardown (including `destroy_sandbox()`) → `wrap_entire_run` exit
...
| Run bootstrap | [`get_model()`][pydantic_ai.capabilities.AbstractCapability.get_model], then [`resolve_model_id()`][pydantic_ai.capabilities.AbstractCapability.resolve_model_id] if the selection is a string; [`create_sandbox()`][pydantic_ai.capabilities.AbstractCapability.create_sandbox] | The run's initial [`RunContext`][pydantic_ai.tools.RunContext] (no tool manager or validation context yet) |
