...
| [`get_sandbox()`][pydantic_ai.capabilities.AbstractCapability.get_sandbox] | [`SandboxBackend`][pydantic_ai.sandboxes.SandboxBackend] `| None` | Supply the run's [sandbox](../sandbox.md). Synchronous and does no I/O: the backend it returns creates or attaches on its first operation. Exactly one capability may return one. |
...
`for_agent()` → bootstrap model selection and resolution → `get_sandbox()` → `for_run()` → per-step selection and preparation → model request → tool/output processing → run completion
...
When `for_run` returns a new instance, the capability's configuration is re-extracted from that replacement at run setup: [`get_instructions`][pydantic_ai.capabilities.AbstractCapability.get_instructions], [`get_toolset`][pydantic_ai.capabilities.AbstractCapability.get_toolset], [`get_native_tools`][pydantic_ai.capabilities.AbstractCapability.get_native_tools], [`get_model_settings`][pydantic_ai.capabilities.AbstractCapability.get_model_settings], and [`get_sandbox()`][pydantic_ai.capabilities.AbstractCapability.get_sandbox] are re-invoked on it, and [`get_wrapper_toolset`][pydantic_ai.capabilities.AbstractCapability.get_wrapper_toolset], [`get_description`][pydantic_ai.capabilities.AbstractCapability.get_description], and post-binding lifecycle hooks run on it. Sandbox selection therefore uses the final per-run capability and still completes before any lifecycle hook executes, so every hook sees the final `ctx.sandbox`. Bootstrap model selection remains the exception: [`get_model()`][pydantic_ai.capabilities.AbstractCapability.get_model] and bootstrap [`resolve_model_id()`][pydantic_ai.capabilities.AbstractCapability.resolve_model_id] run *before* `for_run` on the original instance. The bootstrap model selection is reused unless the replacement actually changes the model contribution — see [Model selection lifecycle and limitations](#model-selection-lifecycle-and-limitations).
