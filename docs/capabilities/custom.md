...
| [`acquire_sandbox()`][pydantic_ai.capabilities.AbstractCapability.acquire_sandbox] | [`SandboxRef`][pydantic_ai.sandboxes.SandboxRef] `| None` | Supply a [sandbox](../sandbox.md) for the run — do not override unless the capability supplies one; [`get_sandbox()`][pydantic_ai.capabilities.AbstractCapability.get_sandbox] connects refs and [`release_sandbox()`][pydantic_ai.capabilities.AbstractCapability.release_sandbox] releases the run's ownership |
...
`for_agent()` → `wrap_entire_run` entry → bootstrap model selection and resolution → `acquire_sandbox()` → initial metadata → `for_run()` → `get_toolset()` → model and toolset entry → `wrap_run` → `before_run` → per-step work → run completion → release (including `release_sandbox()`) → `wrap_entire_run` exit
