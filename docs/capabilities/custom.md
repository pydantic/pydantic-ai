...
| [`get_workspace()`][pydantic_ai.capabilities.AbstractCapability.get_workspace] | [`WorkspaceBackend`][pydantic_ai.workspaces.WorkspaceBackend] `| None` | Supply the run's [workspace](../workspace.md). Synchronous and does no I/O: the backend it returns creates or attaches on its first operation. The first capability to return one wins. |
