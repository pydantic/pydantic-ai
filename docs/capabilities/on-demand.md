...
!!! note "Setting `id` for durable execution"
    A toolset contributed by a capability — via `Capability(tools=[...])`, the local fallback of a [`NativeOrLocalTool`][pydantic_ai.capabilities.NativeOrLocalTool] such as [`WebSearch`](web-search.md), or an [`MCP`](mcp.md) server running locally — inherits its `id` from the capability's [`id`][pydantic_ai.capabilities.AbstractCapability.id]. [Durable execution](../durable_execution/overview.md) identifies each leaf toolset by its `id`, so pass `Capability(id='...', tools=[...])` or `MCP(id='...', url='...')` when combining a capability with Temporal, DBOS, or Prefect. The single-purpose built-ins — [`WebSearch`](web-search.md), [`WebFetch`](web-fetch.md), [`ImageGeneration`](image-generation.md), and [`XSearch`](x-search.md) — name that toolset after themselves (`'web_search'`, `'web_fetch'`, `'image_generation'`, `'x_search'`), so durable execution needs no configuration from you; pass `id=` to rename it. That default is on the contributed toolset, not on the capability, so `defer_loading=True` still requires an explicit `id` — see [Resumable across runs](#resumable-across-runs). A toolset you pass in yourself — via `toolsets=`, or as the `local=` fallback of a `NativeOrLocalTool` — keeps its own `id`, which must be set on the toolset itself. Temporal requires an `id` for every leaf toolset and DBOS for every MCP server — both raise at construction without one. (`MCP` also derives one from the server URL when no `id` is given.) A URL-derived `id` can collide when two different servers share a host and final path segment (`https://a.com/api` and `https://a.com/v2/api` both derive `a.com-api`); DBOS raises at construction and Temporal when the worker starts, so pass an explicit `id` to disambiguate them.
