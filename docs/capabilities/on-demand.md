...
!!! note "Setting `id` for durable execution"
    A toolset Pydantic AI builds for a capability — via `Capability(tools=[...])` or the local fallback of [`WebSearch`](web-search.md), [`WebFetch`](web-fetch.md), [`ImageGeneration`](image-generation.md), or [`XSearch`](x-search.md) — uses the capability's [`id`][pydantic_ai.capabilities.AbstractCapability.id]. Those four single-purpose capabilities each supply a fixed default, so their local fallback works with Temporal and Prefect without extra configuration. Because the default identifies a single purpose, adding it twice is rejected; give each instance a distinct `id` when you need two configurations. Pass `id=None` only to retain the ordinary id-less local-toolset behavior, which Temporal and Prefect cannot wrap. A toolset you supply yourself via `toolsets=` or `local=AbstractToolset(...)` retains its own `id`.

    [`MCP`](mcp.md) is connection-specific: its local server toolset derives an `id` from the URL when one is not passed explicitly. A bare callable-backed MCP stays id-less. DBOS requires an `id` only for MCP and dynamic toolsets; it does not wrap ordinary function toolsets. A URL-derived MCP id can collide when two different servers share a host and final path segment (`https://a.com/api` and `https://a.com/v2/api` both derive `a.com-api`); pass an explicit id to disambiguate them.
