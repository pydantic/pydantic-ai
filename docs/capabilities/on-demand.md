...
- **[Decision models](../models/decision.md#on-demand-capabilities)** cannot fill a free-form `id`, so they offer each capability as a route of their own, described by its `description`, and leave the catalog out of the framing every question carries. The ids travel on the `load_capability` tool's `metadata`, not its schema, so what a language model is sent stays the same whichever capabilities a run has.
