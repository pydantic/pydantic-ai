...
Because these capabilities contribute model-facing tools, their `id`, `description`, and `defer_loading` fields are meaningful: set `description` and `defer_loading` when that tool should stay hidden until the model loads the matching workflow with the `load_capability` tool. [`WebSearch`][pydantic_ai.capabilities.WebSearch], [`WebFetch`][pydantic_ai.capabilities.WebFetch], [`ImageGeneration`][pydantic_ai.capabilities.ImageGeneration], and [`XSearch`][pydantic_ai.capabilities.XSearch] each have a stable default `id` (`'web_search'`, `'web_fetch'`, `'image_generation'`, and `'x_search'`). This includes [`ImageGeneration`][pydantic_ai.capabilities.ImageGeneration] when image generation should only be available for an image-specific workflow, whether it resolves to a native image tool or a fallback subagent tool.

Each of those four capabilities represents one purpose, so only add it once to an agent. To configure two instances of the same capability, give each a distinct `id`. Pass `id=None` to opt out of the fixed identity and retain the ordinary class-derived capability id and id-less local toolset behavior. [`MCP`][pydantic_ai.capabilities.MCP] remains connection-specific: it derives a local toolset id from the server URL, uses an explicit capability id for a callable fallback, and preserves the id of a supplied [`AbstractToolset`][pydantic_ai.toolsets.AbstractToolset].
