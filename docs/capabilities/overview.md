...
Each covers one fixed capability, so when its local fallback is in play each names the toolset it contributes after itself — `'web_search'`, `'web_fetch'`, `'image_generation'`, `'x_search'` — which is what [durable execution](../durable_execution/overview.md) identifies that toolset by. You don't need to configure anything for that; pass `id=` to rename it.
