...
Because these capabilities contribute model-facing tools, their `id`, `description`, and `defer_loading` fields are meaningful: set `description` and `defer_loading` when that tool should stay hidden until the model loads the matching workflow with the `load_capability` tool, and set `id` when deferring — [`defer_loading=True` requires an explicit one](on-demand.md#resumable-across-runs). This includes [`ImageGeneration`][pydantic_ai.capabilities.ImageGeneration] when image generation should only be available for an image-specific workflow, whether it resolves to a native image tool or a fallback subagent tool.

The four single-purpose ones — `WebSearch`, `WebFetch`, `ImageGeneration`, and `XSearch` — each cover one fixed capability, so when the local fallback is in play each names the toolset it contributes after itself (`'web_search'`, `'web_fetch'`, `'image_generation'`, `'x_search'`), which is what [durable execution](../durable_execution/overview.md) identifies that toolset by. You don't need to configure anything for that; pass `id=` to rename it. (`MCP` derives its toolset id from the server URL instead.)
