...
- **Roadmap alignment** -- features that align with our current focus areas get priority. Right now that includes the capabilities/hooks API, provider-adaptive tools, and the [Pydantic Harness](https://github.com/pydantic/pydantic-harness) capability library.
- **Capabilities over core** -- features that could live as a [capability](capabilities.md) should go to [Pydantic Harness](https://github.com/pydantic/pydantic-harness) or ship as your own package — that's often the fastest path. Once it has traction, come back and we can talk about upstreaming.
