...
[above](#from-a-capability) and the run owns the whole lifecycle. Give it a stable `id` so the
durable engine can register its operations. `create_sandbox` and `destroy_sandbox` are inherently
durable hooks: Temporal runs them as activities, DBOS as steps, and Prefect as tasks. Only the ref
returns to workflow or flow code, so replay reuses the recorded ref instead of provisioning again.
The create unit is at-least-once, so `create_sandbox`
...
Teardown runs at the end of the run, including a failed one, but a terminated workflow or flow
may skip the unwind entirely. A crash after create but before destroy can therefore leak the
sandbox on every engine, so **always configure a server-side idle timeout or reaper as the
backstop.** Teardown may also be invoked again after recovery and must tolerate an already-gone
sandbox.

Lifecycle suppliers must be present when the durable agent is bound. A supplier added through a
run's `capabilities=` argument cannot contribute a unit that was never registered with the worker,
so durable runs reject that shape.
...
`get_sandbox` is deliberately never a durable operation. It returns a live connection, which
cannot cross a durable boundary. Connecting happens inside the durable model, tool, event, or
capability unit that needs the sandbox.
