...
A run that also has searchable deferred tools keeps normal model-driven search for those, and where the search runs depends on how the provider reveals tools. On first-party OpenAI Responses, a reveal carries the full definition in an `additional_tools` item, so capability-owned tools stay out of `tools` entirely until their capability loads and native tool search stays server-executed — the server has nothing gated to index. On providers whose reveals reference definitions that must already be advertised with `defer_loading` (Anthropic, or a Responses-compatible endpoint without the `additional_tools` item), the wire flag that hides a capability-owned tool is the same one that puts it in the provider's searchable index, so search runs on our side instead (Anthropic `tool_reference` blocks, OpenAI `execution='client'`) and a query can't return a tool whose capability hasn't loaded.
