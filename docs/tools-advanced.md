...
Under [durable execution](durable_execution/overview.md), a tool with an `args_validator` gets a dedicated validation activity/step/task of its own, so the validator may perform I/O like the tool function itself, and can retry, fail, or defer the call from in there. Tools without one are unaffected: no extra durable unit is scheduled for them.
