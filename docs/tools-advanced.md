...
A trimmed result says so rather than passing for a complete one: it carries a [`total_matches`][pydantic_ai.messages.ToolSearchReturnContent.total_matches] count and a [`message`][pydantic_ai.messages.ToolSearchReturnContent.message] naming how many matched. Under the default keyword strategy that message invites the model to repeat the search for the next page — each page sinks below the matches still undiscovered, so an identical query walks the corpus a page at a time — and switches to advising narrower queries once nothing new is left to surface. A custom search function orders its own results, so its trimmed returns report the total and advise narrowing without promising pagination.
