...
When a run also has searchable deferred tools, first-party OpenAI Responses keeps native tool search server-executed. Capability-owned tools stay out of `tools` entirely and arrive by full definition in `additional_tools` only after loading, so the server has nothing gated to index. Anthropic additions reveal tools by reference to definitions already advertised with `defer_loading`; mixed runs therefore use client-executed search there so capability-owned tools cannot appear in search results before loading.
