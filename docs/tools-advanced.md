...
If you want to wait for all tools in a batch to finish before emitting events, you can wrap the agent run in the [`with agent.parallel_wait_all_tool_calls()`][pydantic_ai.agent.AbstractAgent.parallel_wait_all_tool_calls] context manager.
