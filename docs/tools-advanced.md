...
!!! warning
    Returning `None` from the callback disables **all** tools for that step, it is not a "pass through unchanged" shortcut. To keep every tool as-is, return the `tool_defs` argument; to expose no tools intentionally, return `[]`. A `UserWarning` is emitted when the callback returns `None` so the silent strip is surfaced during development; silence it with `warnings.filterwarnings` if the behavior is intentional.
