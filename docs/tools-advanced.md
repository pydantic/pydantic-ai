...
Runs that include tools owned by [on-demand capabilities](capabilities/on-demand.md) trade hosted-search quality for capability gating and cache stability on native-supporting providers: deferred function tools are searched by Pydantic AI through the provider's client-executed native surface, so each `load_capability` reveal can keep the prompt-cache prefix warm without exposing tools from unloaded capabilities. Runs with only deferred tools that are not capability-owned keep using the provider's hosted search.

Tool search cannot reveal tools owned by an unloaded capability. If the built-in keyword search finds no directly searchable tools but the same query matches tools behind an unloaded capability, Pydantic AI appends a system-level routing hint naming up to three matching capability IDs. The model can then call `load_capability` instead of repeating an empty search. Custom search callables keep full control of their own matching behavior and do not receive this automatic fallback.
