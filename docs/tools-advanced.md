...
You can control tool call execution using [`with agent.tool_calls_mode(mode)`][pydantic_ai.agent.AbstractAgent.tool_calls_mode]:

- `'parallel'` (default): Run tool calls in parallel, yielding events as they complete.
- `'sequential'`: Run tool calls one at a time in order. You can also pass the [`sequential`][pydantic_ai.tools.ToolDefinition.sequential] flag when registering a tool to mark it as requiring sequential execution.
