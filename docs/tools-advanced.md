...
```python {title="force_first_request.py" test="skip"}
from pydantic_ai import Agent
...
def search(query: str) -> str:
    ...
    # In a real application, this would query a database or search API
    return f'Results for "{query}": The sky is blue due to Rayleigh scattering.'
...
When multiple tools have `force_first_request=True`, the model must use at least one of them on the first request:

```python {title="force_first_request_multiple.py" test="skip"}
from pydantic_ai import Agent
...
# Model must use either search_web or search_docs first
result = agent.run_sync('What is the capital of France?')
```

_(This example is complete, it can be run "as is")_

!!! note "`run_step` semantics"
    The "first request" refers to the first model request within a single [`run()`][pydantic_ai.Agent.run] call, not the first `run()` ever called on the agent. Each `run()` starts with `run_step=1`.
...
```python {title="dynamic_tool_choice.py" test="skip"}
...
def my_tool_choice(ctx: RunContext[None]) -> str | list[str] | None:
    """Control tool availability based on the current step."""
    if ctx.run_step == 1:
        return ['search']  # Force search on first request
    elif ctx.run_step == 2:
        return 'auto'  # Let model decide on second request
    return None  # Use default behavior for subsequent requests
...
The callable receives a [`RunContext`][pydantic_ai.RunContext] and should return one of:

- `None` — Use default PydanticAI behavior (auto/required based on output type)
- `'auto'` — Model decides whether to use tools
- `'required'` — Model must use a tool
- `'none'` — Model cannot use function tools (but can still use output tools)
- `list[str]` — Model must use one of the named tools

### Precedence Rules

When both callable `tool_choice` and `force_first_request` are set, the callable takes precedence. A warning is emitted in this case:

```python {title="tool_choice_precedence.py" test="skip"}
from pydantic_ai import Agent, RunContext
...
def always_auto(ctx: RunContext[None]) -> str:
    return 'auto'


# Warning: force_first_request is ignored because callable tool_choice is set
result = agent.run_sync(
    'Hello',
    model_settings={'tool_choice': always_auto},
)
```
