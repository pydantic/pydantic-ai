...
`run_event_stream` wraps the full event stream as an async generator. `event` observes individual events at the same dispatch point as capability [`on_event`][pydantic_ai.capabilities.on_event] listeners, which is downstream of every `run_event_stream` wrapper: observers are terminal, so everything that can rewrite, replace or drop an event runs ahead of them. Callbacks can be synchronous or asynchronous and return `None`.
...
Like `hooks.on.event`, they dispatch *downstream* of every `run_event_stream` wrapper, so a listener sees each event exactly as consumers receive it: rewrites and replacements are already applied, and an event a wrapper dropped never reaches a listener. Use `run_event_stream` when you need to change the stream rather than observe it.
