...
To dynamically modify or filter the available output tools during an agent run, you can define an agent-wide `prepare_output_tools` function that will be called ahead of each step of a run. This function should be of type [`ToolsPrepareFunc`][pydantic_ai.tools.ToolsPrepareFunc], which takes the [`RunContext`][pydantic_ai.tools.RunContext] and a list of [`ToolDefinition`][pydantic_ai.tools.ToolDefinition], and returns a new list of tool definitions. It follows the same return-value rules as the [`prepare_tools` function](tools-advanced.md#prepare-tools): return the `tool_defs` argument to keep every output tool as-is, `[]` to expose no output tools intentionally, or `None` to disable all output tools for that step and emit a `PydanticAIDeprecationWarning` (will raise in v2.0).
