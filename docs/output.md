...
!!! note
    When streaming, [`stream_text()`][pydantic_ai.result.StreamedRunResult.stream_text] yields the raw model text and does **not** apply the `TextOutput` function. To stream the transformed output, use [`stream_output()`][pydantic_ai.result.StreamedRunResult.stream_output] instead. See [Streaming Text](#streaming-text) for details.
...
!!! note "`stream_text()` yields the raw model text"
    [`stream_text()`][pydantic_ai.result.StreamedRunResult.stream_text] always yields the plain text generated by the model. It does **not** apply [`TextOutput`](#text-output) functions, and it only applies [output validators](#output-validator-functions) when `delta=False`. To stream the value transformed by your `TextOutput` function (or output validators), use [`stream_output()`][pydantic_ai.result.StreamedRunResult.stream_output] instead.
