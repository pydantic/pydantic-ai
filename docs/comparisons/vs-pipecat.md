...
| Interfaces | WebRTC and WebSocket transports, telephony, client SDKs | [CLI](../cli.md), [web chat](../web.md), [AG-UI](../ui/ag-ui.md), [Vercel AI](../ui/vercel-ai.md), [ACP](https://pydantic.dev/docs/ai/harness/acp/) (experimental), [A2A](../interfaces.md), [GitHub Agentic Workflows](https://pydantic.dev/docs/ai/harness/gh-aw/) |
...
| Structured conversation flows | Pipecat Flows, in the core package: a graph of nodes, declarative YAML or JSON or programmatic Python, with a visual editor; cascaded text LLMs only, not speech-to-speech | No flow graph; instructions and tools steer the call, and [`pydantic-graph`](../graph.md) is for text agents |
