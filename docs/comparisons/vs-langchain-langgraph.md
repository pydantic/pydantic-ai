...
agent = Agent('openai:gpt-5.6-luna', output_type=[str, DeferredToolRequests])
...
# Hours later, in another process: persist `requests` (or `build_results`), not only the messages.
...
| Trusted state | `context_schema` (invocation context via `runtime.context`); `state_schema` is graph state | `deps_type`, a typed argument tools read through `RunContext` |
