...
| Google | ✅ | No parameter support. No [`NativeToolCallPart`][pydantic_ai.messages.NativeToolCallPart] or [`NativeToolReturnPart`][pydantic_ai.messages.NativeToolReturnPart] is generated when streaming. Gemini 3 supports combining native tools with function tools and [`NativeOutput`][pydantic_ai.output.NativeOutput]. With earlier Gemini models, use [`PromptedOutput`][pydantic_ai.output.PromptedOutput] for structured output alongside native tools; combining native tools with function tools requires Gemini 3. |
...
| Google | ✅ | Gemini 3 supports combining native tools with function tools and [`NativeOutput`][pydantic_ai.output.NativeOutput]. With earlier Gemini models, use [`PromptedOutput`][pydantic_ai.output.PromptedOutput] for structured output alongside native tools; combining native tools with function tools requires Gemini 3. |
...
| Google | ✅ | No parameter support. The limits are fixed at 20 URLs per request with a maximum of 34MB per URL. Gemini 3 supports combining native tools with function tools and [`NativeOutput`][pydantic_ai.output.NativeOutput]. With earlier Gemini models, use [`PromptedOutput`][pydantic_ai.output.PromptedOutput] for structured output alongside native tools; combining native tools with function tools requires Gemini 3. |
...
| Google (Gemini) | ✅ | Requires files to be uploaded via the [Gemini Files API](https://ai.google.dev/gemini-api/docs/files). Files are automatically deleted after 48 hours. Supports up to 2 GB per file and 20 GB per project. Gemini 3 supports combining native tools with function tools and [`NativeOutput`][pydantic_ai.output.NativeOutput]. With earlier Gemini models, use [`PromptedOutput`][pydantic_ai.output.PromptedOutput] for structured output alongside native tools; combining native tools with function tools requires Gemini 3. |
