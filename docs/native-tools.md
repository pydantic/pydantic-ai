...
| Google | ✅ | No parameter support. Web search sources from Gemini grounding chunks are exposed as [`NativeToolCallPart`][pydantic_ai.messages.NativeToolCallPart] and [`NativeToolReturnPart`][pydantic_ai.messages.NativeToolReturnPart]; for streamed responses, Gemini sends grounding metadata after text, so these parts are appended after the streamed text. If Gemini omits grounding chunks in a streamed response, there are no source URLs to expose. Using native tools and function tools (including [output tools](output.md#tool-output)) at the same time is not supported; to use structured output, use [`PromptedOutput`](output.md#prompted-output) instead. |
