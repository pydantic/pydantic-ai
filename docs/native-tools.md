...
| Google | ✅ | No parameter support. Gemini grounding sources are exposed as [`NativeToolReturnPart`][pydantic_ai.messages.NativeToolReturnPart] content. Streaming responses reconstruct the native tool pair when Gemini does not emit one. For explicit server-side tool invocations, the raw Gemini response is preserved in `provider_details['raw_tool_response']`. See [Google tool combinations](#google-tool-combinations). |
