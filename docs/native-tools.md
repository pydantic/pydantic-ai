...
| Google | ✅ | No parameter support. Web search sources from Gemini grounding chunks are exposed as the [`NativeToolReturnPart`][pydantic_ai.messages.NativeToolReturnPart] content; on models that return explicit server-side tool invocations (Gemini 3+), the raw Gemini tool response is preserved in `provider_details['raw_tool_response']`. On older models the [`NativeToolCallPart`][pydantic_ai.messages.NativeToolCallPart] / [`NativeToolReturnPart`][pydantic_ai.messages.NativeToolReturnPart] pair is reconstructed from grounding metadata, which Gemini streams after the text, so those parts are appended after the streamed text. If Gemini 3+ omits grounding chunks, there are no source URLs to expose and the return content stays the raw Gemini tool response; on older models the reconstructed return content is `None`. Using native tools and function tools (including [output tools](output.md#tool-output)) at the same time is not supported; to use structured output, use [`PromptedOutput`](output.md#prompted-output) instead. |
