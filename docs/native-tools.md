...
| Google | ✅ | No parameter support. On Gemini 3+, grounding sources from explicit Web Search invocations are exposed as [`NativeToolReturnPart`][pydantic_ai.messages.NativeToolReturnPart] content for streamed and non-streamed responses. Earlier models do not generate native tool parts when streaming. See [Google tool combinations](#google-tool-combinations). |
