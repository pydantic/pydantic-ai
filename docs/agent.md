...
### Streaming {#streaming-all-events}

[`run_stream()`][pydantic_ai.agent.AbstractAgent.run_stream] streams the agent's final output as the model produces it. [`run_stream_events()`][pydantic_ai.agent.AbstractAgent.run_stream_events], and the `event_stream_handler` argument that `run()` and `run_stream()` both accept, stream every event from the model's response and the agent's execution of tools, including [custom events](streaming.md#custom-events) your own tools emit.

See [Streaming](streaming.md) for all of it, including [which method to reach for](streaming.md#choosing) and [the events you can receive](streaming.md#event-reference).
...
Each node can be streamed individually with `node.stream(run.ctx)`, which is how you take raw events from some nodes and validated output from others in the same run. See [Streaming events and output with `iter`](streaming.md#streaming-all-events-and-output).
