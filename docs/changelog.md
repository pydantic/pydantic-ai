...
- MCP: `MCPServerStdio`/`MCPServerSSE`/`MCPServerStreamableHTTP`/`MCPServerHTTP`, `FastMCPToolset`, `load_mcp_servers`, and `Agent.run_mcp_servers()` are removed; use `pydantic_ai.mcp.MCPToolset`, `pydantic_ai.mcp.load_mcp_toolsets`, and `async with agent:`. `Agent.set_mcp_sampling_model()` is unchanged and still sets the sampling model on every registered `MCPToolset`; `MCPToolset(sampling_model=...)` sets it on one toolset at construction. Note that the new `MCPToolset` defaults differ (e.g. `max_retries`, `read_timeout`, `init_timeout`, `elicitation_handler`). Announced via [#5325](https://github.com/pydantic/pydantic-ai/pull/5325); removed in [#5337](https://github.com/pydantic/pydantic-ai/pull/5337).
