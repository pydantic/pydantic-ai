...
An agent is only as good as the loop around its model: how context is assembled, how tool results and failures flow back, what gets retried, when the run is allowed to stop, and what it all costs. As models have gotten stronger, this layer — the harness — has replaced the model as the bottleneck: the same model in two different harnesses can differ more than two model generations in the same harness. Pydantic AI makes the harness layer explicit and typed. Agents are built from composable [capabilities](capabilities.md) — [web search](capabilities.md#provider-adaptive-tools), [thinking](capabilities.md#thinking), or ones you write yourself — the way web apps are built from middleware, with heavier batteries like sandboxed code execution available in the [Pydantic AI Harness](harness/overview.md) library.
