...
- Pin an explicit `{#custom-id}` on a heading whose generated id would surprise someone writing a link to it — `github-slugger` drops punctuation and turns every space into its own dash, so `## Tools & native abilities` is `#tools--native-abilities` (doubled, the `&` vanished between two spaces) and `## SIP/telephony bridge` is `#siptelephony-bridge` (glued, the `/` vanished inside a word); a literal `-` is kept rather than dropped, and compounds it: `## Agentic Chat - Code` is `#agentic-chat---code` — An explicit id is stable across slugifier changes and reads better than the generated one; `.github/workflows/ci.yml` fails the build on an unresolvable anchor in `docs/`, `examples/`, or a root `*.md`, though not on one inside a docstring
<!-- rule:66 -->
