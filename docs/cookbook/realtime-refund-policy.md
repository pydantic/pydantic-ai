...
async def main() -> None:
    realtime = agent.realtime(
        'openai:gpt-realtime',
        deps=Account(customer_id='cus_123', order_ids={'B200'}),
...
The tool receives `customer_id` from the authenticated application boundary, not from model-generated arguments. The policy handler answers immediately and the denied call is retained in history without executing the refund. Use a standard agent run when a person must approve asynchronously after the call has paused.
