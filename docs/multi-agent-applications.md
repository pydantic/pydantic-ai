...
!!! warning "Delegate from an `async def` function, not a sync one"
    Note that `joke_factory` above is `async def` and uses `await joke_generation_agent.run(...)`. That's required, not stylistic: sync tools and [output functions](output.md#output-functions) run in worker threads, and calling [`run_sync()`][pydantic_ai.agent.AbstractAgent.run_sync] there would try to run the delegate on a second event loop. Async resources used by the agents can be bound to the parent loop, so the two runs can deadlock.

    The standard agent implementation therefore raises [`UserError`][pydantic_ai.exceptions.UserError] if [`run_sync()`][pydantic_ai.agent.AbstractAgent.run_sync] or [`run_stream_sync()`][pydantic_ai.agent.AbstractAgent.run_stream_sync] is called from a framework-managed sync callback. The parent agent can still be started with `run_sync()`, as in the example above — only the delegating callback has to be `async def`. If it also needs to do blocking work, keep it `async def` and push just that part into [`asyncio.to_thread()`][asyncio.to_thread].
