...
!!! note "Setting `id` for durable execution"
    A toolset contributed by a capability — via `Capability(tools=[...])` or an [`MCP`](#mcp) server running locally — inherits its `id` from the capability's [`id`][pydantic_ai.capabilities.AbstractCapability.id]. [Durable execution](durable_execution/overview.md) requires every leaf toolset to have a stable, unique `id`, so pass `Capability(id='...', tools=[...])` or `MCP(id='...', url='...')` when combining a capability with Temporal, DBOS, or Prefect. (`MCP` also derives one from the server URL when no `id` is given.)
