...
!!! warning
    Both capabilities run through the same `PreparedToolset` wrapper as the `Agent` kwargs, so the [`None` → "disable all" semantics and `UserWarning`](tools-advanced.md#prepare-tools) apply equally here: return the `tool_defs` argument for passthrough, or `[]` to intentionally expose no tools.
