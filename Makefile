...
test: ## Run tests and collect coverage data. NOTE: Inline Snapshots will NOT be updated because xdist is used. Run test-update-inline-snapshots if you need them updated
	uv run pytest --cov -n auto --dist=loadgroup
...
test-all-python: ## Run tests on Python 3.9 to 3.13
	UV_PROJECT_ENVIRONMENT=.venv39 uv run --active --python 3.9 --all-extras --all-packages pytest --cov -n auto --dist=loadgroup
	UV_PROJECT_ENVIRONMENT=.venv310 uv run --active --python 3.10 --all-extras --all-packages pytest --cov -n auto --dist=loadgroup
	UV_PROJECT_ENVIRONMENT=.venv311 uv run --active --python 3.11 --all-extras --all-packages pytest --cov -n auto --dist=loadgroup
	UV_PROJECT_ENVIRONMENT=.venv312 uv run --active --python 3.12 --all-extras --all-packages pytest --cov -n auto --dist=loadgroup
	UV_PROJECT_ENVIRONMENT=.venv313 uv run --active --python 3.13 --all-extras --all-packages pytest --cov -n auto --dist=loadgroup
